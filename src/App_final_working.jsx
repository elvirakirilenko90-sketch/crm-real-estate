...
function Feed({posts,setPosts,role}) {
  ...
  async function uploadToNewsMedia(newsId) {
    if (draft.kind === "Ссылка") {
      ...
      const { data, error } = await supabase
        .from("news_media")
        .insert({news_id: newsId, media_type: "Ссылка", media_url: null, file_name: null, link_url: draft.file})
        .select()
        .single();
    ...
    const upload = await supabase.storage.from("news-media").upload(path, fileObj, { upsert: true, contentType: fileObj.type || undefined });
    if (upload.error) { alert("Ошибка загрузки файла: " + upload.error.message); return null; }

    const publicUrl = supabase.storage.from("news-media").getPublicUrl(path).data.publicUrl;
    const { data, error } = await supabase
      .from("news_media")
      .insert({news_id: newsId, media_type: draft.kind, media_url: publicUrl, file_name: originalName, link_url: null})
      .select()
      .single();
  ...
  const publish = async () => {
    ...
    const { data: news, error } = await supabase
      .from("news")
      .insert({title: draft.text ? draft.text.slice(0,80) : "Публикация", content: draft.text || "", likes_count: 0, comments_count: 0})
      .select()
      .single();
    ...
    const kind = media?.media_type || (mediaUrl ? draft.kind : "Текст");

    setPosts(prev => [{
      id: String(news.id), author: "Администратор", date: news.created_at ? new Date(news.created_at).toLocaleString() : new Date().toLocaleString(),
      text: news.content || "", kind, file: mediaUrl, likes: 0, comments: []
    }, ...prev]);
  ...
  const deletePost = async (post) => {
    ...
    try { await supabase.from("comments").delete().eq("news_id", Number(post.id)); } catch {}
    try { await supabase.from("comments").delete().eq("post_id", Number(post.id)); } catch {}
  ...
  async function insertComment(postId, text) {
    const variants = [
      ...
      {entity_type: "news", entity_id: String(postId), text, author_name: role || "CRM"}
    ];
  ...
  const addComment = async (post) => {
    ...
    if (!saved) { alert("Комментарий не записался в таблицу comments. Проверь колонки comments."); return; }
  ...
  const renderPostMedia = (post) => {
    ...
    if (String(post.kind).toLowerCase().includes("ссылка")) {
      return <a className="link" href={normalizeUrl(post.file)} target="_blank" rel="noopener noreferrer">🔗 Открыть ссылку</a>;
    }
    return <Media kind={post.kind} name={post.file} onOpen={setViewer}/>;
  };
  ...
  return <main className="screen"><div className="feedList">
    ...
    {open && isTech && <div className="card composer">
      ...
      {draft.kind === "Ссылка" ? <input className="input" placeholder="Вставьте ссылку" value={draft.file} onChange={e=>setDraft({...draft,file:e.target.value})}/> : <input className="input" type="file" accept="image/*,video/*,.pdf,.doc,.docx,.xls,.xlsx" onChange={e=>setFileObj(e.target.files?.[0] || null)}/>}      
...
function Clients({leads,setLeads,onOpen}) {
  ...
  const visible = leads.filter(l => stage==="Все" || l.status===stage);
  ...
  return <main className="screen">
    ...
    <div className="chips">
      <button className={cn("chip",stage==="Все"&&"active")} onClick={()=>setStage("Все")}>Все</button>
      {stages.map(s=><button key={s} className={cn("chip",stage===s&&"active")} onClick={()=>setStage(s)}>{s}</button>)}
    </div>

    <section className="kanbanCol" style={{width:"100%"}}>
      <div className="kanbanHead"><b>Клиенты</b>{badge(visible.length,"gold")}</div>
      {visible.map(l=><div className="lead" key={l.id}>
        <div className="leadTop"><section><b>{l.name}</b><span>{l.id} · {l.source}</span></section>{badge(l.status,"dark")}</div>
        <div className="phone"><b>☎ {l.phone}</b><p>{l.notes || "Без заметки"}</p></div>
        <div className="grid4">
...
function Contact({phone,onClose}) {
  ...
  return <Modal onClose={onClose}>
    ...
    <a className="btn purple full" href={`viber://chat?number=${encodeURIComponent(plusPhone)}`}>Viber</a>
    <a className="btn green full" href={`https://wa.me/${digits}`} target="_blank" rel="noreferrer">WhatsApp</a>
    <div className="card amber">На телефоне откроется установленное приложение. Если Viber на компьютере не открылся, проверь установку Viber Desktop.</div>
  </Modal>;
}
...
function LeadModal({lead,setLeads,setEvents,setHelp,onClose}) {
  ...
  const save = () => {
    setLeads(prev=>prev.map(l=>l.id===local.id?{
      ...local,
      history:[...(local.history || []), `Сохранены изменения: ${new Date().toLocaleString()}`]
    }:l));
    onClose();
  };
...
function Access({managers,setManagers,agencies,setAgencies,users,setUsers}) {
  const [agency,setAgency] = useState({name:"",email:"",phone:""});
  const [user,setUser] = useState({agency_id:"",name:"",email:"",role:"Менеджер по продажам"});

  const createAgency = async () => {
    if (!agency.name.trim()) return;
    const payload = {name: agency.name.trim(), email: agency.email || null, phone: agency.phone || null};
  ...
  const deleteAgency = async (id) => {
    if (!window.confirm("Удалить агентство и закрыть доступ?")) return;
  ...
  const createUser = async () => {
    ...
    const payload = {agency_id: agencyId, name: user.name.trim(), email: user.email.trim(), role: user.role, access: true};
    const { data, error } = await supabase.from("users").insert(payload).select().single();
    if (error) { alert("Ошибка создания пользователя: " + error.message); return; }
    setUsers(prev => [data, ...prev]);
    setUser({agency_id: agencyId || "", name:"", email:"", role:"Менеджер по продажам"});
  };

  const toggleUser = async (u) => {
    const access = !(u.access ?? true);
    const { error } = await supabase.from("users").update({access}).eq("id", u.id);
    if (error) { alert("Ошибка изменения доступа: " + error.message); return; }
    setUsers(prev => prev.map(x => String(x.id) === String(u.id) ? {...x, access} : x));
  };
  ...
  return <main className="screen">
    <div className="card dark"><h2>Доступ агентствам недвижимости</h2><p>Сначала создай главное агентство, потом открывай доступ пользователям этого агентства.</p></div>
    ...
    <div className="card">
      ...
      <input className="input" placeholder="Email агентства" value={agency.email} onChange={e=>setAgency({...agency,email:e.target.value})}/>
      <input className="input" placeholder="Телефон агентства" value={agency.phone} onChange={e=>setAgency({...agency,phone:e.target.value})}/>
    ...
    <div className="card">
      ...
      <input className="input" placeholder="Имя пользователя" value={user.name} onChange={e=>setUser({...user,name:e.target.value})}/>
    ...
    {agencies.map(a=>{
      ...
      return <div className="card" key={a.id}>
        <div className="row"><section><h3>{a.name}</h3><p className="muted">{a.email || "Без email"} · {a.phone || "Без телефона"}</p></section><Button variant="danger" onClick={()=>deleteAgency(a.id)}>Удалить агентство</Button></div>
        {list.length === 0 && <p className="muted">Пока нет пользователей.</p>}
        {list.map(u=><div className="lead" key={u.id}>
          <div className="row"><section><b>{u.name}</b><p className="muted">{u.email}<br/>{u.role}</p></section>{badge((u.access ?? true) ? "Доступ открыт" : "Доступ закрыт", (u.access ?? true) ? "green" : "red")}</div>
          <div className="grid2"><Button variant="soft" onClick={()=>toggleUser(u)}>{(u.access ?? true) ? "Закрыть доступ" : "Открыть доступ"}</Button><Button variant="danger" onClick={()=>deleteUser(u)}>Удалить пользователя</Button></div>
        </div>)}
      </div>;
    })}
  </main>;
...
function More({role,setRole,setPage}) {
  const isTech = role === "Администратор тех отдел";
  return <main className="screen">
    ...
    {isTech && <div className="card"><h3>Роль для проверки CRM</h3><p className="muted">Переключатель ролей доступен только администратору тех отдела.</p><select className="input" value={role} onChange={e=>setRole(e.target.value)}><option>Менеджер по продажам</option><option>Администратор директор</option><option>Администратор тех отдел</option></select></div>}
    ...
    {role==="Менеджер по продажам"&&<div className="card amber">Менеджеру доступны рабочие разделы, язык и личная аналитика.</div>}
  </main>;
}
...
export default function App(){
  ...
  const comments = (commentRows || [])
    .filter(c => String(c.news_id || c.post_id || c.entity_id) === String(row.id) || (c.entity_type === "news" && String(c.entity_id) === String(row.id)))
  ...
  return {
    ...
    kind: media?.media_type || (media?.link_url || media?.media_url ? "Фото" : "Текст"),
...
async function loadFromSupabase(){
  ...
  const agenciesRes = await supabase.from("agencies").select("*");
  ...
  if (postsRes.error) {
    ...
    setPostsRaw((postsRes.data || []).map(row => postFromDb(row, mediaRows, commentRows)));
  }
...
function setPosts(next){
  ...
  return <div className="layout"><Sidebar page={page} setPage={setPage} role={role}/><div className="mainWrap"><main className="main"><Top page={page} setPage={setPage}/>{page==="feed"&&<Feed posts={posts} setPosts={setPosts} role={role}/>} {page==="clients"&&<Clients leads={leads} setLeads={setLeads} onOpen={setLead}/>} {page==="calendar"&&<Calendar events={events} setEvents={setEvents} leads={leads} setLeads={setLeads}/>} {page==="properties"&&<Properties properties={properties} setProperties={setProperties} onOpen={setProperty}/>} {page==="analytics"&&<Analytics leads={leads} properties={properties} events={events} role={role}/>} {page==="help"&&<Help help={help} setHelp={setHelp} setLeads={setLeads} leads={leads} onOpen={setLead}/>} {page==="access"&&role==="Администратор тех отдел"&&<Access managers={managers} setManagers={setManagers} agencies={agencies} setAgencies={setAgencies} users={users} setUsers={setUsers}/>} {page==="more"&&<More role={role} setRole={setRole} setPage={setPage}/>}<Bottom page={page} setPage={setPage}/>{lead&&<LeadModal lead={lead} setLeads={setLeads} setEvents={setEvents} setHelp={setHelp} onClose={()=>setLead(null)}/>} {property&&<PropertyModal property={property} setProperties={setProperties} onClose={()=>setProperty(null)}/>}</main></div></div>
}
