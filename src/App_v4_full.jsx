...
function More({role,setRole,setPage}) {
  ...
  return <main className="screen">
    ...
    {isTech && <div className="card">
      ...
      <p className="muted">Этот переключатель доступен только тех отделу.</p>
    ...
    {isTech&&<div className="card row"><section><h3>Доступ агентствам недвижимости</h3><p className="muted">Менеджеры, роли, агентства и доступы</p></section><Button onClick={()=>setPage("access")}>Открыть</Button></div>}

    {role==="Менеджер по продажам"&&<div className="card amber">Менеджеру доступны язык, личная аналитика и рабочие разделы.</div>}
  </main>;
}
