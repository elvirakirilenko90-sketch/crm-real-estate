...
function PropertyModal({property,setProperties,onClose,users,currentProfile,role,lang}) {
  ...
  async function addMediaBatch() {
    ...
    for (const f of files) {
      ...
      const safeName = `${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
      const path = `${propertyId}/${safeName}`;
      const upload = await uploadToFirstAvailableBucket(["property-media","property_media","properties-media"], path, f, {upsert:true, contentType:f.type || undefined});
      ...
      setProperties(prev => {
        const exists = prev.some(p => String(p.id) === String(propertyId));
        if (!exists) return [{...local, id:String(propertyId), media:[item]}, ...prev];
        return prev.map(p => String(p.id) === String(propertyId) ? {...p, media:[...(p.media || []), item]} : p);
      });
    }
