...
const extraEn = {
    ...
    "Режим просмотра.":"View mode.","Редактировать этот объект может только ответственный менеджер или администратор тех отдела. Ты можешь смотреть объект, связаться с ответственным менеджером и отправить презентацию клиенту.":"Only the responsible manager or tech admin can edit this property. You can view it, contact the responsible manager and send a presentation to the client.",
...
function storageErrorText(error){
  if (!error) return "Неизвестная ошибка Supabase Storage";
  const parts = [
    error.message,
    error.error,
...
async function uploadPresentationPdf(path, blob){
  // ВАЖНО: презентации отправляем только через специально созданный public bucket.
  // Не маскируем ошибку попытками сохранить PDF в другие buckets.
  ...
  if (upload.error) {
    return { error: upload.error, publicUrl: "" };
  }

  const result = supabase.storage.from("presentations").getPublicUrl(path);
  const publicUrl = result?.data?.publicUrl || "";

  if (!/^https?:\/\//i.test(publicUrl)) {
    return {
      error: new Error("Supabase загрузил PDF, но не вернул публичную https-ссылку. Проверь, что bucket presentations отмечен PUBLIC."),
      publicUrl: ""
    };
  }
...
function openWhatsAppPresentation({phone,url,lang,agencyName,clientName,title,isSelection=false}) {
  if (!url || !/^https?:\/\//i.test(url)) {
    alert("Для отправки через WhatsApp нужна публичная ссылка на PDF. Проверь public bucket presentations в Supabase Storage.");
    return false;
  }
  ...
  const message = presentationSendText(lang,{agencyName,clientName,title,isSelection,url});
  const whatsappUrl = `https://wa.me/${digits}?text=${encodeURIComponent(message)}`;

  // Работает и на телефоне, и на компьютере через WhatsApp Web.
  const opened = window.open(whatsappUrl, "_blank", "noopener,noreferrer");
  if (!opened) window.location.href = whatsappUrl;
  return true;
}

function openViberPresentation({phone,url,lang,agencyName,clientName,title,isSelection=false}) {
  if (!url || !/^https?:\/\//i.test(url)) {
    alert("Для отправки через Viber нужна публичная ссылка на PDF.");
    return false;
  }
  const digits = messengerPhoneDigits(phone);
  if (!digits) {
  ...
  const message = presentationSendText(lang,{agencyName,clientName,title,isSelection,url});
  const viberChatUrl = `viber://chat?number=${encodeURIComponent(plusPhone)}`;

  // Для обычного Viber нет надежного публичного deep-link, который одновременно
  // открывает чат конкретного номера И автоматически вставляет произвольный текст.
  // Поэтому используем уже рабочую логику CRM: копируем готовое сообщение
  // (имя клиента + агентство + ссылка на PDF) и открываем чат конкретного клиента.
  const openChat = () => {
...
function Sidebar({page,setPage,role,lang}) {
  ...
  const items = [["feed","Лента","⌂"],["clients","Канбан и клиенты","👥"],["calendar","Календарь","📅"],["properties","Вторичка","🏢"],["deals","Сделки","💼"],["analytics","Аналитика","📊"],...(canSeeHelp ? [["help","Помощь менеджерам","🔔"]] : []),...(role === "Администратор тех отдел" ? [["access","Доступы","🔐"]] : []),["more","Ещё","☰"]];
...
function Top({page,setPage,agencyName,lang}) {
  const titles = {feed:"Лента новостей",clients:"Клиенты",calendar:"Календарь",properties:"Вторичка",deals:"Сделки",analytics:"Аналитика",help:"Помощь менеджерам",access:"Доступы",more:"Еще"};
...
function addSelectionComparisonPages(pages,{items,themeId,visualId,lang,agencyName}){
  ...
  // Final analytical summary page.
  const validPrice=items.filter(p=>Number(p.price)>0), validArea=items.filter(p=>Number(p.area)>0), validPpm=items.filter(p=>Number(p.price)>0&&Number(p.area)>0);
  const cheapest=validPrice.length?validPrice.reduce((a,b)=>Number(a.price)<=Number(b.price)?a:b):null;
  const largest=validArea.length?validArea.reduce((a,b)=>Number(a.area)>=Number(b.area)?a:b):null;
  const bestPpm=validPpm.length?validPpm.reduce((a,b)=>(Number(a.price)/Number(a.area))<=(Number(b.price)/Number(b.area))?a:b):null;
  const avgPrice=validPrice.length?Math.round(validPrice.reduce((sum,p)=>sum+Number(p.price),0)/validPrice.length):0;
  const avgArea=validArea.length?Math.round(validArea.reduce((sum,p)=>sum+Number(p.area),0)/validArea.length):0;
  const labels=lang==="en"?{title:"SELECTION ANALYTICS",count:"Properties selected",avgPrice:"Average price",avgArea:"Average area",cheapest:"Lowest price",largest:"Largest area",best:"Best price per m²"}:lang==="uk"?{title:"АНАЛІТИКА ДОБІРКИ",count:"Об'єктів у добірці",avgPrice:"Середня ціна",avgArea:"Середня площа",cheapest:"Найнижча ціна",largest:"Найбільша площа",best:"Найкраща ціна за м²"}:{title:"АНАЛИТИКА ПОДБОРКИ",count:"Объектов в подборке",avgPrice:"Средняя цена",avgArea:"Средняя площадь",cheapest:"Самая низкая цена",largest:"Самая большая площадь",best:"Лучшая цена за м²"};
  const a=document.createElement("canvas");a.width=1240;a.height=1754;const ctx=a.getContext("2d");ctx.fillStyle=pal.paper;ctx.fillRect(0,0,1240,1754);ctx.fillStyle=pal.ink;ctx.font='bold 52px Georgia, "Times New Roman", serif';ctx.fillText(labels.title,80,125);ctx.fillStyle=pal.accent;ctx.fillRect(80,155,1080,4);
  const metrics=[[labels.count,String(items.length)],[labels.avgPrice,avgPrice?money(avgPrice):"—"],[labels.avgArea,avgArea?`${avgArea} м²`:"—"]];
  metrics.forEach(([label,value],i)=>{const x=80+i*360;ctx.strokeStyle=pal.line;ctx.lineWidth=2;ctx.strokeRect(x,245,320,180);ctx.fillStyle=pal.muted;ctx.font='20px Arial';ctx.fillText(label,x+24,292);ctx.fillStyle=pal.ink;ctx.font='bold 38px Georgia, "Times New Roman", serif';ctx.fillText(value,x+24,360);});
  const insights=[[labels.cheapest,cheapest,cheapest?money(cheapest.price):"—"],[labels.largest,largest,largest?`${largest.area} м²`:"—"],[labels.best,bestPpm,bestPpm?money(Math.round(Number(bestPpm.price)/Number(bestPpm.area)))+" / м²":"—"]];
  let y=560;insights.forEach(([label,p,value])=>{ctx.fillStyle=pal.accent;ctx.font='bold 21px Arial';ctx.fillText(label.toUpperCase(),80,y);ctx.fillStyle=pal.ink;ctx.font='bold 34px Georgia, "Times New Roman", serif';pdfDrawWrapped(ctx,p?.title||"—",80,y+55,760,42);ctx.fillStyle=pal.ink;ctx.font='bold 31px Arial';ctx.fillText(value,900,y+55);ctx.strokeStyle=pal.line;ctx.beginPath();ctx.moveTo(80,y+125);ctx.lineTo(1160,y+125);ctx.stroke();y+=235;});
  ctx.fillStyle=pal.muted;ctx.font='18px Arial';ctx.fillText(`${agencyName||"CRM Real Estate"} · ${formatPresentationDate(lang)}`,80,1665);
  pages.push({bytes:pdfDataUrlToBytes(a.toDataURL("image/jpeg",.97)),width:1240,height:1754});
}
...
function Properties({properties,setProperties,onOpen,users,currentProfile,lang,agencyName,theme,leads,recordActivity}) {
  ...
  async function createSelection(){
    ...
    try {
      ...
      const blob=buildPdfFromCanvasPages(pages),localUrl=URL.createObjectURL(blob);
      setSelectionDownloadUrl(localUrl);
      setSelectionUrl("");

      const path=`selections/${Date.now()}-vip-selection.pdf`;
      const uploaded=await uploadPresentationPdf(path,blob);

      if(uploaded?.publicUrl){
        setSelectionUrl(uploaded.publicUrl);
        alert("VIP PDF-подборка создана и загружена в Supabase. WhatsApp/Viber готовы к отправке.");
      } else {
        console.error("Presentation upload error:", uploaded?.error);
        alert(
          "PDF создан и его можно скачать, но Supabase не дал загрузить его в public bucket presentations.\n\n" +
          "Точная ошибка: " + storageErrorText(uploaded?.error) +
          "\n\nСделай скрин этого сообщения — по нему будет понятно, что именно блокирует загрузку."
        );
      }
    ...
    <div className="row">
      ...
      {selected.length>0 && <div className="card" style={{width:"100%",marginTop:10}}>
        ...
        <div className="grid3" style={{marginTop:10}}>
          ...
          <label className="chip" style={{alignSelf:"end",minHeight:48}}><input type="checkbox" checked={selectionComparison} onChange={e=>setSelectionComparison(e.target.checked)}/> Сравнение предложений + аналитика</label>
        </div>
        <Button className="full" onClick={createSelection}>{tr(lang,"Создать подборку для клиента")} ({selected.length})</Button>
      </div>}
    </div>
    {(selectionUrl || selectionDownloadUrl) && <div className="card selectionBar">
      <h3 style={{marginTop:0}}>Отправить подборку клиенту прямо из CRM</h3>
      <p className="muted">Выбери клиента выше или введи телефон/email вручную. CRM сформирует сообщение и откроет нужный канал с готовой презентацией.</p>
      {selectionUrl ? <div className="grid4">
        <Button className="full" onClick={()=>{if(openWhatsAppPresentation({phone:selectionClientPhone,url:selectionUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true})) logPresentationSend("whatsapp");}}>Отправить в WhatsApp</Button>
        <Button className="full" variant="purple" onClick={()=>{if(openViberPresentation({phone:selectionClientPhone,url:selectionUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true})) logPresentationSend("viber");}}>Отправить в Viber</Button>
        <Button className="full" variant="soft" onClick={()=>{openEmailPresentation({email:selectionClientEmail,url:selectionUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true});logPresentationSend("email");}}>Email</Button>
        <Button className="full" variant="soft" onClick={()=>{nativeSharePresentation({url:selectionUrl,downloadUrl:selectionDownloadUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true});logPresentationSend("share");}}>Поделиться</Button>
      </div> : <div className="card amber" style={{marginBottom:10}}>
        Подборка создана локально, но Supabase не вернул публичную ссылку. WhatsApp/Viber появятся после успешной загрузки PDF в bucket presentations.
      </div>}
      <div className="grid2" style={{marginTop:10}}>
        ...
        {selectionUrl ? <Button variant="soft" onClick={async()=>{await navigator.clipboard.writeText(selectionUrl);alert(tr(lang,"Ссылка скопирована"));}}>{tr(lang,"Скопировать ссылку подборки")}</Button> : <span className="btn soft full" style={{opacity:.55,cursor:"not-allowed"}}>Публичная ссылка недоступна</span>}
      </div>
    </div>}
...
function PropertyModal({property,setProperties,onClose,users,currentProfile,role,lang,agencyName,theme,leads,recordActivity}) {
  ...
  const [presentationDownloadUrl,setPresentationDownloadUrl] = useState("");
  ...
  async function generateClientPresentation(){
    try{
      ...
      const path=`${propertyId}/${Date.now()}-${safeTitle}-vip-presentation.pdf`;

      if(presentationDownloadUrl?.startsWith("blob:")) URL.revokeObjectURL(presentationDownloadUrl);
      const localUrl=URL.createObjectURL(blob);
      setPresentationDownloadUrl(localUrl);
      setPresentationUrl("");
      ...
      if(uploaded?.publicUrl){
        ...
        alert("VIP PDF-презентация создана и загружена в Supabase. WhatsApp/Viber готовы к отправке.");
      } else {
        console.error("Presentation upload error:", uploaded?.error);
        alert(
          "PDF создан и его можно открыть/скачать локально, но Supabase не дал загрузить его в public bucket presentations.\n\n" +
          "Точная ошибка: " + storageErrorText(uploaded?.error) +
          "\n\nСделай скрин этого сообщения — по нему будет понятно, что именно блокирует загрузку."
        );
      }
  ...
  return <Modal onClose={onClose} wide>
    ...
    <div className="card amber">
      ...
      {(presentationUrl || presentationDownloadUrl) && <div style={{marginTop:12}}>
        {presentationUrl && <div className="card" style={{padding:12,marginBottom:10}}>
          <b>Отправить клиенту прямо из CRM</b>
          <p className="muted" style={{margin:"6px 0 6px"}}>CRM подставит имя клиента, название агентства и готовую ссылку на VIP-презентацию.</p>
          <p className="muted" style={{margin:"0 0 10px"}}><b>WhatsApp:</b> откроется чат клиента с уже готовым сообщением. <b>Viber:</b> откроется чат клиента, а готовый текст со ссылкой автоматически скопируется — останется вставить и нажать «Отправить».</p>
          <div className="grid4">
            <Button className="full" onClick={()=>{if(openWhatsAppPresentation({phone:presentationClientPhone,url:presentationUrl,lang:presentationLang,agencyName,clientName:presentationClient,title:local.title,isSelection:false})) logPropertyPresentationSend("whatsapp");}}>Отправить в WhatsApp</Button>
            <Button className="full" variant="purple" onClick={()=>{if(openViberPresentation({phone:presentationClientPhone,url:presentationUrl,lang:presentationLang,agencyName,clientName:presentationClient,title:local.title,isSelection:false})) logPropertyPresentationSend("viber");}}>Отправить в Viber</Button>
            <Button className="full" variant="soft" onClick={()=>{openEmailPresentation({email:presentationClientEmail,url:presentationUrl,lang:presentationLang,agencyName,clientName:presentationClient,title:local.title,isSelection:false});logPropertyPresentationSend("email");}}>Email</Button>
            <Button className="full" variant="soft" onClick={()=>{nativeSharePresentation({url:presentationUrl,downloadUrl:presentationUrl,lang:presentationLang,agencyName,clientName:presentationClient,title:local.title,isSelection:false});logPropertyPresentationSend("share");}}>Поделиться</Button>
            <a className="btn green full" href={presentationUrl} target="_blank" rel="noopener noreferrer">Открыть PDF</a>
          </div>
        </div>}
        {!presentationUrl && <div className="card amber" style={{marginBottom:10}}>
          PDF создан локально, но публичная ссылка ещё не получена. Открыть/скачать PDF можно, а WhatsApp/Viber станут доступны после успешной загрузки в Supabase Storage.
        </div>}
        <div className="grid2">
          <a className="btn green full" href={presentationUrl || presentationDownloadUrl} target="_blank" rel="noopener noreferrer">Открыть PDF-презентацию</a>
          {presentationUrl
            ...
            : <a className="btn soft full" href={presentationDownloadUrl} download={`VIP-presentation-${new Date().toISOString().slice(0,10)}.pdf`}>Скачать PDF</a>}
        </div>
      </div>}
    </div>
