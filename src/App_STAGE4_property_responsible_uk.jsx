...
const uiPhraseTranslations = {
  uk: {
    ...
    "Презентация для клиента":"Презентація для клієнта", "Описание на украинском":"Опис українською", "Перевести описание на украинский":"Перекласти опис українською", "Язык презентации":"Мова презентації", "Русский язык":"Російська", "Украинский язык":"Українська", "Ответственный менеджер по объекту":"Відповідальний менеджер за об’єкт", "Связь с ответственным менеджером":"Зв’язок з відповідальним менеджером",
...
function translateRealEstateDescriptionToUk(text=""){
  const source = String(text || "");
  if (!source.trim()) return "";
  const dictionary = [
    [/квартира/gi,"квартира"],[/дом/gi,"будинок"],[/таунхаус/gi,"таунхаус"],[/участок/gi,"ділянка"],[/коммерция/gi,"комерція"],
    [/новый ремонт/gi,"новий ремонт"],[/ремонт/gi,"ремонт"],[/вид на море/gi,"вид на море"],[/море/gi,"море"],[/паркинг/gi,"паркінг"],
    [/охрана/gi,"охорона"],[/закрытая территория/gi,"закрита територія"],[/территория/gi,"територія"],[/инфраструктура/gi,"інфраструктура"],
    [/рассрочка/gi,"розстрочка"],[/переуступка/gi,"переуступка"],[/право собственности/gi,"право власності"],[/гос ?программы/gi,"держпрограми"],
    [/комнаты/gi,"кімнати"],[/комната/gi,"кімната"],[/спальни/gi,"спальні"],[/спальня/gi,"спальня"],[/кухня/gi,"кухня"],
    [/санузел/gi,"санвузол"],[/балкон/gi,"балкон"],[/лоджия/gi,"лоджія"],[/этаж/gi,"поверх"],[/площадь/gi,"площа"],
    [/светлая/gi,"світла"],[/светлый/gi,"світлий"],[/просторная/gi,"простора"],[/просторный/gi,"просторий"],[/уютная/gi,"затишна"],
    [/рядом/gi,"поруч"],[/центр/gi,"центр"],[/район/gi,"район"],[/школа/gi,"школа"],[/садик/gi,"садок"],
    [/магазины/gi,"магазини"],[/транспорт/gi,"транспорт"],[/лифт/gi,"ліфт"],[/генератор/gi,"генератор"],[/газ/gi,"газ"],
    [/отопление/gi,"опалення"],[/мебель/gi,"меблі"],[/техника/gi,"техніка"],[/документы/gi,"документи"],
    [/продается/gi,"продається"],[/сдается/gi,"здається"],[/расположен/gi,"розташований"],[/расположена/gi,"розташована"],
    [/есть/gi,"є"],[/без/gi,"без"],[/с/gi,"з"]
  ];
  let result = source;
  dictionary.forEach(([from,to]) => { result = result.replace(from,to); });
  return result;
}
...
function Top({page,setPage,agencyName,lang,search,setSearch}) {
  ...
  return <div className="top"><div className="topLine"><div><p>♛ CRM Real Estate</p><h1>{tr(lang,titles[page])}</h1></div><button className="agencyTop" onClick={()=>setPage("more")}>{agencyName || tr(lang,"Моё агентство")}</button></div><div className="search">🔎 <input value={search || ""} onChange={e=>setSearch && setSearch(e.target.value)} placeholder={tr(lang,"Поиск по CRM...")} /></div></div>;
}
...
function Feed({posts,setPosts,role,currentProfile,lang}) {
  ...
  const filteredPosts = posts.filter(p => inDateRange(p, dateMode, dateFrom, dateTo));
  ...
  const publish = async () => {
    ...
    setPosts(prev => [{
      ...
      createdAt: news.created_at || new Date().toISOString(),
      text: news.content || "",
      kind,
      file: mediaUrl,
      likes: 0,
  ...
  return <main className="screen"><div className="feedList">
    <div className="card dark hero">
      <div><h2>{tr(lang,"Лента новостей")}</h2><p>{tr(lang,"Внутренняя соцсеть агентства: фото, видео, файлы, ссылки, лайки и комментарии.")}</p></div>
      {isTech && <Button onClick={()=>setOpen(true)}>+ {tr(lang,"Новая публикация")}</Button>}
    </div>

    <div className="card">
      <div className="row"><h3>{tr(lang,"Фильтр по датам")}</h3><span className="muted">{tr(lang,"Выбери период просмотра ленты")}</span></div>
      <div className="grid4">
        {[ ["all",tr(lang,"Все")], ["day",tr(lang,"День")], ["week",tr(lang,"Неделя")], ["month",tr(lang,"Месяц")] ].map(([id,label])=><Button key={id} variant={dateMode===id?"primary":"soft"} onClick={()=>setDateMode(id)}>{label}</Button>)}
        <Button variant={dateMode==="custom"?"primary":"soft"} onClick={()=>setDateMode("custom")}>{tr(lang,"Период")}</Button>
      </div>
      {dateMode === "custom" && <div className="grid2"><input className="input" type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)}/><input className="input" type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)}/></div>}
    </div>
...
function Clients({leads,setLeads,onOpen,currentProfile,role,users,search}) {
  ...
  const filteredLeads = leads.filter(l => {
    ...
    const haystack = [l.id,l.name,l.phone,l.notes,l.source,l.status,l.manager,l.manager_email].map(x=>String(x||"").toLowerCase()).join(" ");
...
function LeadModal({lead,setLeads,setEvents,setHelp,help,onClose,role,users,currentProfile}) {
  ...
  const ask = async () => {
    ...
    try {
      await supabase.from("leads").update({director_question:q}).eq("id", Number(local.id));
    } catch {}
    setLocal(prev => ({...prev, director_question:q, history:[...(prev.history || []), `Вопрос директору: ${q}`]}));
    setLeads(prev => prev.map(l => l.id === local.id ? {...l, director_question:q, history:[...(l.history || []), `Вопрос директору: ${q}`]} : l));
    playBeep();
    setQuestion("");
  };
...
function Calendar({events,setEvents,leads,setLeads,onOpenLead,currentProfile,role,lang}) {
  ...
  const monthNames = monthTranslations[lang] || months;
  const weekNames = weekTranslations[lang] || week;
...
Дата следующего касания: ${ev.date} ${ev.time}`);
  ...
  return <main className="screen" style={{maxWidth:"100%"}}>
    ...
    <div className="card">
      <div className="row">
        <div className="grid2" style={{maxWidth:520}}>
          ...
          <select className="input" value={month} onChange={e=>setMonth(Number(e.target.value))}>{monthNames.map((m,i)=><option key={m} value={i}>{m}</option>)}</select>
        </div>
    ...
    {view==="year" && <div className="card">
      <h3>{year} год</h3>
      <div className="yearGrid">{monthNames.map((m,i)=><button className="monthBox" key={m} onClick={()=>{setMonth(i);setView("month")}}><b>{m}</b><span>{events.filter(e=>String(e.date || "").startsWith(`${year}-${String(i+1).padStart(2,"0")}`)).length} {tr(lang,"событий")}</span></button>)}</div>
    </div>}

    {view==="month" && <div className="card">
      <div className="row"><h3>{monthNames[month]} {year}</h3>{badge(events.filter(e=>String(e.date || "").startsWith(monthKey)).length+" "+tr(lang,"событий"),"gold")}</div>
      <div className="week">{weekNames.map(w=><b key={w}>{w}</b>)}</div>
...
function PropertyModal({property,setProperties,onClose,users,currentProfile,role,lang}) {
  ...
  const isNewProperty = String(local.id || "").startsWith("P-") || !Number(local.id);
  const responsibleEmail = String(local.property_manager_email || currentProfile?.email || "");
  const isResponsiblePropertyManager = String(currentProfile?.email || "") === responsibleEmail;
  const canEditLegal = isTech || isResponsiblePropertyManager;
  ...
  async function saveObject(closeAfter = false) {
    let preparedLocal = local;
    const isNew = String(local.id || "").startsWith("P-") || !Number(local.id);
    if (isNew || !preparedLocal.property_manager_email) {
      preparedLocal = {
        ...preparedLocal,
        property_manager_name: currentProfile?.full_name || currentProfile?.name || preparedLocal.property_manager_name || "",
        property_manager_email: currentProfile?.email || preparedLocal.property_manager_email || "",
        property_manager_phone: currentProfile?.phone || preparedLocal.property_manager_phone || ""
      };
      setLocal(preparedLocal);
    }
    const oldLocal = local;
    const localBackup = local;
    Object.assign(local, preparedLocal);
    ...
    const writeProperty = async (safePayload) => {
      if (isNew) {
        return await supabase.from("properties").insert(safePayload).select().single();
      }
      const res = await supabase.from("properties").update(safePayload).eq("id", savedId).select().single();
      return res;
    };

    let { data, error } = await writeProperty(payload);
    if (error && /column|schema|could not find|cache/i.test(error.message || "")) {
      const retry = await writeProperty(basePayload);
      data = retry.data;
      error = retry.error;
    }

    if (error) {
      alert((isNew ? "Ошибка создания объекта: " : "Ошибка сохранения объекта: ") + error.message);
      return null;
    }

    if (isNew && data?.id) savedId = data.id;
  ...
  async function addMediaBatch() {
    ...
    // ВАЖНО: объект сохраняем один раз, а все выбранные фото/видео крепим к одному property_id.
    // Раньше saveObject запускался внутри цикла, поэтому при создании нового объекта
    // каждое фото могло создавать отдельную карточку объекта.
    const propertyId = await saveObject(false);
    if (!propertyId) return;
    ...
    for (const f of files) {
      const detectedKind = f.type?.startsWith("video") ? "Видео" : (f.type?.startsWith("image") ? "Фото" : mediaKind);
      ...
      const upload = await uploadToFirstAvailableBucket(
        ["property-media","property_media","properties-media"],
        path,
        f,
        {upsert:true, contentType:f.type || undefined}
      );
      if (upload.error) { alert("Ошибка загрузки файла: " + upload.error.message); return; }

      const publicUrl = upload.publicUrl;
      const { data, error } = await supabase
        .from("property_media")
        .insert({property_id: Number(propertyId), media_type: detectedKind, media_url: publicUrl, file_name: originalName})
        .select()
        .single();

      if (error) { alert("Файл загрузился, но не записался в таблицу: " + error.message); return; }
      uploadedItems.push({kind:data.media_type || detectedKind, url:data.media_url || publicUrl, name:data.file_name || originalName});
    }
    ...
    setProperties(prev => {
      const exists = prev.some(p => String(p.id) === String(propertyId));
      if (!exists) return [updated, ...prev];
      return prev.map(p => String(p.id) === String(propertyId) ? {...p, ...updated} : p);
    });

    setMediaFile(null);
    setMediaFiles([]);
    alert(`Фото/видео сохранены: ${uploadedItems.length}`);
  }
  ...
  async function generateClientPresentation(){
    ...
    const pdfLabels = isUkPresentation ? {info:"Інформація про об’єкт", type:"Тип", district:"Район", status:"Статус", price:"Ціна", area:"Площа", floor:"Поверх", description:"Опис", disclaimer:"Документ сформовано без персональних даних: власник, телефон власника та відповідальний менеджер не вказані."} : {info:"Информация об объекте", type:"Тип", district:"Район", status:"Статус", price:"Цена", area:"Площадь", floor:"Этаж", description:"Описание", disclaimer:"Документ сформирован без персональных данных: собственник, телефон собственника и ответственный менеджер не указаны."};
    const presentationDescription = isUkPresentation ? (local.description_uk || translateRealEstateDescriptionToUk(local.description || "")) : (local.description || "");
    ...
    ctx.fillText(pdfLabels.info, 90, y);
    y += 60;

    const rows = [
      [pdfLabels.type, local.type || "—"],
      [pdfLabels.district, local.district || "—"],
      [pdfLabels.status, local.status || "—"],
      [pdfLabels.price, money(local.price)],
      [pdfLabels.area, `${local.area || 0} м²`],
      [pdfLabels.floor, local.floor || "—"]
    ];
    ...
    ctx.fillText(pdfLabels.description, 90, y);
    y += 45;
    ctx.font = "26px Arial";
    ctx.fillStyle = "#374151";
    y = drawWrapped(ctx, presentationDescription || (isUkPresentation ? "Опис об’єкта не додано." : "Описание объекта не добавлено."), 90, y, 1060, 38);

    y += 30;
    ctx.fillStyle = "#b45309";
    ctx.font = "bold 24px Arial";
    drawWrapped(ctx, pdfLabels.disclaimer, 90, y, 1060, 34);
  ...
  return <Modal onClose={onClose} wide>
    ...
    <div className="grid2">
      ...
      <Field label={tr(lang,"Ответственный менеджер по объекту")}><input className="input" value={local.property_manager_name || currentProfile?.full_name || currentProfile?.name || "не назначен"} disabled /></Field>
    </div>
    ...
    <Field label={tr(lang,"Описание")}><textarea className="input" value={local.description || ""} onChange={e=>setLocal({...local,description:e.target.value})}/></Field>

    <div className="card">
      <div className="row"><h3>{tr(lang,"Описание на украинском")}</h3><Button variant="soft" onClick={()=>setLocal({...local,description_uk:translateRealEstateDescriptionToUk(local.description || "")})}>{tr(lang,"Перевести описание на украинский")}</Button></div>
      <textarea className="input" value={local.description_uk || ""} onChange={e=>setLocal({...local,description_uk:e.target.value})} placeholder="Опис українською для презентації клієнту"/>
    </div>

    <div className="card"><h3>{tr(lang,"Юридические параметры")}</h3>{!canEditLegal && <p className="muted">Редактировать может только ответственный менеджер объекта или тех отдел.</p>}<div className="grid3"><label className="chip"><input type="checkbox" disabled={!canEditLegal} checked={!!local.ownership_right} onChange={e=>setLocal({...local,ownership_right:e.target.checked})}/> {tr(lang,"Право собственности")}</label><label className="chip"><input type="checkbox" disabled={!canEditLegal} checked={!!local.assignment} onChange={e=>setLocal({...local,assignment:e.target.checked})}/> {tr(lang,"Переуступка")}</label><label className="chip"><input type="checkbox" disabled={!canEditLegal} checked={!!local.government_programs} onChange={e=>setLocal({...local,government_programs:e.target.checked})}/> {tr(lang,"Госпрограммы")}</label></div></div>
    ...
    <div className="card amber">
      ...
      <Field label={tr(lang,"Язык презентации")}><select className="input" value={presentationLang} onChange={e=>setPresentationLang(e.target.value)}><option value="ru">{tr(lang,"Русский язык")}</option><option value="uk">{tr(lang,"Украинский язык")}</option></select></Field>
...
function Help({help,setHelp,leads,setLeads,onOpen,currentProfile,role}) {
  ...
  const answer = async id => {
    ...
    if (item && !String(id).startsWith("lead-") && !Number.isNaN(Number(id))) {
      const { error } = await supabase
        .from("manager_help_requests")
        .update({reply:text,status:"answered",updated_at:nowIso})
        .eq("id", Number(id));
    ...
    // Дублируем ответ в карточку клиента, чтобы менеджер точно видел ответ директора именно там.
    if (item?.leadId || item?.lead_id) {
      try {
        await supabase
          .from("leads")
          .update({director_comment:text})
          .eq("id", Number(item.leadId || item.lead_id));
      } catch {}
    }

    setHelp(prev=>prev.map(h=>String(h.id)===String(id)?{...h,reply:text,status:"answered",updated_at:nowIso}:h));

    if (item) {
      const historyText = `Ответ директора ${new Date().toLocaleString()}: ${text}`;
      setLeads(prev => prev.map(l =>
        String(l.id) === String(item.leadId || item.lead_id)
          ? {...l, director_comment:text, history:[...(l.history || []), historyText]}
          : l
      ));
    }
...
export default function App(){
  ...
  const notify = (text) => {
    const item = {id:Date.now()+Math.random(), text, time:new Date().toLocaleTimeString()};
    setNotifications(prev => [item, ...prev].slice(0,5));
    playReminderSound("soft");
    setTimeout(()=>setNotifications(prev => prev.filter(n => n.id !== item.id)), 6000);
  };
  ...
  function leadFromDb(row){
    return {
      ...
      director_question: row.director_question || "",
  ...
  async function loadFromSupabase(profile = currentProfile){
    ...
    // Помощь менеджерам: директор и техотдел должны видеть все вопросы из manager_help_requests.
    // Дополнительно подтягиваем старые вопросы из leads.director_question, если они были созданы до новой таблицы.
    const helpFromTable = helpRes.error ? [] : (helpRes.data || []).map(h => ({
      ...h,
      id:String(h.id),
      leadId:h.lead_id || h.leadId,
      text:h.question || h.text || "",
      question:h.question || h.text || "",
      reply:h.reply || h.director_comment || "",
      status:h.status || (h.reply ? "answered" : "open")
    }));

    const helpFromLeads = (mapped || [])
      .filter(l => l.director_question || l.director_comment)
      .map(l => ({
        id:`lead-${l.id}`,
        leadId:l.id,
        lead_id:l.id,
        ...
        text:l.director_question || "",
        question:l.director_question || "",
        reply:l.director_comment || "",
        status:l.director_comment ? "answered" : "open",
        created_at:null
      }));

    const existingHelpKeys = new Set(helpFromTable.map(h => String(h.leadId || h.lead_id) + "|" + String(h.question || h.text || "")));
    const mergedHelp = [...helpFromTable, ...helpFromLeads.filter(h => !existingHelpKeys.has(String(h.leadId || h.lead_id) + "|" + String(h.question || h.text || "")))];
    setHelp(mergedHelp);
  }
  ...
  useEffect(() => {
    ...
    const refresh = (label) => { notify(label); loadFromSupabase(currentProfile); };
    const channel = supabase.channel("crm-realtime-refresh")
      .on("postgres_changes", {event:"*", schema:"public", table:"leads"}, () => refresh("Обновились клиенты"))
      .on("postgres_changes", {event:"*", schema:"public", table:"news"}, () => refresh("Обновилась лента новостей"))
      .on("postgres_changes", {event:"*", schema:"public", table:"news_media"}, () => refresh("Обновились медиа в ленте"))
      .on("postgres_changes", {event:"*", schema:"public", table:"comments"}, () => refresh("Новый комментарий"))
      .on("postgres_changes", {event:"*", schema:"public", table:"properties"}, () => refresh("Обновилась вторичка"))
      .on("postgres_changes", {event:"*", schema:"public", table:"property_media"}, () => refresh("Обновились фото объекта"))
      .on("postgres_changes", {event:"*", schema:"public", table:"manager_help_requests"}, () => refresh("Новое сообщение директору"))
      .subscribe();
  ...
  useEffect(() => {
    if (!currentProfile) return;
    const run = () => translateInterfaceText(lang);
    run();
  ...
  }, [lang, page, currentProfile?.id, leads.length, posts.length, properties.length, events.length, help.length, theme]);
  ...
  return <div className="layout" data-theme={theme} style={themeStyle(theme)}><style>{globalThemeCss(theme)}</style>
    ...
    <div className="mainWrap">
      <main className="main">
        <Top page={page} setPage={setPage} agencyName={agencyName} lang={lang} search={search} setSearch={setSearch}/>
        {page==="feed"&&<Feed posts={posts} setPosts={setPosts} role={role} currentProfile={currentProfile} lang={lang}/>} 
        {page==="clients"&&<Clients leads={visibleLeads} setLeads={setLeads} onOpen={setLead} currentProfile={currentProfile} role={role} users={users} search={search}/>} 
        ...
        {notifications.length > 0 && <div style={{position:"fixed",right:18,top:86,zIndex:9999,display:"grid",gap:8,maxWidth:320}}>
          {notifications.map(n=><div key={n.id} className="card" style={{borderLeft:"5px solid var(--accent)",padding:"12px 14px"}}><b>{tr(lang,"Уведомление")}</b><p className="muted" style={{margin:0}}>{tr(lang,n.text)} · {n.time}</p></div>)}
        </div>}
