...
Дата следующего касания: ${ev.date} ${ev.time}`);
  ...
  return <main className="screen" style={{maxWidth:"100%"}}>
    ...
    {view==="year" && <div className="card">
      <h3>{year} год</h3>
      <div className="yearGrid">{months.map((m,i)=><button className="monthBox" key={m} onClick={()=>{setMonth(i);setView("month")}}><b>{tr(lang,m)}</b><span>{events.filter(e=>String(e.date || "").startsWith(`${year}-${String(i+1).padStart(2,"0")}`)).length} событий</span></button>)}</div>
    </div>}

    {view==="month" && <div className="card">
      <div className="row"><h3>{tr(lang,months[month])} {year}</h3>{badge(events.filter(e=>String(e.date || "").startsWith(monthKey)).length+" событий","gold")}</div>
...
function Properties({properties,setProperties,onOpen,users,currentProfile,lang}) {
  ...
  async function createSelection(){
    ...
    const cards = items.map(p=>`<section style="page-break-inside:avoid;border:1px solid #ddd;border-radius:18px;padding:20px;margin:20px 0">
      <h2>${p.title || "Объект"}</h2><p><b>${p.type || ""}</b> · ${p.district || ""}</p>
      <p style="font-size:24px;font-weight:700">${money(p.price)}</p><p>${p.area || 0} м² · ${p.floor || ""}</p>
      <p>${p.description || ""}</p>
      ${(p.media||[]).filter(m=>String(m.kind||"").includes("Фото")).slice(0,3).map(m=>`<img src="${m.url}" style="width:100%;max-width:700px;border-radius:14px;margin:8px 0"/>`).join("")}
    </section>`).join("");
    const html=`<!doctype html><html><head><meta charset="utf-8"><title>Подборка объектов</title></head><body style="font-family:Arial;padding:30px;max-width:900px;margin:auto"><h1>Подборка объектов CRM Real Estate</h1>${cards}</body></html>`;
    const blob=new Blob([html],{type:"text/html"});
    const path=`selections/${Date.now()}-selection.html`;
    const uploaded=await uploadToFirstAvailableBucket(["presentations","property-presentations","property-media"],path,blob,{upsert:true,contentType:"text/html"});
  ...
  return <main className="screen">
    <div className="row">
      ...
      {selected.length>0 && <Button onClick={createSelection}>{tr(lang,"Создать подборку для клиента")} ({selected.length})</Button>}
    </div>
    {selectionUrl && <div className="card selectionBar"><div className="grid2">
...
function Deals({leads,properties,currentProfile,role,lang}) {
    ...
    setDeals(prev=>{
      ...
      dealLeads.forEach(l=>{
        if(!next.some(d=>String(d.leadId)===String(l.id))){
          next.unshift({id:"D-"+Date.now()+"-"+l.id,leadId:l.id,client:l.name||"",manager:l.manager||"",manager_email:l.manager_email||"",propertyId:"",amount:"",commission:"",date:new Date().toISOString().slice(0,10),comment:""});
        }
      });
      return next;
    });
  ...
  const visible=deals.filter(d=>isTech||isDirector||String(d.manager_email||"").toLowerCase()===myEmail);
  const update=(id,patch)=>setDeals(prev=>prev.map(d=>d.id===id?{...d,...patch}:d));

  return <main className="screen">
    <div className="card dark"><h2>{tr(lang,"Сделки")}</h2><p>{isTech||isDirector?"Все сделки агентства":"Только твои сделки"}</p></div>
    {visible.length===0&&<div className="card"><p className="muted">{tr(lang,"Нет сделок")}</p></div>}
    {visible.map(d=><div className="card dealRow" key={d.id}>
      <div className="row"><div><h3>{d.client||tr(lang,"Клиент")}</h3><p className="muted">{d.manager}</p></div>{badge(d.id,"gold")}</div>
      <div className="grid3">
      ...
      <Button className="full" onClick={()=>alert(tr(lang,"Сохранить сделку"))}>{tr(lang,"Сохранить сделку")}</Button>
    </div>)}
  </main>;
}
  ...
  return <div className="layout" data-theme={theme} style={themeStyle(theme)}><style>{globalThemeCss(theme)}</style>
    ...
    <div className="mainWrap">
      <main className="main">
        ...
        {page==="properties"&&<Properties properties={properties} setProperties={setProperties} onOpen={setProperty} users={users} currentProfile={currentProfile} lang={lang}/>} 
