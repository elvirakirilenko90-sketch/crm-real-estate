...
function Analytics({leads,properties,events,role,users,activities,currentProfile,onOpenProperty,agencyName,theme}) {
  ...
  const uniq=(key)=>["Все",...Array.from(new Set(combinedListings.map(x=>String(x?.[key]||"").trim()).filter(Boolean))).sort((a,b)=>a.localeCompare(b,"ru"))];
  const countries=uniq("country"),regions=uniq("region"),cities=uniq("city"),districtsMarket=uniq("district"),complexes=uniq("complex_name"),sourcesMarket=uniq("source_name");
  const filtered=combinedListings.filter(x=>{
    ...
    return inMarketPeriod(x)
      ...
      &&(sourceFilter==="Все"||String(x.source_name||"")===sourceFilter)
  ...
  const resetFilters=()=>{setPeriod("30");setDateFrom("");setDateTo("");setCountry("Все");setRegion("Все");setCity("Все");setDistrictFilter("Все");setComplexFilter("Все");setDealFilter("Все");setSourceFilter("Все");setRoomsFilter("Все");setPriceFrom("");setPriceTo("");setAreaFrom("");setAreaTo("");};
  ...
  return <main className="screen">
    <div className="card dark" style={{padding:26,borderRadius:24}}>
      <div className="row" style={{alignItems:"center",gap:18}}>
        <section><div style={{fontSize:13,fontWeight:900,letterSpacing:1.8,opacity:.7}}>MARKET INTELLIGENCE</div><h2 style={{fontSize:32,margin:"7px 0"}}>📊 Аналитика рынка</h2><p style={{margin:0}}>DOM.RIA • OLX • Telegram • сайты • внутренние объекты CRM</p></section>
    ...
    <div className="card" style={{padding:22,borderRadius:24}}>
      ...
      <div className="grid4">
        <div style={filterBox}><Field label="Страна"><select className="input" value={country} onChange={e=>setCountry(e.target.value)}>{countries.map(x=><option key={x}>{x}</option>)}</select></Field></div>
        ...
        <div style={filterBox}><Field label="Город"><select className="input" value={city} onChange={e=>setCity(e.target.value)}>{cities.map(x=><option key={x}>{x}</option>)}</select></Field></div>
        ...
        <div style={filterBox}><Field label="Источник"><select className="input" value={sourceFilter} onChange={e=>setSourceFilter(e.target.value)}>{sourcesMarket.map(x=><option key={x}>{x}</option>)}</select></Field></div>
    ...
    try{
      const {data,error}=await supabase.functions.invoke("market-sync",{
        body:{
          agency_id:Number(agencyId),
          sync_all:true,
          // Передаём активные фильтры в Edge Function. DIM.RIA использует их
          // для официального API-поиска, а не тянет случайную общую выдачу.
          market_filters:{
            country:filters.country||"Украина",
            city:filters.city||"",
            district:filters.district||"",
            deal_type:deal||"Продажа",
            rooms:filters.rooms||"",
            price_from:filters.priceFrom||"",
            price_to:filters.priceTo||"",
            area_from:filters.areaFrom||"",
            area_to:filters.areaTo||"",
            lang:lang||"ru"
          }
        }
      });
