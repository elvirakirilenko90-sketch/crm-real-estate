...
function Feed({posts,setPosts,role}) {
  ...
  const commentsKey = (id) => `news-comments-${id}`;
  const readComments = (id) => {
    try { return JSON.parse(localStorage.getItem(commentsKey(id)) || "[]"); } catch { return []; }
  };
  ...
  async function uploadToNewsMedia(newsId) {
    if (draft.kind === "Ссылка") {
      ...
      if (error) {
        alert("Ошибка сохранения ссылки: " + error.message);
        return null;
      }
    ...
    const upload = await supabase
      .storage
      .from("news-media")
      .upload(path, fileObj, { upsert: true, contentType: fileObj.type || undefined });
    ...
    const realKind = fileObj.type?.startsWith("video") ? "Видео" : draft.kind;
  ...
  const publish = async () => {
    ...
    if (editing) {
      ...
      let mediaUrl = draft.file || editing.file || "";
      let kind = draft.kind || editing.kind || "Фото";

      if (fileObj || draft.kind === "Ссылка") {
        await supabase.from("news_media").delete().eq("news_id", editing.id);
        const media = await uploadToNewsMedia(editing.id);
        mediaUrl = media?.link_url || media?.media_url || mediaUrl;
        kind = media?.media_type || kind;
      }

      setPosts(prev => prev.map(p => String(p.id) === String(editing.id) ? {
        ...p,
        text: data.content || "",
        kind,
        file: mediaUrl
  ...
  const deletePost = async (post) => {
    ...
    localStorage.removeItem(commentsKey(post.id));
  ...
  const addComment = async (post) => {
    ...
    if (!text) return;
    const newComment = `${new Date().toLocaleString()}: ${text}`;
  ...
  return <main className="screen"><div className="feedList">
    <div className="card dark hero"><div><h2>Лента новостей</h2><p>Внутренняя соцсеть агентства: фото, видео, файлы, ссылки, лайки и комментарии.</p></div>{canManage ? <Button onClick={openCreate}>+ Новая публикация</Button> : badge("Только просмотр", "dark")}</div>

    {open && canManage && <div className="card composer">
      <div className="row"><h2>{editing ? "Редактировать публикацию" : "Новая публикация"}</h2><button className="icon" onClick={()=>setOpen(false)}>×</button></div>
      ...
      <Media kind={draft.kind} name={draft.kind === "Ссылка" ? draft.file : fileObj?.name || draft.file || ""}/>
    ...
    {posts.map(post=><article className="card post" key={post.id}>
      ...
      <div className="actions"><button className="linkBtn" onClick={()=>likePost(post)}>♡ {post.likes}</button><span>💬 {(post.comments || []).length}</span><span>↗</span></div>
      {canManage && <div className="grid2"><Button variant="soft" onClick={()=>openEdit(post)}>Редактировать</Button><Button variant="danger" onClick={()=>deletePost(post)}>Удалить</Button></div>}
      <div className="comments">
        {(post.comments || []).map((c,i)=><p key={i}><b>Комментарий:</b> {c}</p>)}
        <div className="grid2"><input className="input" placeholder="Написать комментарий..." value={commentText[post.id] || ""} onChange={e=>setCommentText(prev=>({...prev,[post.id]:e.target.value}))}/><Button onClick={()=>addComment(post)}>Отправить</Button></div>
      </div>
    </article>)}
...
function Properties({properties,setProperties,onOpen}) {
  ...
  return <main className="screen">
    ...
    <div className="chips">{["Все", ...types].map(t =>
      <button key={t} className={cn("chip", type===t && "active")} onClick={()=>setType(t)}>{t}</button>
    )}</div>

    <div className="chips">{["Все", ...districts].map(d =>
...
function PropertyModal({property,setProperties,onClose}) {
  ...
  async function saveObject(closeAfter = false) {
    ...
    const saved = { ...local, id: String(savedId), media: local.media || [] };
    setLocal(saved);
    setProperties(prev => {
      const withoutOld = prev.filter(p => String(p.id) !== String(local.id) && String(p.id) !== String(savedId));
  ...
  async function addMedia() {
    ...
    const { error: uploadError } = await supabase
      .storage
      .from("property-media")
      .upload(path, mediaFile, { upsert: true, contentType: mediaFile.type || undefined });
    ...
    const { data, error } = await supabase
      .from("property_media")
      .insert({ property_id: propertyId, media_type: detectedKind, media_url: publicUrl, file_name: originalName })
      .select()
      .single();
    ...
    const updated = { ...local, id: String(propertyId), media: [...(local.media || []), item] };
    setLocal(updated);
    setProperties(prev => {
      const withoutOld = prev.filter(p => String(p.id) !== String(local.id) && String(p.id) !== String(propertyId));
  ...
  const nextMedia = () => setViewer(i => (i + 1) % mediaList.length);
  const prevMedia = () => setViewer(i => (i - 1 + mediaList.length) % mediaList.length);

  return <Modal onClose={onClose} wide>
    <div className="propHero"><button className="icon heroClose" onClick={onClose}>×</button><h2>{local.title || "Новый объект"}</h2><p>{local.type} · {local.district} · {money(local.price)}</p></div>
    ...
    <div className="card"><h3>Фото / видео объекта</h3>
      <div className="grid4"><Button variant={mediaKind==="Фото" ? "primary" : "soft"} onClick={()=>setMediaKind("Фото")}>Фото</Button><Button variant={mediaKind==="Видео" ? "primary" : "soft"} onClick={()=>setMediaKind("Видео")}>Видео</Button></div>
      ...
      <div className="grid2">{mediaList.map((m,i)=><div key={i} onClick={()=>setViewer(i)} style={{cursor:"pointer"}}><Media kind={m.kind} name={m.url} small /></div>)}</div>
    </div>

    {currentMedia && <Modal onClose={()=>setViewer(null)} wide>
      <div className="row"><button className="icon" onClick={prevMedia}>‹</button><h2>{currentMedia.kind} {viewer + 1}/{mediaList.length}</h2><button className="icon" onClick={nextMedia}>›</button><button className="icon" onClick={()=>setViewer(null)}>×</button></div>
      {currentMedia.kind === "Видео" ? <video src={currentMedia.url} controls autoPlay style={{width:"100%",borderRadius:20}} /> : <img src={currentMedia.url} alt="Фото" style={{width:"100%",borderRadius:20}} />}
    </Modal>}
  </Modal>;
}
...
function Help({help,setHelp,leads,setLeads,onOpen}) {
  ...
  const playSound = (freq=980) => { try{ const ctx=new AudioContext(); const osc=ctx.createOscillator(); const gain=ctx.createGain(); osc.frequency.value=freq; gain.gain.value=.05; osc.connect(gain); gain.connect(ctx.destination); osc.start(); setTimeout(()=>{osc.stop();ctx.close()},400);}catch{} };
  const answer = id => {
    if(!reply[id]) return;
    const item = help.find(h=>h.id===id);
    setHelp(prev=>prev.map(h=>h.id===id?{...h,reply:reply[id],status:"answered"}:h));
    if (item) {
      const entry = `${new Date().toLocaleString()} — Ответ директора: ${reply[id]}`;
      setLeads(prev => prev.map(l => l.id === item.leadId ? {...l, notes:`${l.notes || ""}\n${entry}`.trim(), history:[...(l.history||[]), entry]} : l));
    }
    playSound(1180);
    setReply({...reply,[id]:""});
  };
  return <main className="screen"><div className="card dark"><div className="row"><section><h2>Помощь менеджерам</h2><p>Вопросы из карточек клиентов</p></section><Button onClick={()=>playSound()}>Звук</Button></div></div>{help.map(h=>{const lead=leads.find(l=>l.id===h.leadId);return <div className="card" key={h.id}><div className="row"><section><h3>{h.client}</h3><p className="muted">{h.manager}</p></section>{badge(h.status==="open"?"Ожидает":"Отвечено",h.status==="open"?"red":"green")}</div><div className="question"><b>Вопрос менеджера</b><p>{h.text}</p></div>{h.reply&&<div className="reply"><b>Директор:</b> {h.reply}</div>}<textarea className="input" placeholder="Ответить менеджеру..." value={reply[h.id]||""} onChange={e=>setReply({...reply,[h.id]:e.target.value})}/><div className="grid2"><Button onClick={()=>answer(h.id)}>Ответить</Button><Button variant="soft" onClick={()=>lead&&onOpen(lead)}>Открыть клиента</Button></div></div>})}</main>;
}
...
function More({role,setPage,setRole}) {
  const isTech = role === "Администратор тех отдел";
  return <main className="screen">
    <div className="card"><h3>Язык интерфейса</h3><div className="grid3"><Button>Русский</Button><Button variant="soft">Українська</Button><Button variant="soft">English</Button></div></div>
    {isTech && <div className="card"><h3>Режим доступа</h3><select className="input" value={role} onChange={e=>setRole(e.target.value)}><option>Менеджер по продажам</option><option>Администратор директор</option><option>Администратор тех отдел</option></select><p className="muted">Переключение ролей доступно только тех отделу.</p></div>}
    <div className="card row"><section><h3>Аналитика</h3><p className="muted">{role==="Менеджер по продажам"?"Только моя аналитика":"Доступ по роли"}</p></section><Button onClick={()=>setPage("analytics")}>Открыть</Button></div>
    ...
    {isTech&&<div className="card row"><section><h3>Доступ агентствам недвижимости</h3><p className="muted">Менеджеры, роли, доступы и агентства</p></section><Button onClick={()=>setPage("access")}>Открыть</Button></div>}
    {role==="Менеджер по продажам"&&<div className="card amber">Менеджеру доступны только язык и личная аналитика.</div>}
  </main>;
}
...
export default function App(){
  ...
  function postFromDb(row, mediaRows = []){
    ...
    let comments = [];
    try { comments = JSON.parse(localStorage.getItem(`news-comments-${row.id}`) || "[]"); } catch {}
...
async function loadFromSupabase(){
 ...
 async function syncPropertyToSupabase(item){
  ...
  return <div className="layout"><Sidebar page={page} setPage={setPage} role={role}/><div className="mainWrap"><main className="main"><Top page={page} setPage={setPage}/>{page==="feed"&&<Feed posts={posts} setPosts={setPosts} role={role}/>} {page==="clients"&&<Clients leads={leads} setLeads={setLeads} onOpen={setLead}/>} {page==="calendar"&&<Calendar events={events} setEvents={setEvents} leads={leads} setLeads={setLeads}/>} {page==="properties"&&<Properties properties={properties} setProperties={setProperties} onOpen={setProperty}/>} {page==="analytics"&&<Analytics leads={leads} properties={properties} events={events} role={role}/>} {page==="help"&&<Help help={help} setHelp={setHelp} setLeads={setLeads} leads={leads} onOpen={setLead}/>} {page==="access"&&<Access managers={managers} setManagers={setManagers}/>} {page==="more"&&<More role={role} setPage={setPage} setRole={setRole}/>}<Bottom page={page} setPage={setPage}/>{lead&&<LeadModal lead={lead} setLeads={setLeads} setEvents={setEvents} setHelp={setHelp} onClose={()=>setLead(null)}/>} {property&&<PropertyModal property={property} setProperties={setProperties} onClose={()=>setProperty(null)}/>}</main></div></div>
}
