...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  // market-sync may return parsed DIM.RIA cards in the response even when an older
  // database insert path inside the Edge Function skipped them. Analytics should not
  // throw those cards away: extract them, show them immediately, and safely persist
  // only the fields that market_listings already uses elsewhere in this CRM.
  const extractSyncListings=(payload)=>{
    const out=[];const seen=new Set();
    const visit=(value,sourceHint="",depth=0)=>{
      if(value==null||depth>7)return;
      if(Array.isArray(value)){value.forEach(x=>visit(x,sourceHint,depth+1));return;}
      if(typeof value!=="object")return;
      if(seen.has(value))return;seen.add(value);

      const hint=String(value.source_name||value.source||value.portal||value.name||sourceHint||"").trim();
      const normalized=normalizeMarketRow({...value,source_name:value.source_name||hint,raw_data:value.raw_data||value});
      const looksLikeCard=!!normalized.source_url && (
        !!String(normalized.title||"").trim() || Number(normalized.price||0)>0 ||
        Number(normalized.area||0)>0 || Number(normalized.rooms||0)>0
      );
      if(looksLikeCard&&!isSourcePlaceholder(normalized)){
        const key=normalized.source_url||`${normalized.source_name}|${normalized.external_id||normalized.title}`;
        if(key&&!out.some(x=>(x.source_url||`${x.source_name}|${x.external_id||x.title}`)===key)) out.push(normalized);
      }

      Object.entries(value).forEach(([k,v])=>{
        if(v&&typeof v==="object"&&/(listing|listings|item|items|candidate|candidates|property|properties|ad|ads|data|result|results|offers)/i.test(k)){
          visit(v,hint,depth+1);
        }
      });
    };
    visit(payload);
  ...
  const syncListingDbPayload=(row)=>{
    const normalized=normalizeMarketRow(row);
    const raw=normalized.raw_data&&typeof normalized.raw_data==="object"?normalized.raw_data:row;
    const derivedId=String(normalized.external_id||firstText(raw,["external_id","id","realty_id","object_id","offer_id"])||"").trim();
    return {
      agency_id:agencyId,
      external_id:derivedId||null,
      source_name:normalized.source_name||"DIM.RIA",
      source_url:normalized.source_url,
      deal_type:normalized.deal_type||firstText(raw,["deal_type","operation","operation_type","type"])||"Продажа",
      title:normalized.title||firstText(raw,["title","name","headline","address"])||"Объект DIM.RIA",
      country:normalized.country||firstText(raw,["country","country_name"])||"",
      city:normalized.city||firstText(raw,["city","locality","town"])||"",
      district:normalized.district||firstText(raw,["district","raion","neighborhood"])||"",
      rooms:Number(normalized.rooms||0)||null,
      price:Number(normalized.price||0)||0,
      area:Number(normalized.area||0)||0,
      currency:normalized.currency||detectCurrency(normalized.price,raw)||"USD",
      description:normalized.description||firstText(raw,["description","text","body"])||"",
      raw_data:{...raw,_crm_market_sync:true,source_name:normalized.source_name||"DIM.RIA",source_url:normalized.source_url}
    };
  };

  const persistReturnedListings=async(rows)=>{
    let saved=0;
    for(const sourceRow of rows){
      const row=syncListingDbPayload(sourceRow);
      if(!row.source_url)continue;
      const existing=await supabase.from("market_listings").select("id").eq("agency_id",agencyId).eq("source_url",row.source_url).limit(1);
      if(existing.error){console.warn("Analytics DIM.RIA duplicate check:",existing.error);continue;}
      if(existing.data?.[0]?.id){
        const upd=await supabase.from("market_listings").update(row).eq("id",existing.data[0].id).eq("agency_id",agencyId);
        if(!upd.error)saved++;else console.warn("Analytics DIM.RIA update:",upd.error);
      }else{
        const ins=await supabase.from("market_listings").insert(row);
        if(!ins.error)saved++;else console.warn("Analytics DIM.RIA insert:",ins.error);
      }
    }
    return saved;
  };
  ...
  const loadMarket=async()=>{
    ...
    const [listRes,sourceRes]=await Promise.all([
      supabase.from("market_listings").select("*").eq("agency_id",agencyId).order("last_seen_at",{ascending:false}).limit(3000),
      supabase.from("market_sources").select("*").eq("agency_id",agencyId).order("created_at",{ascending:false})
    ]);
    if(listRes.error){console.error(listRes.error);setMarketError(listRes.error.message||"Не удалось загрузить объявления рынка.");}
    else setMarketListings(listRes.data||[]);
  ...
  const syncMarket=async()=>{
    ...
    try{
      const cleanFilter=v=>String(v??"").trim()&&!/^Все$/i.test(String(v??"").trim())?v:undefined;
      const marketFilters={
        country:cleanFilter(country),region:cleanFilter(region),city:cleanFilter(city),
        district:cleanFilter(districtFilter),complex:cleanFilter(complexFilter),
        deal_type:cleanFilter(dealFilter),rooms:cleanFilter(roomsFilter),
        price_from:priceFrom!==""?Number(priceFrom):undefined,price_to:priceTo!==""?Number(priceTo):undefined,
        area_from:areaFrom!==""?Number(areaFrom):undefined,area_to:areaTo!==""?Number(areaTo):undefined,
        lang:lang||"ru"
      };
      Object.keys(marketFilters).forEach(k=>marketFilters[k]===undefined&&delete marketFilters[k]);

      const requestBody={agency_id:Number(agencyId),sync_all:true};
      if(Object.keys(marketFilters).length>1)requestBody.market_filters=marketFilters;

      const {data,error}=await supabase.functions.invoke("market-sync",{body:requestBody});
      if(error){
        let details="";
        try{
          const response=error?.context;
          if(response&&typeof response.clone==="function")details=await response.clone().text();
        }catch{}
        throw new Error([error?.message,details].filter(Boolean).join(" — ")||"market-sync завершился с ошибкой");
      }

      const returnedListings=extractSyncListings(data);
      let frontendSaved=0;
      if(returnedListings.length){
        // Show real cards immediately, even before the follow-up SELECT completes.
        setMarketListings(prev=>{
          const byUrl=new Map(prev.map(x=>[pickListingUrl(x)||String(x.id),x]));
          returnedListings.forEach((x,i)=>{
            const key=x.source_url||`sync-${Date.now()}-${i}`;
            byUrl.set(key,{...x,id:x.id||`sync-${Date.now()}-${i}`,agency_id:agencyId,last_seen_at:x.last_seen_at||new Date().toISOString()});
          });
          return Array.from(byUrl.values());
        });
        frontendSaved=await persistReturnedListings(returnedListings);
      }

      await new Promise(resolve=>setTimeout(resolve,450));
      const [afterListRes,afterSourceRes]=await Promise.all([
        supabase.from("market_listings").select("*").eq("agency_id",agencyId).order("last_seen_at",{ascending:false}).limit(3000),
        supabase.from("market_sources").select("*").eq("agency_id",agencyId).order("created_at",{ascending:false})
      ]);
      if(afterListRes.error)throw afterListRes.error;
      const afterRows=afterListRes.data||[];
      const mergedRows=[...afterRows];
      const knownUrls=new Set(afterRows.map(x=>pickListingUrl(x)).filter(Boolean));
      returnedListings.forEach((x,i)=>{
        const url=pickListingUrl(x);
        if(url&&!knownUrls.has(url)){
          mergedRows.push({...x,id:x.id||`sync-${Date.now()}-${i}`,agency_id:agencyId,last_seen_at:x.last_seen_at||new Date().toISOString()});
          knownUrls.add(url);
        }
      });
      setMarketListings(mergedRows);
      if(!afterSourceRes.error)setMarketSources(afterSourceRes.data||[]);

      const results=Array.isArray(data?.results)?data.results:[];
      const found=results.reduce((n,x)=>n+Number(x?.listings_found||x?.found||x?.candidates_found||0),Number(data?.listings_found||data?.found||0));
      const importedByFunction=results.reduce((n,x)=>n+Number(x?.listings_imported??x?.imported_count??x?.added_count??x?.inserted_count??x?.upserted_count??x?.last_imported_count??0),Number(data?.listings_imported||data?.imported_count||0));
      const concrete=(mergedRows||[]).map(x=>normalizeMarketRow({...x,origin:"market"})).filter(x=>!isSourcePlaceholder(x));
      const concreteCount=concrete.length;
      const dimCount=concrete.filter(x=>/dim\.?ria|dom\.ria/i.test([x.source_name,x.source_url,x.source_home_url].filter(Boolean).join(" "))).length;
      const recovered=returnedListings.length;
      const message=`Обновление завершено. Реально сохранённых объявлений: ${concreteCount}. DIM.RIA: ${dimCount}${found?`. Найдено источниками: ${found}`:""}${importedByFunction?`. Сохранено функцией: ${importedByFunction}`:""}${recovered?`. Получено карточек из ответа: ${recovered}`:""}${frontendSaved?`. CRM дополнительно сохранила: ${frontendSaved}`:""}.`;
      alert(message);
  ...
  const internalMarketListings=properties.map(p=>({
    ...
    country:p.country||p.country_name||"", region:p.region||p.oblast||p.region_name||"", city:p.city||p.locality||"", district:p.district||"", complex_name:p.complex_name||p.complex||"",
  ...
  const countries=["Все",...Array.from(new Set([...catalogCountries,...values(analyticsListings,"country")])).sort((a,b)=>a.localeCompare(b,"ru"))];
  ...
  const filtered=analyticsListings.filter(x=>{
  ...
  const resetFilters=()=>{setPeriod("30");setDateFrom("");setDateTo("");setCountry("Все");setRegion("Все");setCity("Все");setDistrictFilter("Все");setComplexFilter("Все");setDealFilter("Все");setSelectedSources([]);setSourceCountry("Все");setSearchQuery("");setRoomsFilter("Все");setPriceFrom("");setPriceTo("");setAreaFrom("");setAreaTo("");};
  ...
  return <main className="screen">
    ...
    <div className="card" style={{padding:22,borderRadius:24}}>
      ...
      <div className="grid4">
        <div style={filterBox}><Field label="Страна"><select className="input" value={country} onChange={e=>{const next=e.target.value;setCountry(next);setRegion("Все");setCity("Все");setDistrictFilter("Все");setComplexFilter("Все");setSelectedSources([]);setSourceCountry(next==="Все"?"Все":next);}}>{countries.map(x=><option key={x}>{x}</option>)}</select></Field></div>
        <div style={filterBox}><Field label="Область / регион"><select className="input" value={region} onChange={e=>{setRegion(e.target.value);setCity("Все");setDistrictFilter("Все");setComplexFilter("Все");}}>{regions.map(x=><option key={x}>{x}</option>)}</select></Field></div>
    ...
    {!loadingMarket&&filtered.length===0&&<div className="card"><h3>По выбранным фильтрам объявлений нет</h3><p className="muted">Фильтр сработал, но среди реально сохранённых в market_listings карточек совпадений нет. Если площадка подключена, а здесь пусто — проблема уже не в фильтре: коннектор должен сохранить полноценные объявления, а не только сообщить число найденных кандидатов.</p></div>}
    <div className="grid2">
      {filtered.map(x=>{
