...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  // Аналитика всегда читает весь рынок агентства. Общего лимита строк здесь нет.
  const fetchAllMarketListings=async()=>{
    // Быстрый путь: серверная RPC возвращает весь массив независимо от PostgREST row cap.
    const rpc=await supabase.rpc("analytics_market_feed_json",{p_agency_id:Number(agencyId)});
    if(!rpc.error){
      let rows=rpc.data;
      ...
      if(Array.isArray(rows))return [...new Map(rows.map(x=>[String(x.id),x])).values()];
    }

    // Резерв: cursor pagination до физического конца market_listings.
    const pageSize=1000;
    const rows=[];
    let lastId=null;
    while(true){
  ...
  const getListingDate=x=>x.published_at||x.first_seen_at||x.created_at||x.last_seen_at||x.updated_at||"";
  ...
  async function loadMarketData(){
    ...
    try{
      const allRowsPromise=(async()=>{
        const rpc=await supabase.rpc("analytics_market_feed_json",{p_agency_id:Number(agencyId)});
        if(!rpc.error){let rows=rpc.data;if(typeof rows==="string"){try{rows=JSON.parse(rows)}catch{rows=[]}}if(rows&&!Array.isArray(rows)&&Array.isArray(rows.items))rows=rows.items;if(Array.isArray(rows))return {data:rows,error:null};}
        const rows=[];let lastId=null;const pageSize=1000;
        while(true){let q=supabase.from("market_listings").select("*").eq("agency_id",agencyId).order("id",{ascending:true}).limit(pageSize);if(lastId!==null)q=q.gt("id",lastId);const {data,error}=await q;if(error)return {data:null,error};const batch=data||[];if(!batch.length)break;rows.push(...batch);const nextId=batch[batch.length-1]?.id;if(nextId===null||nextId===undefined||String(nextId)===String(lastId))break;lastId=nextId;if(batch.length<pageSize)break;}
