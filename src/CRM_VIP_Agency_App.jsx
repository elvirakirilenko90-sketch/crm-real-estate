...
function Properties({properties,setProperties,onOpen,users,currentProfile,lang,agencyName,theme}) {
  ...
  const create = () => {
    const p = {
      ...
      ownership_right:false, assignment:false, government_programs:false
    };
    onOpen(p);
  };
  ...
  async function createSelection(){
    ...
    try {
      ...
      const brand=loadAgencyBrand(currentProfile?.agency_id); const txt=vipPresentationText(selectionLang); const pages=[];
      const cover=document.createElement("canvas");cover.width=1240;cover.height=1754;let c=cover.getContext("2d");
      const v=currentThemeVars(theme), dark=v["--dark"]||"#0f172a", accent=v["--accent"]||"#d4af37", soft=v["--soft"]||"#f5f2ea";
      c.fillStyle=soft;c.fillRect(0,0,1240,1754);c.fillStyle=dark;c.fillRect(0,0,1240,1754);c.fillStyle=accent;c.fillRect(78,180,6,1180);
      if(brand.logo_url) await drawRemoteImage(c,brand.logo_url,90,80,180,100,"contain");
      c.fillStyle="#fff";c.font="bold 38px Arial";c.fillText(agencyName||"CRM Real Estate",90,250);c.font="bold 64px Arial";pdfDrawWrapped(c,txt.selection,90,430,980,72);
      c.fillStyle=accent;c.font="bold 28px Arial";c.fillText(`${txt.date}: ${formatPresentationDate(selectionLang)}`,90,690);
      if(selectionClient.trim()){c.fillStyle="#fff";c.font="28px Arial";c.fillText(txt.prepared,90,780);c.font="bold 42px Arial";pdfDrawWrapped(c,selectionClient.trim(),90,840,950,50);}
      c.fillStyle="#d1d5db";c.font="24px Arial";c.fillText(`${items.length} ${selectionLang==="en"?"properties":selectionLang==="uk"?"об'єктів":"объектов"}`,90,1050);
      pages.push({bytes:pdfDataUrlToBytes(cover.toDataURL("image/jpeg",.94)),width:1240,height:1754});
      for(let index=0;index<items.length;index++){
        const p=items[index], canvas=document.createElement("canvas");canvas.width=1240;canvas.height=1754;const ctx=canvas.getContext("2d");ctx.fillStyle="#fff";ctx.fillRect(0,0,1240,1754);
        drawVipHeader(ctx,{agencyName,brand,themeId:theme,title:p.title||`${txt.property} ${index+1}`,subtitle:`${index+1} / ${items.length} · ${formatPresentationDate(selectionLang)}`});
        if(brand.logo_url) await drawRemoteImage(ctx,brand.logo_url,950,35,190,90,"contain");
        const photo=(p.media||[]).find(m=>String(m.kind||"").includes("Фото")&&m.url);if(photo) await drawRemoteImage(ctx,photo.url,90,260,1060,520,"cover");
        let y=850;ctx.fillStyle="#111827";ctx.font="bold 34px Arial";ctx.fillText(txt.about,90,y);y+=55;
        const rows=[[txt.type,p.type||"—"],[txt.district,p.district||"—"],[txt.status,p.status||"—"],[txt.price,money(p.price)],[txt.area,`${p.area||0} м²`],[txt.floor,p.floor||"—"]];
        rows.forEach(([a,b],i)=>{const x=i%2?650:90, yy=y+Math.floor(i/2)*92;ctx.fillStyle="#6b7280";ctx.font="20px Arial";ctx.fillText(a,x,yy);ctx.fillStyle="#111827";ctx.font="bold 27px Arial";ctx.fillText(String(b),x,yy+34);});y+=310;
        ctx.fillStyle="#111827";ctx.font="bold 28px Arial";ctx.fillText(txt.description,90,y);y+=42;ctx.fillStyle="#374151";ctx.font="23px Arial";pdfDrawWrapped(ctx,(selectionLang==="uk"?(p.description_uk||p.description):p.description)||txt.noDescription,90,y,1060,34);
        pages.push({bytes:pdfDataUrlToBytes(canvas.toDataURL("image/jpeg",.94)),width:1240,height:1754});
      }
      const blob=buildPdfFromCanvasPages(pages),localUrl=URL.createObjectURL(blob);setSelectionDownloadUrl(localUrl);
      const path=`selections/${Date.now()}-vip-selection.pdf`;const uploaded=await uploadToFirstAvailableBucket(["presentations","property-presentations","property_media","property-media"],path,blob,{upsert:true,contentType:"application/pdf"});
  ...
  return <main className="screen">
    <div className="row">
      ...
      {selected.length>0 && <><input className="input" style={{maxWidth:240}} placeholder="Имя клиента (VIP)" value={selectionClient} onChange={e=>setSelectionClient(e.target.value)}/><select className="input" style={{maxWidth:150}} value={selectionLang} onChange={e=>setSelectionLang(e.target.value)}><option value="ru">RU</option><option value="uk">UA</option><option value="en">EN</option></select><Button onClick={createSelection}>{tr(lang,"Создать подборку для клиента")} ({selected.length})</Button></>}
    </div>
    {selectionUrl && <div className="card selectionBar"><div className="grid3">
      <a className="btn green full" href={selectionUrl} target="_blank" rel="noopener noreferrer">{tr(lang,"Открыть подборку")}</a>
...
function PropertyModal({property,setProperties,onClose,users,currentProfile,role,lang,agencyName,theme}) {
  ...
  async function generateClientPresentation(){
    try{
      const txt=vipPresentationText(presentationLang),brand=loadAgencyBrand(currentProfile?.agency_id),v=currentThemeVars(theme),dark=v["--dark"]||"#0f172a",accent=v["--accent"]||"#d4af37",soft=v["--soft"]||"#f5f2ea";
      if(presentationUrl?.startsWith("blob:")) URL.revokeObjectURL(presentationUrl);
      const pages=[];const cover=document.createElement("canvas");cover.width=1240;cover.height=1754;let ctx=cover.getContext("2d");ctx.fillStyle=dark;ctx.fillRect(0,0,1240,1754);ctx.fillStyle=accent;ctx.fillRect(78,175,6,1180);
      const hero=(local.media||[]).find(m=>String(m.kind||"").includes("Фото")&&m.url);if(hero){ctx.globalAlpha=.32;await drawRemoteImage(ctx,hero.url,360,0,880,1754,"cover");ctx.globalAlpha=1;}
      if(brand.logo_url) await drawRemoteImage(ctx,brand.logo_url,90,70,200,110,"contain");ctx.fillStyle="#fff";ctx.font="bold 38px Arial";ctx.fillText(agencyName||"CRM Real Estate",90,250);ctx.font="bold 60px Arial";pdfDrawWrapped(ctx,txt.personal,90,430,980,70);ctx.fillStyle=accent;ctx.font="bold 27px Arial";ctx.fillText(`${txt.date}: ${formatPresentationDate(presentationLang)}`,90,690);
      if(presentationClient.trim()){ctx.fillStyle="#fff";ctx.font="27px Arial";ctx.fillText(txt.prepared,90,790);ctx.font="bold 42px Arial";pdfDrawWrapped(ctx,presentationClient.trim(),90,850,950,50);}ctx.fillStyle="#d1d5db";ctx.font="24px Arial";pdfDrawWrapped(ctx,local.title||txt.property,90,1100,850,34);
      pages.push({bytes:dataUrlToBytes(cover.toDataURL("image/jpeg",.94)),width:1240,height:1754});
      const info=document.createElement("canvas");info.width=1240;info.height=1754;ctx=info.getContext("2d");ctx.fillStyle="#fff";ctx.fillRect(0,0,1240,1754);drawVipHeader(ctx,{agencyName,brand,themeId:theme,title:local.title||txt.property,subtitle:formatPresentationDate(presentationLang)});if(brand.logo_url) await drawRemoteImage(ctx,brand.logo_url,950,35,190,90,"contain");if(hero) await drawRemoteImage(ctx,hero.url,90,260,1060,520,"cover");let y=850;ctx.fillStyle="#111827";ctx.font="bold 34px Arial";ctx.fillText(txt.about,90,y);y+=55;const rows=[[txt.type,local.type||"—"],[txt.district,local.district||"—"],[txt.status,local.status||"—"],[txt.price,money(local.price)],[txt.area,`${local.area||0} м²`],[txt.floor,local.floor||"—"]];rows.forEach(([a,b],i)=>{const x=i%2?650:90,yy=y+Math.floor(i/2)*92;ctx.fillStyle="#6b7280";ctx.font="20px Arial";ctx.fillText(a,x,yy);ctx.fillStyle="#111827";ctx.font="bold 27px Arial";ctx.fillText(String(b),x,yy+34);});y+=310;ctx.fillStyle="#111827";ctx.font="bold 28px Arial";ctx.fillText(txt.description,90,y);y+=42;ctx.fillStyle="#374151";ctx.font="23px Arial";pdfDrawWrapped(ctx,(presentationLang==="uk"?(local.description_uk||local.description):local.description)||txt.noDescription,90,y,1060,34);pages.push({bytes:dataUrlToBytes(info.toDataURL("image/jpeg",.94)),width:1240,height:1754});
      const photos=(local.media||[]).filter(m=>String(m.kind||"").includes("Фото")&&m.url).slice(0,6);if(photos.length){const gallery=document.createElement("canvas");gallery.width=1240;gallery.height=1754;ctx=gallery.getContext("2d");ctx.fillStyle="#fff";ctx.fillRect(0,0,1240,1754);drawVipHeader(ctx,{agencyName,brand,themeId:theme,title:txt.photos,subtitle:local.title||""});for(let i=0;i<photos.length;i++){const col=i%2,row=Math.floor(i/2);await drawRemoteImage(ctx,photos[i].url,90+col*535,280+row*410,500,365,"cover");}pages.push({bytes:dataUrlToBytes(gallery.toDataURL("image/jpeg",.94)),width:1240,height:1754});}
      const endp=document.createElement("canvas");endp.width=1240;endp.height=1754;ctx=endp.getContext("2d");ctx.fillStyle=dark;ctx.fillRect(0,0,1240,1754);if(brand.logo_url) await drawRemoteImage(ctx,brand.logo_url,90,80,220,120,"contain");ctx.fillStyle=accent;ctx.font="bold 32px Arial";ctx.fillText(agencyName||"CRM Real Estate",90,280);ctx.fillStyle="#fff";ctx.font="bold 54px Arial";pdfDrawWrapped(ctx,txt.contact,90,500,1000,62);ctx.font="bold 34px Arial";ctx.fillStyle=accent;ctx.fillText(managerDisplayName(currentProfile),90,720);ctx.fillStyle="#fff";ctx.font="26px Arial";ctx.fillText(currentProfile?.phone||currentProfile?.email||"",90,775);ctx.font="30px Arial";ctx.fillText(txt.thanks,90,1050);pages.push({bytes:dataUrlToBytes(endp.toDataURL("image/jpeg",.94)),width:1240,height:1754});
      const blob=buildPdfFromJpegs(pages),propertyId=Number(local.id)||Date.now(),safeTitle=(local.title||"object").replace(/[^a-zA-Zа-яА-Я0-9_-]+/g,"_").slice(0,60),path=`${propertyId}/${Date.now()}-${safeTitle}-vip-presentation.pdf`;const uploaded=await uploadToFirstAvailableBucket(["presentations","property-presentations","property_media","property-media"],path,blob,{upsert:true,contentType:"application/pdf"});setPresentationUrl(uploaded?.publicUrl||URL.createObjectURL(blob));alert(uploaded?.publicUrl?"VIP PDF-презентация создана.":"VIP PDF создан локально.");
  ...
  return <Modal onClose={onClose} wide>
    ...
    <div className="card amber">
      ...
      <div className="grid2" style={{marginBottom:12}}>
        ...
        <Button variant={presentationLang === "en" ? "primary" : "soft"} onClick={()=>setPresentationLang("en")}>English presentation</Button>
      </div>
      <input className="input" style={{marginBottom:10}} placeholder="Имя клиента для VIP-презентации (необязательно)" value={presentationClient} onChange={e=>setPresentationClient(e.target.value)}/>
  ...
  return <div className="layout" data-theme={theme} style={themeStyle(theme)}><style>{globalThemeCss(theme)}</style>
    ...
    <div className="mainWrap">
      <main className="main">
        ...
        {page==="properties"&&<Properties properties={properties} setProperties={setProperties} onOpen={setProperty} users={users} currentProfile={currentProfile} lang={lang} agencyName={agencyName} theme={theme}/>} 
        ...
        {property&&<PropertyModal property={property} setProperties={setProperties} onClose={()=>setProperty(null)} users={users} currentProfile={currentProfile} role={role} lang={lang} agencyName={agencyName} theme={theme}/>} 
      </main>
    </div>
  </div>;
}
