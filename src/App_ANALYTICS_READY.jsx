...
function Analytics({leads,properties,events,role,users,activities,currentProfile}) {
...
function MarketTools({lang,currentProfile,properties=[],onOpenProperty,leads=[],agencyName="",theme="classic",recordActivity}){
  ...
  const [filters,setFilters]=useState({
    country:"",region:"",city:"",district:"",complex:"",propertyType:"",rooms:"",source:"",
    priceFrom:"",priceTo:"",areaFrom:"",areaTo:"",currency:"USD",dateFrom:"",dateTo:""
  });
  ...
  const [sf,setSf]=useState({name:"",country:"Украина",type:"WEB",url:"",enabled:true});
  const [cf,setCf]=useState({name:"",url:"",country:"Украина",city:"",enabled:true});
  const [listingForm,setListingForm]=useState({
    title:"",deal_type:"Продажа",country:"Украина",city:"",district:"",
    rooms:"",price:"",area:"",currency:"USD",source_name:"",source_url:"",description:""
  });

  const [favorites,setFavorites]=useStorage(`market_favorites_${agencyId}`,[]);
  const [selected,setSelected]=useState([]);
  const [selectionUrl,setSelectionUrl]=useState("");
  const [selectionDownloadUrl,setSelectionDownloadUrl]=useState("");
  const [selectionLang,setSelectionLang]=useState(lang||"ru");
  ...
  async function loadMarketData(){
    ...
    try{
      const [srcRes,listRes]=await Promise.all([
        ...
        supabase.from("market_listings").select("*").eq("agency_id",agencyId).order("updated_at",{ascending:false}).limit(5000)
      ]);
      const missingTable=[srcRes.error,listRes.error].some(e=>e&&/does not exist|relation .* does not exist|42P01/i.test(e.message||""));
      if(missingTable){setMarketDbReady(false);loadLocalMarketFallback();return;}
      if(srcRes.error) throw srcRes.error;
      if(listRes.error) throw listRes.error;
      setMarketDbReady(true);
      const allSources=srcRes.data||[];
      setSourcesX(allSources.filter(s=>String(s.source_type||s.connection_method||"").toUpperCase()!=="TELEGRAM"));
      setChannels(allSources.filter(s=>String(s.source_type||s.connection_method||"").toUpperCase()==="TELEGRAM"));
  ...
  async function loadMarketSyncHistory(){
    ...
    try{
      const {data,error}=await supabase.from("market_sync_log").select("*").eq("agency_id",agencyId).order("created_at",{ascending:false}).limit(30);
      if(error){if(!/does not exist|relation .* does not exist|42P01/i.test(error.message||"")) console.warn("Market sync log:",error);return;}
      const rows=data||[];setMarketSyncLogs(rows);
      const latest=rows.find(x=>x.finished_at||x.created_at);setLastMarketSyncAt(latest?.finished_at||latest?.created_at||null);
    }catch(e){console.warn("Market sync history:",e);}
  }

  useEffect(()=>{loadMarketData();loadMarketSyncHistory();},[agencyId]);
  useEffect(()=>{setVisibleCount(60);},[filters,deal,periodPreset,sortBy]);

  function marketListingPhotos(x){
    const raw=[];
    if(x.main_photo_url) raw.push(x.main_photo_url);
    if(Array.isArray(x.photo_urls)) raw.push(...x.photo_urls);
    if(Array.isArray(x.raw_data?.photos)) raw.push(...x.raw_data.photos);
    const seen=new Set();
    return raw.map(v=>typeof v==="string"?v:(v?.url||v?.src||"")).filter(v=>v&&!seen.has(v)&&seen.add(v));
  }

  function normalizeListing(x,origin="external"){
    const photos=origin==="crm"
      ? (x.media||[]).map(m=>m?.url||m?.media_url||"").filter(Boolean)
      : marketListingPhotos(x);
    return {
      id:String(x.id||`${origin}-${Date.now()}-${Math.random()}`),
      external_id:x.external_id||"",
      title:x.title||"Объект",
      deal_type:x.deal_type||x.operation_type||"Продажа",
      property_type:x.property_type||x.type||"",
      country:x.country||"",
      region:x.region||x.state||x.oblast||x.raw_data?.region||x.raw_data?.oblast||"",
      city:x.city||"",
      district:x.district||"",
      complex_name:x.complex_name||x.complex||x.raw_data?.complex_name||"",
      rooms:x.rooms??"",
      price:Number(x.price)||0,
      area:Number(x.area)||0,
      floor:x.floor??"",
      total_floors:x.total_floors??"",
      currency:x.currency||"USD",
      description:x.description||"",
      source_name:x.source_name||x.name||(origin==="crm"?"CRM":"Источник"),
      source_url:x.source_url||x.url||"",
      published_at:x.published_at||null,
      first_seen_at:x.first_seen_at||x.created_at||null,
      updated_at:x.updated_at||x.last_seen_at||x.created_at||null,
      status:x.status||"",
      is_active:x.is_active!==false,
      photos,
      main_photo_url:photos[0]||"",
      origin,
      crmProperty:origin==="crm"?(x.crmProperty||x):null,
      raw:x
    };
  }

  const internalListings=useMemo(()=>properties.map(p=>normalizeListing({
    ...p,id:`crm-${p.id}`,source_name:"CRM",source_url:"",crmProperty:p,
    property_type:p.type,deal_type:p.deal_type||p.operation_type||"Продажа"
  },"crm")),[properties]);

  const externalNormalized=useMemo(()=>externalListings.map(x=>normalizeListing(x,"external")),[externalListings]);
  const allListings=useMemo(()=>[...internalListings,...externalNormalized],[internalListings,externalNormalized]);

  function normText(v){return String(v||"").toLowerCase().replace(/[^a-zа-яёіїєґ0-9]+/gi," ").trim();}
  function crmMatchForListing(listing){
    if(listing.origin==="crm") return listing.crmProperty;
    const titleN=normText(listing.title);
    return properties.find(p=>{
      const pTitle=normText(p.title);
      const exactTitle=titleN&&pTitle&&titleN===pTitle;
      const samePrice=listing.price>0&&Number(p.price)>0&&Math.abs(Number(p.price)-listing.price)<=Math.max(500,listing.price*.01);
      const sameArea=listing.area>0&&Number(p.area)>0&&Math.abs(Number(p.area)-listing.area)<=1.5;
      const sameDistrict=!listing.district||!p.district||normText(listing.district)===normText(p.district);
      return exactTitle||(samePrice&&sameArea&&sameDistrict);
    })||null;
  }

  function listingDate(p){
    const raw=p.published_at||p.first_seen_at||p.updated_at;
  ...
  function periodBounds(){
    ...
    const days=Number(periodPreset)||1;
    const from=startOfDay(now);from.setDate(from.getDate()-(days-1));
    return {from,to:now};
  }

  const optionSets=useMemo(()=>{
    const uniq=key=>[...new Set(allListings.map(x=>String(x[key]||"").trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"ru"));
    return {countries:uniq("country"),regions:uniq("region"),cities:uniq("city"),sources:uniq("source_name"),types:uniq("property_type")};
  },[allListings]);

  const filteredListings=useMemo(()=>{
    const minP=Number(filters.priceFrom)||0,maxP=Number(filters.priceTo)||Infinity;
    const minA=Number(filters.areaFrom)||0,maxA=Number(filters.areaTo)||Infinity;
    const {from,to}=periodBounds();
    const rows=allListings.filter(p=>{
      const d=listingDate(p);
      const hay=normText(`${p.title} ${p.country} ${p.region} ${p.city} ${p.district} ${p.complex_name} ${p.description}`);
      const dateOk=(!from&&!to)||(!d?periodPreset==="all":(!from||d>=from)&&(!to||d<=to));
      return dateOk&&
        (!deal||p.deal_type===deal)&&
        (!filters.country||String(p.country||"")===filters.country)&&
        (!filters.region||String(p.region||"")===filters.region)&&
        (!filters.city||String(p.city||"")===filters.city)&&
        (!filters.district||hay.includes(normText(filters.district)))&&
        (!filters.complex||hay.includes(normText(filters.complex)))&&
        (!filters.propertyType||String(p.property_type||"")===filters.propertyType)&&
        (!filters.source||String(p.source_name||"")===filters.source)&&
        (!filters.rooms||String(p.rooms||"")===String(filters.rooms))&&
        p.price>=minP&&p.price<=maxP&&p.area>=minA&&p.area<=maxA;
    });
    return [...rows].sort((a,b)=>{
      if(sortBy==="priceAsc") return a.price-b.price;
      if(sortBy==="priceDesc") return b.price-a.price;
      if(sortBy==="sqmAsc") return (a.area?a.price/a.area:Infinity)-(b.area?b.price/b.area:Infinity);
      const ad=listingDate(a)?.getTime()||0,bd=listingDate(b)?.getTime()||0;return bd-ad;
    });
  },[allListings,filters,deal,periodPreset,sortBy]);

  const stats=useMemo(()=>{
    const marketRows=filteredListings.filter(x=>x.origin!=="crm");
    const prices=marketRows.map(x=>Number(x.price)).filter(x=>x>0).sort((a,b)=>a-b);
    const sqm=marketRows.map(x=>x.area>0?x.price/x.area:0).filter(x=>x>0).sort((a,b)=>a-b);
    const avg=a=>a.length?Math.round(a.reduce((s,x)=>s+x,0)/a.length):0;
    const median=a=>a.length?a[Math.floor((a.length-1)/2)]:0;
    const districtMap={},sourceMap={};
    marketRows.forEach(x=>{districtMap[x.district||"Без района"]=(districtMap[x.district||"Без района"]||0)+1;sourceMap[x.source_name||"Источник"]=(sourceMap[x.source_name||"Источник"]||0)+1;});
    ...
    const belowMarket=marketRows.filter(x=>averagePrice>0&&x.price>0&&x.price<=averagePrice*.9);
    const crmCount=filteredListings.filter(x=>x.origin==="crm").length;
    return {count:marketRows.length,total:filteredListings.length,crmCount,avg:averagePrice,median:median(prices),sqm:avg(sqm),topDistrict,topSource,belowMarket,sourceMap};
  },[filteredListings]);

  async function persistSource(row){
    if(!marketDbReady){
      const next=row.source_type==="TELEGRAM"?[row,...channels]:[row,...sourcesX];
      if(row.source_type==="TELEGRAM"){setChannels(next);saveLocalMarketFallback(sourcesX,next,externalListings);}else{setSourcesX(next);saveLocalMarketFallback(next,channels,externalListings);}return true;
    }
    const sourceType=String(row.source_type||"MANUAL").toUpperCase();
    const connectionMethod=sourceType==="TELEGRAM"?"TELEGRAM":["API","XML","RSS","WEB","HTML","CSV","MANUAL"].includes(sourceType)?sourceType:"MANUAL";
    const payload={agency_id:agencyId,name:row.name,country:row.country||"",city:row.city||"",source_type:sourceType,endpoint_url:row.endpoint_url||"",enabled:row.enabled!==false,config:row.config||{},connector_key:sourceType==="TELEGRAM"?"telegram":`generic_${connectionMethod.toLowerCase()}`,source_category:sourceType==="TELEGRAM"?"telegram":"real_estate_portal",connection_method:connectionMethod,sync_mode:sourceType==="CSV"||connectionMethod==="MANUAL"?"manual":"scheduled",auth_type:sourceType==="TELEGRAM"?"telegram_session":"none",connection_status:"not_checked",paused:false};
    ...
    if(data.source_type==="TELEGRAM") setChannels(x=>[data,...x]);else setSourcesX(x=>[data,...x]);return true;
  }

  async function addSource(){
    if(!sf.name.trim()||!sf.url.trim()) return alert("Укажи название и адрес источника.");
    const ok=await persistSource({id:`SRC-${Date.now()}`,name:sf.name.trim(),country:sf.country.trim(),source_type:sf.type,endpoint_url:sf.url.trim(),enabled:true});
    if(ok) setSf({...sf,name:"",url:""});
  }
  async function addChannel(){
    if(!cf.name.trim()||!cf.url.trim()) return alert("Укажи название и ссылку Telegram-канала.");
    const ok=await persistSource({id:`TG-${Date.now()}`,name:cf.name.trim(),country:cf.country.trim(),city:cf.city.trim(),source_type:"TELEGRAM",endpoint_url:cf.url.trim(),enabled:true});
    if(ok) setCf({...cf,name:"",url:""});
  }
  async function toggleSource(item){
    if(!marketDbReady){
      if(item.source_type==="TELEGRAM"){const next=channels.map(x=>x.id===item.id?{...x,enabled:!x.enabled}:x);setChannels(next);saveLocalMarketFallback(sourcesX,next,externalListings);}else{const next=sourcesX.map(x=>x.id===item.id?{...x,enabled:!x.enabled}:x);setSourcesX(next);saveLocalMarketFallback(next,channels,externalListings);}return;
    }
    const {error}=await supabase.from("market_sources").update({enabled:!item.enabled}).eq("id",item.id).eq("agency_id",agencyId);if(error)return alert(error.message);loadMarketData();
  }
  async function removeSource(item){
    if(!confirm(`Удалить источник «${item.name}»?`)) return;
    if(!marketDbReady){
      if(item.source_type==="TELEGRAM"){const next=channels.filter(x=>x.id!==item.id);setChannels(next);saveLocalMarketFallback(sourcesX,next,externalListings);}else{const next=sourcesX.filter(x=>x.id!==item.id);setSourcesX(next);saveLocalMarketFallback(next,channels,externalListings);}return;
    }
    const {error}=await supabase.from("market_sources").delete().eq("id",item.id).eq("agency_id",agencyId);if(error)return alert(error.message);loadMarketData();
  }
  ...
  async function addExternalListing(){
    ...
    const row={id:`LOCAL-${Date.now()}`,agency_id:agencyId,...listingForm,rooms:listingForm.rooms?Number(listingForm.rooms):null,price:Number(listingForm.price)||0,area:Number(listingForm.area)||0,updated_at:new Date().toISOString()};
    if(!marketDbReady){const next=[row,...externalListings];setExternalListings(next);saveLocalMarketFallback(sourcesX,channels,next);}else{
      const {error}=await supabase.from("market_listings").insert({agency_id:agencyId,external_id:null,source_name:row.source_name||"Ручной источник",source_url:row.source_url,deal_type:row.deal_type,title:row.title,country:row.country,city:row.city,district:row.district,rooms:row.rooms,price:row.price,area:row.area,currency:row.currency,description:row.description,raw_data:{manual:true}});
      if(error)return alert("Ошибка добавления объекта: "+error.message);await loadMarketData();
    }
    setListingForm({title:"",deal_type:deal,country:filters.country||"Украина",city:filters.city||"",district:"",rooms:"",price:"",area:"",currency:filters.currency||"USD",source_name:"",source_url:"",description:""});
  }

  async function importMarketCsv(file){
    if(!file)return;const raw=await file.text();const lines=raw.split(/\r?\n/).filter(Boolean);if(lines.length<2)return alert("Файл пустой.");
    const delimiter=(lines[0].split(";").length>lines[0].split(",").length)?";":",";const headers=lines[0].split(delimiter).map(x=>x.trim().toLowerCase());
    const get=(cols,names)=>{for(const n of names){const i=headers.indexOf(n);if(i>=0)return cols[i]?.trim()||"";}return "";};
    const parsed=lines.slice(1).map(line=>{const c=line.split(delimiter);return {agency_id:agencyId,external_id:get(c,["external_id","id"]),source_name:get(c,["source_name","source","источник"])||"CSV",source_url:get(c,["source_url","url","link","ссылка"]),deal_type:get(c,["deal_type","operation","тип сделки"])||deal,title:get(c,["title","name","название"])||"Объект",country:get(c,["country","страна"])||filters.country,city:get(c,["city","город"])||filters.city,district:get(c,["district","район"]),rooms:Number(get(c,["rooms","комнаты"]))||null,price:Number(String(get(c,["price","цена"])).replace(/[^\d.]/g,""))||0,area:Number(String(get(c,["area","площадь"])).replace(/[^\d.]/g,""))||0,currency:get(c,["currency","валюта"])||filters.currency,description:get(c,["description","описание"]),raw_data:{imported_from_csv:true}};}).filter(x=>x.source_url||x.title);
    if(!parsed.length)return alert("Не удалось распознать строки.");
    if(!marketDbReady){const next=[...parsed.map((x,i)=>({...x,id:`CSV-${Date.now()}-${i}`,updated_at:new Date().toISOString()})),...externalListings];setExternalListings(next);saveLocalMarketFallback(sourcesX,channels,next);alert(`Импортировано локально: ${parsed.length}`);return;}
    const {error}=await supabase.from("market_listings").insert(parsed);if(error)return alert("Ошибка импорта: "+error.message);alert(`Импортировано: ${parsed.length}`);await loadMarketData();
  }
  ...
  async function syncMarketSources(){
    ...
    setSyncingMarket(true);setMarketSyncResult(null);
    try{
      const {data,error}=await supabase.functions.invoke("market-sync",{body:{agency_id:Number(agencyId),sync_all:true}});
      if(error)throw error;if(!data)throw new Error("Функция не вернула результат.");setMarketSyncResult(data);await Promise.all([loadMarketData(),loadMarketSyncHistory()]);if(data.success===false)alert(data.error||data.message||"Обновление рынка завершилось с ошибкой.");
    }catch(e){const message=e?.message||String(e);setMarketSyncResult({success:false,error:message,processed:0,failed:1,results:[]});alert("Не удалось обновить рынок: "+message);}finally{setSyncingMarket(false);}
  }

  function syncStatusLabel(status){const labels={completed:"Готово",failed:"Ошибка",cancelled:"Отменено",connector_not_configured:"Коннектор ожидает подключения",manual_source:"Ручной источник",manual_import:"Ручной импорт",connected:"Подключен",warning:"Требует настройки",error:"Ошибка",never:"Ещё не запускался",not_checked:"Не проверен"};return labels[status]||status||"—";}
  function formatSyncDate(value){if(!value)return"Ещё не обновлялось";const date=new Date(value);return Number.isNaN(date.getTime())?"Ещё не обновлялось":date.toLocaleString("ru-RU");}
  function formatListingDate(p){const d=listingDate(p);return d?d.toLocaleString("ru-RU",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}):"Дата не указана";}
  function sourceTone(name){const s=normText(name);if(s.includes("telegram"))return"#229ED9";if(s.includes("ria"))return"#16a34a";if(s.includes("olx"))return"#6d28d9";if(s==="crm")return"var(--accent)";return"#475569";}
  function listingKey(p){return `${p.origin}:${p.id}`;}
  function toggleFavorite(p){const key=listingKey(p);setFavorites(prev=>prev.includes(key)?prev.filter(x=>x!==key):[...prev,key]);}
  function toggleSelected(p){const key=listingKey(p);setSelected(prev=>prev.includes(key)?prev.filter(x=>x!==key):[...prev,key]);}
  function resetFilters(){setFilters({country:"",region:"",city:"",district:"",complex:"",propertyType:"",rooms:"",source:"",priceFrom:"",priceTo:"",areaFrom:"",areaTo:"",currency:"USD",dateFrom:"",dateTo:""});setDeal("Продажа");setPeriodPreset("all");setSortBy("newest");}

  const selectedListings=useMemo(()=>allListings.filter(p=>selected.includes(listingKey(p))),[allListings,selected]);
  function presentationItem(p){
    const media=(p.photos||[]).map((url,i)=>({kind:"Фото",url,name:`photo-${i+1}.jpg`}));
    return {id:p.id,title:p.title,type:p.property_type||(`${p.rooms||""}-комнатные`),district:p.district||p.complex_name||p.city||"—",price:p.price,area:p.area,floor:p.total_floors?`${p.floor||"—"}/${p.total_floors}`:p.floor||"—",description:[p.description,p.source_name?`Источник: ${p.source_name}`:"",p.source_url?`Ссылка: ${p.source_url}`:""].filter(Boolean).join("\n\n"),media,currency:p.currency};
  }

  async function createMarketSelection(){
    const items=selectedListings.map(presentationItem);if(!items.length){alert("Выбери хотя бы один объект");return;}
    try{
      if(selectionUrl?.startsWith("blob:"))URL.revokeObjectURL(selectionUrl);if(selectionDownloadUrl?.startsWith("blob:"))URL.revokeObjectURL(selectionDownloadUrl);
      const brand=loadAgencyBrand(currentProfile?.agency_id),txt=vipPresentationText(selectionLang),pages=[],pal=presentationPalette(selectionVisual,theme);
      const cover=document.createElement("canvas");cover.width=1240;cover.height=1754;let c=cover.getContext("2d");c.fillStyle=pal.bg;c.fillRect(0,0,1240,1754);
      const firstHero=extractPropertyPhotos(items[0])[0];if(firstHero){c.globalAlpha=selectionVisual==="luxuryDark"?.34:.20;await drawRemoteImage(c,firstHero.url,380,0,860,1754,"cover");c.globalAlpha=1;}
      c.fillStyle=pal.accent;c.fillRect(72,150,4,1300);if(brand.logo_url)await drawRemoteImage(c,brand.logo_url,95,75,180,100,"contain");c.fillStyle=selectionVisual==="luxuryDark"?"#fff":pal.ink;c.font='600 30px Arial';c.fillText(agencyName||"CRM Real Estate",95,245);c.font='bold 68px Georgia, "Times New Roman", serif';pdfDrawWrapped(c,txt.selection,95,430,980,78);c.fillStyle=pal.accent;c.font='bold 25px Arial';c.fillText(`${txt.date}: ${formatPresentationDate(selectionLang)}`,95,700);if(selectionClient.trim()){c.fillStyle=selectionVisual==="luxuryDark"?"#eee":pal.muted;c.font='24px Arial';c.fillText(txt.prepared,95,805);c.fillStyle=selectionVisual==="luxuryDark"?"#fff":pal.ink;c.font='bold 46px Georgia, "Times New Roman", serif';pdfDrawWrapped(c,selectionClient.trim(),95,875,920,55);}c.fillStyle=selectionVisual==="luxuryDark"?"#ddd":pal.muted;c.font='22px Arial';c.fillText(`${items.length} ${selectionLang==="en"?"properties":selectionLang==="uk"?"об'єктів":"объектов"}`,95,1110);pages.push({bytes:pdfDataUrlToBytes(cover.toDataURL("image/jpeg",.96)),width:1240,height:1754});
      for(let index=0;index<items.length;index++){
        const p=items[index],photo=extractPropertyPhotos(p)[0],canvas=document.createElement("canvas");canvas.width=1240;canvas.height=1754;const ctx=canvas.getContext("2d");ctx.fillStyle=pal.paper;ctx.fillRect(0,0,1240,1754);ctx.fillStyle=pal.accent;ctx.font='bold 78px Georgia, "Times New Roman", serif';ctx.fillText(String(index+1).padStart(2,"0"),80,115);ctx.fillStyle=pal.ink;ctx.font='bold 45px Georgia, "Times New Roman", serif';pdfDrawWrapped(ctx,p.title||`${txt.property} ${index+1}`,220,75,900,50);ctx.fillStyle=pal.muted;ctx.font='20px Arial';ctx.fillText(`${formatPresentationDate(selectionLang)} · ${index+1}/${items.length}`,220,165);if(photo)await drawRemoteImage(ctx,photo.url,80,225,1080,690,"cover");const boxes=[[txt.price,marketMoney(p.price,p.currency||"USD")],[txt.area,`${p.area||0} м²`],[txt.floor,p.floor||"—"],[txt.district,p.district||"—"],[txt.type,p.type||"—"]];boxes.forEach(([a,b],i)=>{const x=80+i*216;ctx.strokeStyle=pal.line;ctx.lineWidth=1;ctx.strokeRect(x,965,196,130);ctx.fillStyle=pal.muted;ctx.font='17px Arial';ctx.fillText(a,x+16,1002);ctx.fillStyle=pal.ink;ctx.font='bold 24px Arial';pdfDrawWrapped(ctx,String(b),x+16,1043,165,28);});ctx.fillStyle=pal.ink;ctx.font='bold 31px Georgia, "Times New Roman", serif';ctx.fillText(txt.about,80,1185);const preview=String((selectionLang==="uk"?(p.description_uk||p.description):p.description)||txt.noDescription);ctx.fillStyle=pal.muted;ctx.font='22px Georgia, "Times New Roman", serif';const teaserLines=canvasTextLines(ctx,preview,1080).slice(0,7);let ty=1240;teaserLines.forEach(line=>{ctx.fillText(line,80,ty);ty+=34;});pages.push({bytes:pdfDataUrlToBytes(canvas.toDataURL("image/jpeg",.97)),width:1240,height:1754});addFullDescriptionPages(pages,{text:preview,title:p.title,agencyName,brand,themeId:theme,visualId:selectionVisual,lang:selectionLang});await addLuxuryGalleryPage(pages,{photos:extractPropertyPhotos(p),title:p.title,agencyName,themeId:theme,visualId:selectionVisual,lang:selectionLang});
      }
      if(selectionComparison&&items.length>1)addSelectionComparisonPages(pages,{items,themeId:theme,visualId:selectionVisual,lang:selectionLang,agencyName});
      const blob=buildPdfFromCanvasPages(pages),localUrl=URL.createObjectURL(blob);setSelectionDownloadUrl(localUrl);const path=`selections/${Date.now()}-market-selection.pdf`,uploaded=await uploadToFirstAvailableBucket(["presentations"],path,blob,{upsert:false,contentType:"application/pdf",cacheControl:"3600"});setSelectionUrl(uploaded?.publicUrl||localUrl);recordActivity?.({action_type:"market_selection_created",entity_type:"selection",entity_id:selected.join(","),client_id:selectionClientId,client_name:selectionClient});alert(uploaded?.publicUrl?"Подборка из аналитики создана.":"Подборка создана локально.");
    }catch(e){console.error(e);alert("Не удалось создать подборку: "+(e?.message||e));}
  }

  const sourceCount=sourcesX.filter(x=>x.enabled&&!x.paused).length+channels.filter(x=>x.enabled&&!x.paused).length;
  const syncResults=Array.isArray(marketSyncResult?.results)?marketSyncResult.results:[];
  const displayListings=filteredListings.slice(0,visibleCount);

  const premiumCss=`
    .mt-shell{display:grid;gap:16px}.mt-hero{position:relative;overflow:hidden;border-radius:28px;padding:26px;background:linear-gradient(135deg,var(--dark),color-mix(in srgb,var(--dark) 82%,var(--accent)));color:#fff;box-shadow:0 24px 65px rgba(15,23,42,.22)}
    .mt-hero:after{content:"";position:absolute;width:360px;height:360px;border-radius:50%;right:-110px;top:-180px;background:radial-gradient(circle,color-mix(in srgb,var(--accent) 55%,transparent),transparent 68%);pointer-events:none}.mt-eyebrow{font-size:12px;letter-spacing:.16em;text-transform:uppercase;opacity:.72;font-weight:900}.mt-title{font:800 clamp(28px,4vw,46px) Georgia,"Times New Roman",serif;margin:8px 0 9px}.mt-sub{max-width:780px;opacity:.78;line-height:1.55;margin:0}.mt-heroActions{display:flex;gap:10px;flex-wrap:wrap;margin-top:20px}.mt-heroActions .btn{min-height:48px;border-radius:15px!important}.mt-sync{background:linear-gradient(135deg,var(--accent),var(--accent2))!important;color:#111!important;border:0!important;font-weight:950!important;box-shadow:0 14px 30px color-mix(in srgb,var(--accent) 28%,transparent)!important}
    .mt-tabs{display:flex;gap:8px;flex-wrap:wrap;padding:7px;background:color-mix(in srgb,var(--soft) 60%,var(--card));border:1px solid color-mix(in srgb,var(--accent) 22%,#e5e7eb);border-radius:18px}.mt-tabs .btn{border-radius:13px!important}
    .mt-panel{border-radius:24px!important;padding:20px!important}.mt-period{display:flex;gap:8px;flex-wrap:wrap}.mt-period button{border:1px solid color-mix(in srgb,var(--accent) 28%,#ddd);background:var(--card);padding:10px 14px;border-radius:999px;font-weight:850;cursor:pointer;color:var(--text)}.mt-period button.active{background:linear-gradient(135deg,var(--accent),var(--accent2));color:#111;border-color:var(--accent);box-shadow:0 9px 20px color-mix(in srgb,var(--accent) 22%,transparent)}
    .mt-filterGrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:16px}.mt-filterCell{padding:12px;border:1px solid color-mix(in srgb,var(--accent) 22%,#e5e7eb);border-radius:16px;background:color-mix(in srgb,var(--soft) 45%,var(--card))}.mt-filterCell label{display:block;font-size:11px;letter-spacing:.08em;text-transform:uppercase;font-weight:900;opacity:.58;margin-bottom:7px}.mt-filterCell input,.mt-filterCell select{width:100%;min-height:42px;border-radius:11px!important}
    .mt-kpis{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:12px}.mt-kpi{position:relative;overflow:hidden;border-radius:21px;padding:17px;background:linear-gradient(145deg,var(--card),color-mix(in srgb,var(--soft) 58%,var(--card)));border:1px solid color-mix(in srgb,var(--accent) 22%,#e5e7eb);box-shadow:0 14px 38px rgba(15,23,42,.07)}.mt-kpi small{display:block;font-size:11px;text-transform:uppercase;letter-spacing:.07em;opacity:.58;font-weight:900}.mt-kpi strong{display:block;font-size:26px;margin-top:8px;line-height:1.05}.mt-kpi .mt-kpiMark{position:absolute;right:14px;top:13px;font-size:20px;opacity:.22}
    .mt-listHead{display:flex;align-items:end;justify-content:space-between;gap:12px;flex-wrap:wrap}.mt-listHead h3{margin:0}.mt-listHead p{margin:5px 0 0}.mt-cards{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-top:14px}.mt-card{display:grid;grid-template-columns:190px minmax(0,1fr);min-height:235px;overflow:hidden;border-radius:22px;background:var(--card);border:1px solid color-mix(in srgb,var(--accent) 22%,#e5e7eb);box-shadow:0 16px 44px rgba(15,23,42,.08);transition:.18s ease}.mt-card:hover{transform:translateY(-2px);box-shadow:0 22px 55px rgba(15,23,42,.12)}.mt-photo{position:relative;background:linear-gradient(135deg,color-mix(in srgb,var(--soft) 80%,#ddd),#eee);min-height:235px;overflow:hidden}.mt-photo img{width:100%;height:100%;object-fit:cover;display:block}.mt-photoEmpty{height:100%;min-height:235px;display:grid;place-items:center;font-size:44px;opacity:.24}.mt-sourcePill{position:absolute;left:11px;top:11px;color:#fff;padding:7px 10px;border-radius:999px;font-size:11px;font-weight:950;box-shadow:0 7px 16px rgba(0,0,0,.16)}.mt-fav{position:absolute;right:10px;top:10px;width:36px;height:36px;border-radius:50%;border:0;background:rgba(255,255,255,.94);font-size:20px;cursor:pointer;box-shadow:0 7px 18px rgba(0,0,0,.14)}.mt-cardBody{padding:16px;min-width:0;display:flex;flex-direction:column}.mt-cardTitle{font:800 20px Georgia,"Times New Roman",serif;margin:0 0 7px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.mt-location{font-size:13px;opacity:.66;margin-bottom:12px}.mt-price{font-size:24px;font-weight:950;margin-bottom:9px}.mt-facts{display:flex;gap:7px;flex-wrap:wrap}.mt-fact{padding:7px 9px;border-radius:10px;background:color-mix(in srgb,var(--soft) 62%,var(--card));border:1px solid color-mix(in srgb,var(--accent) 16%,#e5e7eb);font-size:12px;font-weight:800}.mt-cardMeta{display:flex;justify-content:space-between;gap:8px;align-items:center;margin-top:12px;font-size:11px;opacity:.62}.mt-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:auto;padding-top:13px}.mt-actions .btn,.mt-actions a{min-height:42px;border-radius:12px!important;display:flex;align-items:center;justify-content:center;text-align:center}.mt-selectBtn.selected{background:linear-gradient(135deg,var(--accent),var(--accent2))!important;color:#111!important;font-weight:950!important}.mt-crmBox{margin-top:10px;border:1px solid color-mix(in srgb,var(--accent) 38%,#ddd);border-radius:13px;padding:9px 10px;background:color-mix(in srgb,var(--accent) 8%,var(--card));display:flex;align-items:center;justify-content:space-between;gap:8px;font-size:12px}.mt-crmBox b{font-size:12px}.mt-empty{padding:48px 20px;text-align:center;border:1px dashed color-mix(in srgb,var(--accent) 35%,#ccc);border-radius:20px;opacity:.75}.mt-selection{border-radius:25px!important;border:1.5px solid color-mix(in srgb,var(--accent) 38%,#ddd)!important;background:linear-gradient(145deg,color-mix(in srgb,var(--accent) 10%,var(--card)),var(--card))!important}.mt-selectionTop{display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap}.mt-selectedStrip{display:flex;gap:8px;overflow:auto;padding:8px 0 2px}.mt-selectedMini{flex:0 0 165px;border:1px solid color-mix(in srgb,var(--accent) 20%,#ddd);border-radius:13px;padding:9px;background:var(--card)}.mt-selectedMini b{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:12px}.mt-selectedMini span{font-size:11px;opacity:.6}.mt-sourceGrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.mt-sourceCard{padding:14px;border:1px solid color-mix(in srgb,var(--accent) 22%,#ddd);border-radius:16px;background:var(--card)}.mt-sourceCard h4{margin:0 0 5px}.mt-sourceCard .row{margin-top:10px}.mt-history details{border-radius:15px}.mt-loadMore{display:flex;justify-content:center;margin-top:16px}
    @media(max-width:1180px){.mt-filterGrid{grid-template-columns:repeat(3,1fr)}.mt-kpis{grid-template-columns:repeat(3,1fr)}.mt-cards{grid-template-columns:1fr}.mt-sourceGrid{grid-template-columns:repeat(2,1fr)}}
    @media(max-width:760px){.mt-filterGrid{grid-template-columns:1fr 1fr}.mt-kpis{grid-template-columns:1fr 1fr}.mt-card{grid-template-columns:1fr}.mt-photo{height:220px;min-height:220px}.mt-sourceGrid{grid-template-columns:1fr}.mt-actions{grid-template-columns:1fr}.mt-hero{padding:20px}.mt-selectionTop{align-items:flex-start}.mt-cards{gap:11px}}
  `;

  return <div className="mt-shell">
    <style>{premiumCss}</style>

    <section className="mt-hero">
      <div className="mt-eyebrow">MARKET INTELLIGENCE · CRM REAL ESTATE</div>
      <div className="row" style={{alignItems:"flex-start",position:"relative",zIndex:1}}>
        <div style={{flex:"1 1 520px"}}>
          <h1 className="mt-title">Аналитика рынка недвижимости</h1>
          <p className="mt-sub">Единый центр внешних объявлений и объектов CRM: фильтры по рынку, выбранный период, прямые ссылки на источники, переход в карточку CRM и создание клиентской подборки прямо из аналитики.</p>
          <div className="mt-heroActions">
            <Button className="mt-sync" onClick={syncMarketSources} disabled={syncingMarket||!marketDbReady||sourceCount===0}>{syncingMarket?"⏳ Обновляем рынок...":"↻ Обновить рынок"}</Button>
            <Button variant="soft" onClick={loadMarketData} disabled={loadingMarket}>{loadingMarket?"Загрузка...":"Обновить список"}</Button>
            <span className="badge" style={{alignSelf:"center",background:"rgba(255,255,255,.10)",color:"#fff",borderColor:"rgba(255,255,255,.18)"}}>{sourceCount} активных источника</span>
          </div>
        </div>
        <div style={{minWidth:230,textAlign:"right",position:"relative",zIndex:1}}>
          <div className="mt-eyebrow">Последняя синхронизация</div>
          <div style={{fontWeight:900,fontSize:16,marginTop:7}}>{formatSyncDate(lastMarketSyncAt)}</div>
          <div style={{opacity:.62,fontSize:12,marginTop:5}}>База: {externalListings.length} внешних объявлений</div>
        </div>
      </div>
    </section>

    <div className="mt-tabs">
      {[["market","Рынок"],["sources","Источники"],["telegram","Telegram"],["import","Импорт"],["ai","AI-обзор"]].map(([k,n])=><Button key={k} variant={tab===k?"primary":"soft"} onClick={()=>setTab(k)}>{n}</Button>)}
    </div>

    {!marketDbReady&&<div className="card amber"><b>Режим настройки</b><p className="muted" style={{marginBottom:0}}>Таблицы рыночной аналитики недоступны. Временные данные отображаются из локального хранилища.</p></div>}

    {marketSyncResult&&<div className="card mt-panel">
      <div className="row"><div><b>{marketSyncResult.success!==false?"Обновление завершено":"Обновление завершено с ошибкой"}</b><div className="muted">{marketSyncResult.message||marketSyncResult.error||"Результат получен."}</div></div><span className={cn("badge",marketSyncResult.success!==false?"gold":"")}>Обработано: {Number(marketSyncResult.processed||0)}</span><span className="badge">Ошибок: {Number(marketSyncResult.failed||0)}</span></div>
      {!!syncResults.length&&<div style={{marginTop:10}}>{syncResults.map((item,index)=><div className="dealRow" key={`${item.source_id||index}-${index}`}><div><b>{item.source_name||"Источник"}</b><div className="muted">{item.message||"Без сообщения"}</div></div><span className="badge">{item.connector||"—"}</span><span className={cn("badge",item.success!==false?"gold":"")}>{syncStatusLabel(item.status)}</span></div>)}</div>}
    </div>}

    {tab==="market"&&<>
      <div className="card mt-panel">
        <div className="row" style={{alignItems:"flex-start"}}><div><div className="mt-eyebrow" style={{color:"var(--text)",opacity:.52}}>ПЕРИОД И ПАРАМЕТРЫ</div><h3 style={{margin:"5px 0 0"}}>Настрой выборку</h3></div><Button variant="soft" onClick={resetFilters}>Сбросить фильтры</Button></div>
        <div className="mt-period" style={{marginTop:14}}>
          {[["all","Всё время"],["1","Сегодня"],["7","7 дней"],["30","30 дней"],["90","90 дней"],["custom","Свой период"]].map(([v,n])=><button key={v} className={periodPreset===v?"active":""} onClick={()=>setPeriodPreset(v)}>{n}</button>)}
        </div>
        {periodPreset==="custom"&&<div className="grid2" style={{marginTop:12}}><Field label="Дата от"><input type="date" value={filters.dateFrom} onChange={e=>setFilters({...filters,dateFrom:e.target.value})}/></Field><Field label="Дата до"><input type="date" value={filters.dateTo} onChange={e=>setFilters({...filters,dateTo:e.target.value})}/></Field></div>}
        <div className="row" style={{justifyContent:"flex-start",flexWrap:"wrap",marginTop:16}}>{MARKET_DEAL_TYPES.map(x=><Button key={x} variant={deal===x?"primary":"soft"} onClick={()=>setDeal(x)}>{x}</Button>)}</div>
        <div className="mt-filterGrid">
          <div className="mt-filterCell"><label>Страна</label><select value={filters.country} onChange={e=>setFilters({...filters,country:e.target.value,region:"",city:""})}><option value="">Все страны</option>{optionSets.countries.map(x=><option key={x}>{x}</option>)}</select></div>
          <div className="mt-filterCell"><label>Область / регион</label><input placeholder="Например: Одесская" value={filters.region} onChange={e=>setFilters({...filters,region:e.target.value})}/></div>
          <div className="mt-filterCell"><label>Город</label><select value={filters.city} onChange={e=>setFilters({...filters,city:e.target.value})}><option value="">Все города</option>{optionSets.cities.map(x=><option key={x}>{x}</option>)}</select></div>
          <div className="mt-filterCell"><label>Район / улица</label><input placeholder="Район, улица..." value={filters.district} onChange={e=>setFilters({...filters,district:e.target.value})}/></div>
          <div className="mt-filterCell"><label>ЖК / комплекс</label><input placeholder="Название ЖК..." value={filters.complex} onChange={e=>setFilters({...filters,complex:e.target.value})}/></div>
          <div className="mt-filterCell"><label>Тип недвижимости</label><select value={filters.propertyType} onChange={e=>setFilters({...filters,propertyType:e.target.value})}><option value="">Все типы</option>{optionSets.types.map(x=><option key={x}>{x}</option>)}</select></div>
          <div className="mt-filterCell"><label>Источник</label><select value={filters.source} onChange={e=>setFilters({...filters,source:e.target.value})}><option value="">Все источники</option>{optionSets.sources.map(x=><option key={x}>{x}</option>)}</select></div>
          <div className="mt-filterCell"><label>Комнат</label><select value={filters.rooms} onChange={e=>setFilters({...filters,rooms:e.target.value})}><option value="">Любое количество</option>{[1,2,3,4,5].map(x=><option key={x} value={x}>{x}{x===5?"+":""}</option>)}</select></div>
          <div className="mt-filterCell"><label>Цена от</label><input type="number" placeholder="0" value={filters.priceFrom} onChange={e=>setFilters({...filters,priceFrom:e.target.value})}/></div>
          <div className="mt-filterCell"><label>Цена до</label><input type="number" placeholder="без лимита" value={filters.priceTo} onChange={e=>setFilters({...filters,priceTo:e.target.value})}/></div>
          <div className="mt-filterCell"><label>Площадь от, м²</label><input type="number" placeholder="0" value={filters.areaFrom} onChange={e=>setFilters({...filters,areaFrom:e.target.value})}/></div>
          <div className="mt-filterCell"><label>Площадь до, м²</label><input type="number" placeholder="без лимита" value={filters.areaTo} onChange={e=>setFilters({...filters,areaTo:e.target.value})}/></div>
        </div>
      </div>

      <div className="mt-kpis">
        <div className="mt-kpi"><span className="mt-kpiMark">⌂</span><small>Объявлений рынка</small><strong>{stats.count}</strong></div>
        <div className="mt-kpi"><span className="mt-kpiMark">$</span><small>Средняя цена</small><strong>{marketMoney(stats.avg,filters.currency)}</strong></div>
        <div className="mt-kpi"><span className="mt-kpiMark">㎡</span><small>Средняя цена м²</small><strong>{marketMoney(stats.sqm,filters.currency)}</strong></div>
        <div className="mt-kpi"><span className="mt-kpiMark">↓</span><small>Ниже рынка 10%+</small><strong>{stats.belowMarket.length}</strong></div>
        <div className="mt-kpi"><span className="mt-kpiMark">◉</span><small>Активный район</small><strong style={{fontSize:19}}>{stats.topDistrict}</strong></div>
        <div className="mt-kpi"><span className="mt-kpiMark">CRM</span><small>Объектов CRM в выборке</small><strong>{stats.crmCount}</strong></div>
      </div>

      {selected.length>0&&<div className="card mt-selection">
        <div className="mt-selectionTop"><div><div className="mt-eyebrow" style={{color:"var(--text)",opacity:.5}}>КЛИЕНТСКАЯ ПОДБОРКА</div><h3 style={{margin:"5px 0"}}>Выбрано объектов: {selected.length}</h3><p className="muted" style={{margin:0}}>В подборку можно одновременно добавлять объявления из внешних источников и объекты CRM.</p></div><Button variant="soft" onClick={()=>setSelected([])}>Очистить</Button></div>
        <div className="mt-selectedStrip">{selectedListings.map(p=><div className="mt-selectedMini" key={listingKey(p)}><b>{p.title}</b><span>{p.source_name} · {marketMoney(p.price,p.currency)}</span></div>)}</div>
        <div className="grid4" style={{marginTop:12}}>
          <Field label="Клиент из CRM"><select className="input" value={selectionClientId} onChange={e=>{const id=e.target.value;setSelectionClientId(id);const c=(leads||[]).find(x=>String(x.id)===String(id));if(c){setSelectionClient(c.name||"");setSelectionClientPhone(c.phone||"");setSelectionClientEmail(c.email||"");}}}><option value="">Выбрать клиента...</option>{(leads||[]).map(c=><option key={c.id} value={c.id}>{c.name||"Без имени"} · {c.phone||"без телефона"}</option>)}</select></Field>
          <Field label="Имя клиента"><input value={selectionClient} placeholder="Например: Александр" onChange={e=>setSelectionClient(e.target.value)}/></Field>
          <Field label="Язык"><select value={selectionLang} onChange={e=>setSelectionLang(e.target.value)}><option value="ru">Русский</option><option value="uk">Українська</option><option value="en">English</option></select></Field>
          <Field label="Визуал"><select value={selectionVisual} onChange={e=>setSelectionVisual(e.target.value)}>{presentationVisualOptions.map(x=><option key={x.id} value={x.id}>{x.label}</option>)}</select></Field>
        </div>
        <div className="grid3"><Field label="Телефон"><input value={selectionClientPhone} placeholder="+380..." onChange={e=>setSelectionClientPhone(e.target.value)}/></Field><Field label="Email"><input value={selectionClientEmail} placeholder="client@email.com" onChange={e=>setSelectionClientEmail(e.target.value)}/></Field><label className="chip" style={{alignSelf:"end",minHeight:48}}><input type="checkbox" checked={selectionComparison} onChange={e=>setSelectionComparison(e.target.checked)}/> Сравнительная таблица</label></div>
        <Button className="full" onClick={createMarketSelection}>Создать VIP-подборку ({selected.length})</Button>
      </div>}

      {selectionUrl&&<div className="card presentationSendPanel"><h3 style={{marginTop:0}}>Отправить подборку из аналитики</h3><p className="muted sendIntro">Готовый PDF можно сразу отправить выбранному клиенту.</p><div className="grid4"><Button className="full sendWhatsApp" onClick={async()=>{if(await openWhatsAppPresentation({phone:selectionClientPhone,url:selectionUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true}))recordActivity?.({action_type:"presentation_sent",entity_type:"market_selection",channel:"whatsapp",client_id:selectionClientId,client_name:selectionClient});}}>WhatsApp</Button><Button className="full sendViber" onClick={async()=>{if(await openViberPresentation({phone:selectionClientPhone,url:selectionUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true}))recordActivity?.({action_type:"presentation_sent",entity_type:"market_selection",channel:"viber",client_id:selectionClientId,client_name:selectionClient});}}>Viber</Button><Button className="full sendEmail" onClick={()=>openEmailPresentation({email:selectionClientEmail,url:selectionUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true})}>Email</Button><Button className="full sendShare" onClick={()=>nativeSharePresentation({url:selectionUrl,downloadUrl:selectionDownloadUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true})}>Поделиться</Button></div><div className="grid2" style={{marginTop:10}}><a className="btn primary full" href={selectionDownloadUrl||selectionUrl} download={`CRM-market-selection-${new Date().toISOString().slice(0,10)}.pdf`}>Скачать подборку</a><Button variant="soft" onClick={async()=>{await navigator.clipboard.writeText(selectionUrl);alert("Ссылка скопирована");}}>Скопировать ссылку</Button></div></div>}

      <div className="card mt-panel">
        <div className="mt-listHead"><div><div className="mt-eyebrow" style={{color:"var(--text)",opacity:.5}}>КОНКРЕТНЫЕ ОБЪЕКТЫ</div><h3>{deal} · {filteredListings.length}</h3><p className="muted">Каждая карточка ведёт к оригиналу. Если объект найден в CRM — можно сразу открыть его карточку.</p></div><Field label="Сортировка"><select value={sortBy} onChange={e=>setSortBy(e.target.value)}><option value="newest">Сначала новые</option><option value="priceAsc">Цена: дешевле</option><option value="priceDesc">Цена: дороже</option><option value="sqmAsc">Цена за м²: дешевле</option></select></Field></div>
        {displayListings.length?<div className="mt-cards">{displayListings.map(p=>{
          const key=listingKey(p),fav=favorites.includes(key),checked=selected.includes(key),crmMatch=crmMatchForListing(p),pricePerSqm=p.area>0?Math.round(p.price/p.area):0;
          return <article className="mt-card" key={key}>
            <div className="mt-photo">
              {p.main_photo_url?<img src={p.main_photo_url} alt={p.title} onError={e=>{e.currentTarget.style.display="none";e.currentTarget.nextElementSibling&&(e.currentTarget.nextElementSibling.style.display="grid");}}/>:null}
              <div className="mt-photoEmpty" style={{display:p.main_photo_url?"none":"grid"}}>⌂</div>
              <span className="mt-sourcePill" style={{background:sourceTone(p.source_name)}}>{p.source_name}</span>
              <button className="mt-fav" onClick={()=>toggleFavorite(p)} title="Избранное">{fav?"★":"☆"}</button>
            </div>
            <div className="mt-cardBody">
              <h3 className="mt-cardTitle" title={p.title}>{p.title}</h3>
              <div className="mt-location">{[p.country,p.region,p.city,p.district,p.complex_name].filter(Boolean).join(" · ")||"Локация не указана"}</div>
              <div className="mt-price">{marketMoney(p.price,p.currency||filters.currency)}</div>
              <div className="mt-facts"><span className="mt-fact">{p.rooms||"—"} комн.</span><span className="mt-fact">{p.area||"—"} м²</span>{pricePerSqm>0&&<span className="mt-fact">{marketMoney(pricePerSqm,p.currency)}/м²</span>}{p.floor&&<span className="mt-fact">этаж {p.total_floors?`${p.floor}/${p.total_floors}`:p.floor}</span>}</div>
              <div className="mt-cardMeta"><span>{formatListingDate(p)}</span><span>{p.origin==="crm"?"Объект CRM":"Внешнее объявление"}</span></div>
              {crmMatch&&<div className="mt-crmBox"><div><b>✓ Объект есть в CRM</b><div style={{opacity:.62,marginTop:2}}>{crmMatch.title||"Карточка объекта"}</div></div><Button variant="soft" onClick={()=>onOpenProperty?.(crmMatch)}>Открыть CRM</Button></div>}
              <div className="mt-actions">
                {p.source_url?<a className="btn soft" href={p.source_url} target="_blank" rel="noopener noreferrer">Открыть оригинал ↗</a>:p.origin==="crm"?<Button variant="soft" onClick={()=>onOpenProperty?.(p.crmProperty)}>Открыть карточку CRM</Button>:<span/>}
                <Button className={cn("mt-selectBtn",checked&&"selected")} variant={checked?"primary":"soft"} onClick={()=>toggleSelected(p)}>{checked?"✓ В подборке":"＋ Добавить в подборку"}</Button>
              </div>
            </div>
          </article>;
        })}</div>:<div className="mt-empty"><h3>По этим параметрам объектов нет</h3><p>Измени период или сбрось часть фильтров. Для проверки уже загруженных данных выбери «Всё время».</p><Button variant="soft" onClick={resetFilters}>Показать всю выборку</Button></div>}
        {filteredListings.length>visibleCount&&<div className="mt-loadMore"><Button variant="soft" onClick={()=>setVisibleCount(x=>x+60)}>Показать ещё ({filteredListings.length-visibleCount})</Button></div>}
      </div>
    </>}

    {tab==="sources"&&<div className="card mt-panel"><div className="row"><div><div className="mt-eyebrow" style={{color:"var(--text)",opacity:.5}}>ПОДКЛЮЧЕНИЯ</div><h3 style={{margin:"5px 0"}}>Источники данных</h3><p className="muted">Техотдел подключает площадки любой страны через уже существующий универсальный механизм.</p></div><span className="badge gold">{sourcesX.length} источников</span></div>{isTech&&<><div className="grid2"><Field label="Название"><input value={sf.name} onChange={e=>setSf({...sf,name:e.target.value})}/></Field><Field label="Страна"><input value={sf.country} onChange={e=>setSf({...sf,country:e.target.value})}/></Field><Field label="Тип"><select value={sf.type} onChange={e=>setSf({...sf,type:e.target.value})}>{MARKET_SOURCE_TYPES.map(x=><option key={x}>{x}</option>)}</select></Field><Field label="API / XML / RSS / URL"><input value={sf.url} onChange={e=>setSf({...sf,url:e.target.value})}/></Field></div><Button className="full" onClick={addSource}>＋ Подключить источник</Button></>}<div className="mt-sourceGrid" style={{marginTop:14}}>{sourcesX.map(s=><div className="mt-sourceCard" key={s.id}><div className="row"><h4>{s.name}</h4><span className={cn("badge",s.enabled&&!s.paused?"gold":"")}>{s.enabled&&!s.paused?"Активен":"Отключен"}</span></div><div className="muted">{s.country||"Страна не указана"} · {s.connection_method||s.source_type||"—"}</div><div className="muted" style={{marginTop:5}}>Синхронизация: {formatSyncDate(s.last_sync_at)}</div><div className="row"> <span className="badge">{syncStatusLabel(s.connection_status)}</span>{isTech&&<><Button variant="soft" onClick={()=>toggleSource(s)}>{s.enabled?"Отключить":"Включить"}</Button><Button variant="danger" onClick={()=>removeSource(s)}>Удалить</Button></>}</div></div>)}</div>{!sourcesX.length&&<p className="muted">Площадок пока нет.</p>}</div>}

    {tab==="telegram"&&<div className="card mt-panel"><div className="row"><div><div className="mt-eyebrow" style={{color:"var(--text)",opacity:.5}}>TELEGRAM SOURCES</div><h3 style={{margin:"5px 0"}}>Публичные Telegram-каналы</h3><p className="muted">Ссылки на открытые каналы используются действующим Telegram-коннектором.</p></div><span className="badge gold">{channels.length} каналов</span></div>{isTech&&<><div className="grid2"><Field label="Название"><input value={cf.name} onChange={e=>setCf({...cf,name:e.target.value})}/></Field><Field label="Ссылка / @username"><input value={cf.url} onChange={e=>setCf({...cf,url:e.target.value})}/></Field><Field label="Страна"><input value={cf.country} onChange={e=>setCf({...cf,country:e.target.value})}/></Field><Field label="Город"><input value={cf.city} onChange={e=>setCf({...cf,city:e.target.value})}/></Field></div><Button className="full" onClick={addChannel}>＋ Добавить Telegram-канал</Button></>}<div className="mt-sourceGrid" style={{marginTop:14}}>{channels.map(c=><div className="mt-sourceCard" key={c.id}><div className="row"><h4>{c.name}</h4><span className={cn("badge",c.enabled&&!c.paused?"gold":"")}>{c.enabled&&!c.paused?"Активен":"Отключен"}</span></div><div className="muted">{c.city||"—"} · {c.country||"—"}</div><div className="muted" style={{marginTop:5}}>Синхронизация: {formatSyncDate(c.last_sync_at)}</div><div className="row"><span className="badge">Telegram</span><span className="badge">{syncStatusLabel(c.connection_status)}</span>{isTech&&<><Button variant="soft" onClick={()=>toggleSource(c)}>{c.enabled?"Отключить":"Включить"}</Button><Button variant="danger" onClick={()=>removeSource(c)}>Удалить</Button></>}</div></div>)}</div>{!channels.length&&<p className="muted">Telegram-каналы пока не добавлены.</p>}</div>}

    {tab==="import"&&<div className="card mt-panel"><div className="row"><div><div className="mt-eyebrow" style={{color:"var(--text)",opacity:.5}}>DATA IMPORT</div><h3 style={{margin:"5px 0"}}>Объекты рынка</h3><p className="muted">Ручное добавление и CSV остаются как резервный способ наполнения аналитики.</p></div><span className="badge gold">{externalListings.length} объявлений</span></div>{isTech&&<><div className="grid2"><Field label="Название объекта"><input value={listingForm.title} onChange={e=>setListingForm({...listingForm,title:e.target.value})}/></Field><Field label="Тип сделки"><select value={listingForm.deal_type} onChange={e=>setListingForm({...listingForm,deal_type:e.target.value})}>{MARKET_DEAL_TYPES.map(x=><option key={x}>{x}</option>)}</select></Field><Field label="Страна"><input value={listingForm.country} onChange={e=>setListingForm({...listingForm,country:e.target.value})}/></Field><Field label="Город"><input value={listingForm.city} onChange={e=>setListingForm({...listingForm,city:e.target.value})}/></Field><Field label="Район / ЖК"><input value={listingForm.district} onChange={e=>setListingForm({...listingForm,district:e.target.value})}/></Field><Field label="Комнат"><input type="number" value={listingForm.rooms} onChange={e=>setListingForm({...listingForm,rooms:e.target.value})}/></Field><Field label="Цена"><input type="number" value={listingForm.price} onChange={e=>setListingForm({...listingForm,price:e.target.value})}/></Field><Field label="Площадь, м²"><input type="number" value={listingForm.area} onChange={e=>setListingForm({...listingForm,area:e.target.value})}/></Field><Field label="Источник"><input placeholder="OLX / DIM.RIA / канал..." value={listingForm.source_name} onChange={e=>setListingForm({...listingForm,source_name:e.target.value})}/></Field><Field label="Ссылка на оригинал"><input placeholder="https://..." value={listingForm.source_url} onChange={e=>setListingForm({...listingForm,source_url:e.target.value})}/></Field></div><Field label="Описание"><textarea value={listingForm.description} onChange={e=>setListingForm({...listingForm,description:e.target.value})}/></Field><Button className="full" onClick={addExternalListing}>＋ Добавить объект в аналитику</Button><div className="card" style={{marginTop:14}}><h3>Массовый импорт CSV</h3><p className="muted">title, deal_type, country, city, district, rooms, price, area, currency, source_name, source_url, description.</p><input className="input" type="file" accept=".csv,.txt" onChange={e=>importMarketCsv(e.target.files?.[0])}/></div></>}</div>}

    {tab==="ai"&&<div className="card mt-panel"><div className="mt-eyebrow" style={{color:"var(--text)",opacity:.5}}>SMART MARKET REVIEW</div><h3 style={{margin:"5px 0 14px"}}>AI-обзор текущей выборки</h3><div className="mt-kpis"><div className="mt-kpi"><small>Рынок</small><strong>{stats.count}</strong></div><div className="mt-kpi"><small>Средняя цена</small><strong>{marketMoney(stats.avg,filters.currency)}</strong></div><div className="mt-kpi"><small>Средняя цена м²</small><strong>{marketMoney(stats.sqm,filters.currency)}</strong></div><div className="mt-kpi"><small>Активный район</small><strong style={{fontSize:18}}>{stats.topDistrict}</strong></div><div className="mt-kpi"><small>Ведущий источник</small><strong style={{fontSize:18}}>{stats.topSource}</strong></div><div className="mt-kpi"><small>Ниже рынка</small><strong>{stats.belowMarket.length}</strong></div></div><div className="card amber" style={{marginTop:14}}><b>Автоматический вывод</b><p style={{lineHeight:1.6}}>В выбранном периоде найдено {stats.count} внешних объявлений. Средняя цена — {marketMoney(stats.avg,filters.currency)}, медианная — {marketMoney(stats.median,filters.currency)}, средняя цена квадратного метра — {marketMoney(stats.sqm,filters.currency)}. Чаще всего встречается район «{stats.topDistrict}». Найдено {stats.belowMarket.length} предложений минимум на 10% ниже средней цены текущей выборки.</p></div>{stats.belowMarket.slice(0,12).map(p=><div className="dealRow" key={`below-${p.id}`}><div><b>{p.title}</b><div className="muted">{p.source_name} · {[p.city,p.district].filter(Boolean).join(" · ")}</div></div><b>{marketMoney(p.price,p.currency)}</b>{p.source_url?<a className="btn soft" href={p.source_url} target="_blank" rel="noopener noreferrer">Оригинал</a>:<span/>}</div>)}</div>}

    {!!marketSyncLogs.length&&<div className="card mt-panel mt-history"><details><summary style={{cursor:"pointer",fontWeight:900}}>История обновлений ({marketSyncLogs.length})</summary><div style={{marginTop:10}}>{marketSyncLogs.slice(0,15).map(log=><div className="dealRow" key={log.id}><div><b>{log.source_name||"Источник"}</b><div className="muted">{formatSyncDate(log.finished_at||log.created_at)}</div></div><span className="badge">{log.source_type||"—"}</span><span className={cn("badge",log.status==="completed"?"gold":"")}>{syncStatusLabel(log.status)}</span><span>Найдено: {Number(log.listings_found||0)}</span><span>Добавлено: {Number(log.listings_created||0)}</span>{log.error_message?<span style={{color:"#b91c1c"}}>{log.error_message}</span>:<span/>}</div>)}</div></details></div>}
  </div>;
}
  ...
  return <div className="layout" data-theme={theme} style={themeStyle(theme)}><style>{globalThemeCss(theme)}</style>
    ...
    <div className="mainWrap">
      <main className="main">
        ...
        {page==="analytics"&&<Analytics leads={visibleLeads} properties={properties} events={visibleEvents} role={role} users={users} activities={activities} currentProfile={currentProfile}/>} 
        {page==="tools"&&<MarketTools lang={lang} currentProfile={currentProfile} properties={properties} onOpenProperty={setProperty} leads={visibleLeads} agencyName={agencyName} theme={theme} recordActivity={recordManagerActivity}/>} 
