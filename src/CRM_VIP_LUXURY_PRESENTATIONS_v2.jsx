...
function addFullDescriptionPages(pages,{text,title,agencyName,brand,themeId,visualId,lang}){
  ...
  const probe=document.createElement("canvas");probe.width=1240;probe.height=1754;const px=probe.getContext("2d");px.font='25px Georgia, "Times New Roman", serif';
  const lines=canvasTextLines(px,text||txt.noDescription,1030);
  const perPage=32;
  for(let offset=0; offset<lines.length; offset+=perPage){
    ...
    ctx.fillStyle=pal.dark;ctx.fillRect(0,0,1240,170);ctx.fillStyle=pal.accent;ctx.fillRect(80,165,1080,3);
    ctx.fillStyle="#fff";ctx.font='600 25px Arial';ctx.fillText(agencyName||"CRM Real Estate",90,62);
    ctx.font='bold 39px Georgia, "Times New Roman", serif';ctx.fillText(txt.description,90,125);
    ctx.fillStyle=pal.ink;ctx.font='bold 34px Georgia, "Times New Roman", serif';pdfDrawWrapped(ctx,title||txt.property,90,250,1020,44);
    let y=370;ctx.fillStyle=pal.ink;ctx.font='25px Georgia, "Times New Roman", serif';
    for(const line of lines.slice(offset,offset+perPage)){ctx.fillText(line,90,y);y+=39;}
    ctx.fillStyle=pal.muted;ctx.font='18px Arial';ctx.fillText(`${formatPresentationDate(lang)}  ·  ${Math.floor(offset/perPage)+1}`,90,1665);
    pages.push({bytes:pdfDataUrlToBytes(c.toDataURL("image/jpeg",.96)),width:1240,height:1754});
  }
}
async function addLuxuryGalleryPage(pages,{photos,title,agencyName,themeId,visualId,lang}){
  ...
  for(let start=0; start<photos.length; start+=4){
    ...
    ctx.fillStyle=pal.paper;ctx.fillRect(0,0,1240,1754);ctx.fillStyle=pal.ink;ctx.font='bold 44px Georgia, "Times New Roman", serif';ctx.fillText(txt.photos,80,105);
    ctx.fillStyle=pal.accent;ctx.fillRect(80,135,1080,3);ctx.fillStyle=pal.muted;ctx.font='21px Arial';ctx.fillText(title||"",80,178);
    const boxes=[[80,240,700,650],[805,240,355,310],[805,580,355,310],[80,920,1080,650]];
    for(let i=0;i<chunk.length;i++) await drawRemoteImage(ctx,chunk[i].url,...boxes[i],"cover");
    pages.push({bytes:pdfDataUrlToBytes(c.toDataURL("image/jpeg",.96)),width:1240,height:1754});
  }
}
...
function Properties({properties,setProperties,onOpen,users,currentProfile,lang,agencyName,theme}) {
  ...
  async function createSelection(){
    ...
    try {
      ...
      const firstHero=(items[0]?.media||[]).find(m=>String(m.kind||"").includes("Фото")&&m.url);
      ...
      for(let index=0;index<items.length;index++){
        const p=items[index],photo=(p.media||[]).find(m=>String(m.kind||"").includes("Фото")&&m.url);
        ...
        ctx.fillStyle=pal.muted;ctx.font='23px Georgia, "Times New Roman", serif';const preview=String((selectionLang==="uk"?(p.description_uk||p.description):p.description)||txt.noDescription);pdfDrawWrapped(ctx,preview,80,1240,1080,34);
        pages.push({bytes:pdfDataUrlToBytes(canvas.toDataURL("image/jpeg",.96)),width:1240,height:1754});
        ...
        const photos=(p.media||[]).filter(m=>String(m.kind||"").includes("Фото")&&m.url).slice(0,8);
      ...
      if(selectionComparison && items.length>1){
        ...
        ctx.fillStyle=pal.ink;ctx.font='bold 48px Georgia, "Times New Roman", serif';ctx.fillText(txt.comparison,80,120);ctx.fillStyle=pal.accent;ctx.fillRect(80,150,1080,3);
        const headers=selectionLang==="en"?["#","Property","Price","Area","Price/m²","District","Floor"]:selectionLang==="uk"?["#","Об'єкт","Ціна","Площа","Ціна/м²","Район","Поверх"]:["#","Объект","Цена","Площадь","Цена/м²","Район","Этаж"];
        const xs=[80,135,480,650,790,930,1080];ctx.font='bold 17px Arial';ctx.fillStyle=pal.muted;headers.forEach((h,i)=>ctx.fillText(h,xs[i],235));
        let y=285;for(let i=0;i<items.length && i<14;i++){const p=items[i],ppm=Number(p.area)?Math.round(Number(p.price||0)/Number(p.area)):0;ctx.strokeStyle=pal.line;ctx.beginPath();ctx.moveTo(80,y-25);ctx.lineTo(1160,y-25);ctx.stroke();ctx.fillStyle=pal.accent;ctx.font='bold 20px Georgia';ctx.fillText(String(i+1).padStart(2,"0"),80,y);ctx.fillStyle=pal.ink;ctx.font='18px Arial';pdfDrawWrapped(ctx,p.title||"—",135,y,320,22);ctx.fillText(money(p.price),480,y);ctx.fillText(`${p.area||0} м²`,650,y);ctx.fillText(ppm?money(ppm):"—",790,y);ctx.fillText(p.district||"—",930,y);ctx.fillText(p.floor||"—",1080,y);y+=92;}
        pages.push({bytes:pdfDataUrlToBytes(cmp.toDataURL("image/jpeg",.96)),width:1240,height:1754});
      }
  ...
  return <main className="screen">
    <div className="row">
      ...
      {selected.length>0 && <div className="card" style={{width:"100%",marginTop:10}}><b>VIP-подборка для клиента</b><div className="grid4" style={{marginTop:10}}><Field label="Персональное имя клиента"><input className="input" placeholder="Например: Александр" value={selectionClient} onChange={e=>setSelectionClient(e.target.value)}/></Field><Field label="Язык презентации"><select className="input" value={selectionLang} onChange={e=>setSelectionLang(e.target.value)}><option value="ru">Русский</option><option value="uk">Українська</option><option value="en">English</option></select></Field><Field label="Визуал презентации"><select className="input" value={selectionVisual} onChange={e=>setSelectionVisual(e.target.value)}>{presentationVisualOptions.map(x=><option key={x.id} value={x.id}>{x.label}</option>)}</select></Field><label className="chip" style={{alignSelf:"end",minHeight:48}}><input type="checkbox" checked={selectionComparison} onChange={e=>setSelectionComparison(e.target.checked)}/> Сравнение предложений</label></div><Button className="full" onClick={createSelection}>{tr(lang,"Создать подборку для клиента")} ({selected.length})</Button></div>}
    </div>
...
function PropertyModal({property,setProperties,onClose,users,currentProfile,role,lang,agencyName,theme}) {
  ...
  async function generateClientPresentation(){
    try{
      ...
      const pages=[],hero=(local.media||[]).find(m=>String(m.kind||"").includes("Фото")&&m.url);
      ...
      ctx.fillStyle=pal.ink;ctx.font='bold 31px Georgia, "Times New Roman", serif';ctx.fillText(txt.about,80,1185);const desc=(presentationLang==="uk"?(local.description_uk||local.description):local.description)||txt.noDescription;ctx.fillStyle=pal.muted;ctx.font='23px Georgia, "Times New Roman", serif';pdfDrawWrapped(ctx,desc,80,1240,1080,34);pages.push({bytes:dataUrlToBytes(info.toDataURL("image/jpeg",.96)),width:1240,height:1754});
      ...
      const photos=(local.media||[]).filter(m=>String(m.kind||"").includes("Фото")&&m.url).slice(0,12);await addLuxuryGalleryPage(pages,{photos,title:local.title,agencyName,themeId:theme,visualId:presentationVisual,lang:presentationLang});
