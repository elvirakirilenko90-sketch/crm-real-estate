  ...
  const createSelection=async()=>{
    ...
    try{
      ...
      for(let index=0;index<items.length;index++){
        ...
        ctx.fillStyle=pal.ink;ctx.font='bold 31px Georgia, "Times New Roman", serif';ctx.fillText(txt.about,80,1185);
        const preview=String((selectionLang==="uk"?(p.description_uk||p.description):p.description)||txt.noDescription);
        ctx.fillStyle=pal.muted;ctx.font='22px Georgia, "Times New Roman", serif';const teaserLines=canvasTextLines(ctx,preview,1080).slice(0,7);let ty=1240;teaserLines.forEach(line=>{ctx.fillText(line,80,ty);ty+=34;});
        pages.push({bytes:pdfDataUrlToBytes(canvas.toDataURL("image/jpeg",.97)),width:1240,height:1754});
        addFullDescriptionPages(pages,{text:preview,title:p.title,agencyName,brand,themeId:theme,visualId:selectionVisual,lang:selectionLang});
        await addLuxuryGalleryPage(pages,{photos:extractPropertyPhotos(p),title:p.title,agencyName,themeId:theme,visualId:selectionVisual,lang:selectionLang});
      }
      ...
      ec.fillStyle=pal.accent;ec.font='bold 36px Georgia, "Times New Roman", serif';ec.fillText(managerDisplayName(currentProfile),92,720);
      ec.fillStyle=selectionVisual==="luxuryDark"?"#fff":pal.ink;ec.font='25px Arial';ec.fillText(currentProfile?.phone||currentProfile?.email||"",92,780);ec.font='30px Georgia, "Times New Roman", serif';ec.fillText(txt.thanks,92,1080);
