...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  const sourceCountryOptions=["Все",...Array.from(new Set([...analyticsSourceCatalog.map(x=>x.country),...values(analyticsListings,"country")])).sort((a,b)=>a.localeCompare(b,"ru"))];
  const catalogForCountry=analyticsSourceCatalog.filter(x=>sourceCountry==="Все"||geoEq(x.country,sourceCountry));
  const liveSourceNames=values(sourceCountry==="Все"?analyticsListings:analyticsListings.filter(x=>geoEq(x.country,sourceCountry)),"source_name");
  ...
  const sourceCountryMatches=x=>sourceCountry==="Все"||geoEq(x.country,sourceCountry)||geoEq(sourceCatalogEntry(x)?.country,sourceCountry);
