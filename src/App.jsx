...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  // Supabase/PostgREST может отдавать максимум 1000 строк за один запрос.
  // ВАЖНО: offset/range + сортировка по last_seen_at ненадёжны для постоянно обновляемого рынка:
  // во время синхронизации строки меняют last_seen_at и страницы могут повторяться/пропускаться.
  // Поэтому грузим ВСЮ таблицу стабильным курсором по уникальному id.
  const fetchAllMarketListings=async()=>{
    ...
    while(rows.length<maxRows){
      ...
      if(nextId===undefined||nextId===null||String(nextId)===String(lastId))break;
      lastId=nextId;
      if(batch.length<pageSize)break;
    }
    // Защита от случайных дублей при повторной загрузке.
    return Array.from(new Map(rows.map(x=>[String(x.id),x])).values());
  };
