...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  const cleanPresentationDescription=value=>decodePresentationText(value)
    ...
    .replace(/\b(?:ID|Код\s+(?:объекта|об'єкта))\s*[:#№-]?\s*[A-Za-zА-Яа-яІіЇїЄє0-9_-]+\b/giu," ")
    .replace(/\b(?:Источник|Джерело|Source)\s*:\s*[^\n|]{1,120}/giu," ")
    .replace(/\s+/g," ").trim();
  ...
  const enrichAnalyticsPresentationItem=async item=>{
    ...
    try{
      ...
      const all=(remotePhotos.length?remotePhotos:currentPhotos.map(x=>x.url)).filter(Boolean);
  ...
  const createSelection=async()=>{
    ...
    try{
      ...
      for(let index=0;index<items.length;index++){
        ...
        else{ctx.fillStyle="#eee8dc";ctx.fillRect(80,225,1080,690);ctx.fillStyle=pal.muted;ctx.font='bold 28px Arial';ctx.textAlign="center";ctx.fillText("Фото не передано рекламной площадкой",620,550);ctx.textAlign="left";}
        ...
        await addLuxuryGalleryPage(pages,{photos:extractPropertyPhotos(p),title:cleanTitle,agencyName,themeId:theme,visualId:selectionVisual,lang:selectionLang,drawImage:drawAnalyticsPresentationImage});
      }
