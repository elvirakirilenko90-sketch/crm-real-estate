...
async function openViberPresentation({phone,url,lang,agencyName,clientName,title,isSelection=false}) {
  ...
  try {
    ...
    // На телефонах сначала пробуем отправить именно PDF-файл через системное меню Share.
    // Так PDF приходит в Viber как вложение, а не только как ссылка.
    if (navigator.share && navigator.canShare) {
      try {
        const response = await fetch(publicUrl);
        if (response.ok) {
          const blob = await response.blob();
          const fileName = `${isSelection ? "property-selection" : "property-presentation"}.pdf`;
          const file = new File([blob], fileName, {type:"application/pdf"});
          if (navigator.canShare({files:[file]})) {
            await navigator.share({
              title: isSelection ? "VIP property selection" : (title || "VIP property presentation"),
              text: message,
              files: [file]
            });
            return true;
        ...
        console.warn("Viber PDF share fallback:", shareError);
      }
    }

    // Fallback для Desktop/браузеров без Web Share API:
    // официальный Viber Share deep-link переносит готовый текст и публичную PDF-ссылку
    // прямо в Viber. Пользователь выбирает нужный контакт и нажимает «Отправить».
    const viberForwardUrl = `viber://forward?text=${encodeURIComponent(message)}`;
    window.location.href = viberForwardUrl;
    return true;
  } catch (e) {
...
function Properties({properties,setProperties,onOpen,users,currentProfile,lang,agencyName,theme,leads,recordActivity}) {
  ...
  return <main className="screen">
    ...
    {selectionUrl && <div className="card selectionBar presentationSendPanel">
      ...
      <p className="muted sendIntro">Выбери канал отправки. WhatsApp откроется с готовым сообщением и ссылкой. На телефоне Viber постарается отправить сам PDF-файл; если устройство не поддерживает передачу файла, откроется Viber Share с готовой ссылкой.</p>
      <div className="grid4">
        ...
        <Button className="full sendViber" onClick={async()=>{if(await openViberPresentation({phone:selectionClientPhone,url:selectionUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true})) logPresentationSend("viber");}}>Viber · PDF</Button>
    ...
    <div className="card amber">
      ...
      {presentationUrl && <div style={{marginTop:12}}>
        <div className="card presentationSendPanel" style={{marginBottom:10}}>
          ...
          <p className="muted sendIntro">CRM подставит имя клиента и название текущего агентства. WhatsApp получит готовое сообщение со ссылкой. На телефоне Viber попробует передать сам PDF-файл; в остальных случаях откроется Viber Share с готовой ссылкой на презентацию.</p>
          <div className="grid4">
            ...
            <Button className="full sendViber" onClick={async()=>{if(await openViberPresentation({phone:presentationClientPhone,url:presentationUrl,lang:presentationLang,agencyName,clientName:presentationClient,title:local.title,isSelection:false})) logPropertyPresentationSend("viber");}}>Viber · PDF</Button>
