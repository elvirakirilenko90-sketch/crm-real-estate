...
const themeOptions = [
  {id:"classic", name:"Черный + золото", vars:{"--accent":"#d6a500","--accent2":"#f3c21b","--dark":"#070707","--soft":"#f5f2ea","--card":"#ffffff","--text":"#111827"}},
  {id:"whiteGold", name:"Белый + золото", vars:{"--accent":"#d8a800","--accent2":"#f2d15b","--dark":"#111827","--soft":"#fff8df","--card":"#ffffff","--text":"#111827"}},
  {id:"graphite", name:"Графит + янтарь", vars:{"--accent":"#f59e0b","--accent2":"#fbbf24","--dark":"#111827","--soft":"#f3f4f6","--card":"#ffffff","--text":"#111827"}},
  {id:"navy", name:"Синий + золото", vars:{"--accent":"#d4af37","--accent2":"#facc15","--dark":"#0f172a","--soft":"#eef2ff","--card":"#ffffff","--text":"#0f172a"}},
  {id:"green", name:"Изумруд + беж", vars:{"--accent":"#10b981","--accent2":"#34d399","--dark":"#052e2b","--soft":"#ecfdf5","--card":"#ffffff","--text":"#052e2b"}},
  {id:"ocean", name:"Океан + белый", vars:{"--accent":"#0ea5e9","--accent2":"#38bdf8","--dark":"#082f49","--soft":"#e0f2fe","--card":"#ffffff","--text":"#0c4a6e"}},
  {id:"royal", name:"Королевский синий", vars:{"--accent":"#2563eb","--accent2":"#60a5fa","--dark":"#111827","--soft":"#eff6ff","--card":"#ffffff","--text":"#111827"}},
  {id:"purple", name:"Фиолетовый premium", vars:{"--accent":"#8b5cf6","--accent2":"#c084fc","--dark":"#2e1065","--soft":"#f5f3ff","--card":"#ffffff","--text":"#2e1065"}},
  {id:"rose", name:"Пудровый + графит", vars:{"--accent":"#f472b6","--accent2":"#fb7185","--dark":"#3f3f46","--soft":"#fff1f2","--card":"#ffffff","--text":"#3f3f46"}},
  {id:"terracotta", name:"Терракота + крем", vars:{"--accent":"#ea580c","--accent2":"#fb923c","--dark":"#431407","--soft":"#ffedd5","--card":"#fffaf5","--text":"#431407"}},
  {id:"mint", name:"Мята + графит", vars:{"--accent":"#14b8a6","--accent2":"#5eead4","--dark":"#134e4a","--soft":"#ccfbf1","--card":"#ffffff","--text":"#134e4a"}},
  {id:"coffee", name:"Кофе + карамель", vars:{"--accent":"#b45309","--accent2":"#f59e0b","--dark":"#292524","--soft":"#fef3c7","--card":"#fffdf7","--text":"#292524"}},
  {id:"mono", name:"Минимализм black/white", vars:{"--accent":"#111827","--accent2":"#6b7280","--dark":"#000000","--soft":"#f9fafb","--card":"#ffffff","--text":"#111827"}},
  {id:"sunset", name:"Закат + индиго", vars:{"--accent":"#f97316","--accent2":"#fb7185","--dark":"#312e81","--soft":"#fff7ed","--card":"#ffffff","--text":"#312e81"}},
  {id:"lime", name:"Лайм + темный", vars:{"--accent":"#84cc16","--accent2":"#bef264","--dark":"#1a2e05","--soft":"#f7fee7","--card":"#ffffff","--text":"#1a2e05"}}
];
...
function themeStyle(themeId){
  ...
  return {
    ...found.vars,
    color: found.vars["--text"] || "#111827",
    background: `linear-gradient(135deg, ${found.vars["--soft"] || "#f5f2ea"}, #ffffff)`
  };
}

const reminderSoundOptions = [
  {id:"classic", ru:"Классический", uk:"Класичний", en:"Classic"},
  {id:"soft", ru:"Мягкий", uk:"М'який", en:"Soft"},
  {id:"double", ru:"Двойной", uk:"Подвійний", en:"Double"},
  {id:"bell", ru:"Колокольчик", uk:"Дзвіночок", en:"Bell"},
  {id:"success", ru:"Успех", uk:"Успіх", en:"Success"},
  {id:"urgent", ru:"Срочный", uk:"Терміновий", en:"Urgent"},
  {id:"iphone", ru:"iPhone стиль", uk:"iPhone стиль", en:"iPhone style"},
  {id:"silent", ru:"Без звука", uk:"Без звуку", en:"Silent"}
];

const uiTranslations = {
  uk: {
    "Лента":"Стрічка", "Клиенты":"Клієнти", "Календарь":"Календар", "Объекты":"Об'єкти", "Ещё":"Ще",
    "Лента новостей":"Стрічка новин", "Канбан и клиенты":"Канбан і клієнти", "Вторичка":"Вторинка", "Аналитика":"Аналітика", "Помощь менеджерам":"Допомога менеджерам", "Доступы":"Доступи", "Еще":"Ще",
    "Поиск по CRM...":"Пошук по CRM...", "Новая публикация":"Нова публікація", "Опубликовать":"Опублікувати", "Отправить":"Надіслати", "Написать комментарий...":"Написати коментар...", "Комментарий":"Коментар",
    "Сегодня":"Сьогодні", "Неделя":"Тиждень", "Месяц":"Місяць", "Выбрать период":"Обрати період", "Все":"Усі", "День":"День", "Год":"Рік",
    "Создать клиента":"Створити клієнта", "Следующий этап":"Наступний етап", "Ответственный":"Відповідальний", "Ответственный менеджер":"Відповідальний менеджер", "Без заметки":"Без нотатки", "Без имени":"Без імені",
    "Имя клиента":"Ім'я клієнта", "Телефон":"Телефон", "Источник":"Джерело", "Статус":"Статус", "Следующий контакт":"Наступний контакт", "Заметки менеджера":"Нотатки менеджера", "Сохранить клиента":"Зберегти клієнта",
    "История работы с клиентом":"Історія роботи з клієнтом", "Добавить комментарий в историю":"Додати коментар в історію", "Календарь клиента":"Календар клієнта", "Дата следующего касания":"Дата наступного контакту", "Время":"Час", "Название":"Назва", "Напомнить за":"Нагадати за", "Звук":"Звук", "Звук напоминания":"Звук нагадування", "Сохранить событие":"Зберегти подію",
    "Встреча":"Зустріч", "Звонок":"Дзвінок", "Показ":"Показ", "Задача":"Завдання", "Проверить звук":"Перевірити звук", "ОТКРЫТЬ ПОЛНУЮ КАРТОЧКУ КЛИЕНТА":"ВІДКРИТИ ПОВНУ КАРТКУ КЛІЄНТА",
    "Добавить объект":"Додати об'єкт", "Название объекта":"Назва об'єкта", "Тип":"Тип", "Район":"Район", "Цена":"Ціна", "Площадь":"Площа", "Этаж":"Поверх", "Описание":"Опис", "Сохранить объект":"Зберегти об'єкт", "Фото / видео объекта":"Фото / відео об'єкта", "Добавить фото/видео":"Додати фото/відео", "Отправить презентацию клиенту":"Надіслати презентацію клієнту", "Скачать PDF-презентацию":"Завантажити PDF-презентацію",
    "Язык интерфейса":"Мова інтерфейсу", "Цветовая гамма CRM":"Кольорова гама CRM", "Выйти из CRM":"Вийти з CRM", "Русский":"Російська", "Українська":"Українська", "English":"English",
    "Создать агентство недвижимости":"Створити агентство нерухомості", "Открыть доступ пользователю":"Відкрити доступ користувачу", "Создать доступ":"Створити доступ", "Удалить пользователя":"Видалити користувача", "Удалить агентство":"Видалити агентство", "Редактировать":"Редагувати",
    "Классический":"Класичний", "Мягкий":"М'який", "Двойной":"Подвійний", "Колокольчик":"Дзвіночок", "Успех":"Успіх", "Срочный":"Терміновий", "Без звука":"Без звуку", "Напоминание":"Нагадування"
  },
  en: {
    "Лента":"Feed", "Клиенты":"Clients", "Календарь":"Calendar", "Объекты":"Objects", "Ещё":"More",
    "Лента новостей":"News feed", "Канбан и клиенты":"Kanban and clients", "Вторичка":"Resale", "Аналитика":"Analytics", "Помощь менеджерам":"Manager help", "Доступы":"Access", "Еще":"More",
    "Поиск по CRM...":"Search CRM...", "Новая публикация":"New post", "Опубликовать":"Publish", "Отправить":"Send", "Написать комментарий...":"Write a comment...", "Комментарий":"Comment",
    "Сегодня":"Today", "Неделя":"Week", "Месяц":"Month", "Выбрать период":"Choose period", "Все":"All", "День":"Day", "Год":"Year",
    "Создать клиента":"Create client", "Следующий этап":"Next stage", "Ответственный":"Responsible", "Ответственный менеджер":"Responsible manager", "Без заметки":"No note", "Без имени":"No name",
    "Имя клиента":"Client name", "Телефон":"Phone", "Источник":"Source", "Статус":"Status", "Следующий контакт":"Next contact", "Заметки менеджера":"Manager notes", "Сохранить клиента":"Save client",
    "История работы с клиентом":"Client work history", "Добавить комментарий в историю":"Add comment to history", "Календарь клиента":"Client calendar", "Дата следующего касания":"Next contact date", "Время":"Time", "Название":"Title", "Напомнить за":"Remind before", "Звук":"Sound", "Звук напоминания":"Reminder sound", "Сохранить событие":"Save event",
    "Встреча":"Meeting", "Звонок":"Call", "Показ":"Showing", "Задача":"Task", "Проверить звук":"Test sound", "ОТКРЫТЬ ПОЛНУЮ КАРТОЧКУ КЛИЕНТА":"OPEN FULL CLIENT CARD",
    "Добавить объект":"Add object", "Название объекта":"Object title", "Тип":"Type", "Район":"District", "Цена":"Price", "Площадь":"Area", "Этаж":"Floor", "Описание":"Description", "Сохранить объект":"Save object", "Фото / видео объекта":"Object photo/video", "Добавить фото/видео":"Add photo/video", "Отправить презентацию клиенту":"Send presentation to client", "Скачать PDF-презентацию":"Download PDF presentation",
    "Язык интерфейса":"Interface language", "Цветовая гамма CRM":"CRM color theme", "Выйти из CRM":"Log out", "Русский":"Russian", "Українська":"Ukrainian", "English":"English",
    "Создать агентство недвижимости":"Create real estate agency", "Открыть доступ пользователю":"Open user access", "Создать доступ":"Create access", "Удалить пользователя":"Delete user", "Удалить агентство":"Delete agency", "Редактировать":"Edit",
    "Классический":"Classic", "Мягкий":"Soft", "Двойной":"Double", "Колокольчик":"Bell", "Успех":"Success", "Срочный":"Urgent", "Без звука":"Silent", "Напоминание":"Reminder"
  }
};

function trText(text, lang="ru"){
...
function translateStaticDom(lang){
  ...
  if (!root) return;
  const dict = uiTranslations[lang] || {};
  const reverse = Object.values(uiTranslations).reduce((acc, d) => {
    Object.entries(d).forEach(([ru, val]) => { acc[val] = ru; });
    return acc;
  }, {});
  ...
  nodes.forEach(node => {
    const original = node.__crmOriginalText || reverse[node.nodeValue.trim()] || node.nodeValue.trim();
    node.__crmOriginalText = original;
    let next = dict[original] || original;
    if (node.nodeValue.startsWith(" ")) next = " " + next;
    if (node.nodeValue.endsWith(" ")) next = next + " ";
    node.nodeValue = next;
  });
  document.querySelectorAll("input[placeholder], textarea[placeholder]").forEach(el => {
    const original = el.dataset.crmOriginalPlaceholder || reverse[el.placeholder] || el.placeholder;
    el.dataset.crmOriginalPlaceholder = original;
    el.placeholder = dict[original] || original;
  });
}

function SoundOptions({value,onChange,lang="ru"}){
  return <select className="input" value={value || "classic"} onChange={e=>onChange(e.target.value)}>
    {reminderSoundOptions.map(s => <option key={s.id} value={s.id}>{s[lang] || s.ru}</option>)}
  </select>;
}
...
function playReminderSound(type="classic"){
  ...
  try{
    ...
    const toneMap = {
      ...
      urgent:[1100, 740, 1100, 740, 1100],
      iphone:[784, 988, 1175, 988]
    };
    const tones = toneMap[type] || toneMap.classic;
    ...
    if (navigator.vibrate && type === "urgent") navigator.vibrate([120,80,120]);
...
function inRange(date, from, to){
  if (!date) return true;
  const t = new Date(isoDate(date)).getTime();
...
function Sidebar({page,setPage,role,lang="ru"}) {
  ...
  return <aside className="sidebar"><div className="logo"><div>♛</div><section><b>CRM Real Estate</b><span>Premium SaaS</span></section></div><div className="roleBox">{role}</div>{items.map(([id,label,icon])=><button key={id} onClick={()=>setPage(id)} className={cn("sideBtn", page===id && "active")}><span>{icon}</span>{trText(label, lang)}</button>)}</aside>;
}

function Top({page,setPage,agencyName,search,setSearch,hideSearch=false,lang="ru"}) {
  const tr = {
    ru:{feed:"Лента новостей",clients:"Клиенты",calendar:"Календарь",properties:"Вторичка",analytics:"Аналитика",help:"Помощь менеджерам",access:"Доступы",more:"Еще",search:"Поиск по CRM..."},
    uk:{feed:"Стрічка новин",clients:"Клієнти",calendar:"Календар",properties:"Вторинка",analytics:"Аналітика",help:"Допомога менеджерам",access:"Доступи",more:"Ще",search:"Пошук по CRM..."},
    en:{feed:"News feed",clients:"Clients",calendar:"Calendar",properties:"Properties",analytics:"Analytics",help:"Manager help",access:"Access",more:"More",search:"Search CRM..."}
  };
  const t = tr[lang] || tr.ru;
  return <div className={cn("top", hideSearch && "topCompact")}>
    <div className="topLine"><div><p>♛ CRM Real Estate</p><h1>{t[page]}</h1></div><button className="agencyTop" onClick={()=>setPage("more")}>{agencyName || "Моё агентство"}</button></div>
    {!hideSearch && <div className="search">🔎 <input value={search || ""} onChange={e=>setSearch&&setSearch(e.target.value)} placeholder={t.search} /></div>}
  </div>;
}

function Bottom({page,setPage,lang="ru"}) {
  const items = [["feed","Лента","⌂"],["clients","Клиенты","👥"],["calendar","Календарь","📅"],["properties","Объекты","🏢"],["more","Ещё","☰"]];
  return <nav className="bottom">{items.map(([id,label,icon])=><button key={id} className={page===id ? "active" : ""} onClick={()=>setPage(id)}><span>{icon}</span>{trText(label, lang)}</button>)}</nav>;
}

function GlobalSearchResults({search,setSearch,leads,properties,posts,events,setPage,onOpenLead,onOpenProperty}) {
  const q = String(search || "").trim().toLowerCase();
  if (!q) return null;
  const hit = (v) => String(v || "").toLowerCase().includes(q);
  const leadHits = (leads || []).filter(l => hit(l.name)||hit(l.phone)||hit(l.notes)||hit(l.source)||hit(l.status)||hit(l.manager)).slice(0,8);
  const propHits = (properties || []).filter(p => hit(p.title)||hit(p.district)||hit(p.type)||hit(p.description)||hit(p.owner)||hit(p.price)).slice(0,8);
  const postHits = (posts || []).filter(p => hit(p.text)||hit(p.file)||hit(p.kind)||hit(p.author)).slice(0,8);
  const eventHits = (events || []).filter(e => hit(e.title)||hit(e.client)||hit(e.type)||hit(e.date)||hit(e.time)).slice(0,8);
  const none = !leadHits.length && !propHits.length && !postHits.length && !eventHits.length;
  return <div className="searchPanel card">
    <div className="row"><h3>Результаты поиска</h3><button className="icon" onClick={()=>setSearch("")}>×</button></div>
    {none && <p className="muted">Ничего не найдено.</p>}
    {!!leadHits.length && <section><h4>Клиенты</h4>{leadHits.map(l=><button className="searchItem" key={l.id} onClick={()=>{setPage("clients"); onOpenLead(l); setSearch("");}}><b>{l.name || "Без имени"}</b><span>{l.phone} · {l.status} · {l.manager || "без менеджера"}</span></button>)}</section>}
    {!!propHits.length && <section><h4>Вторичка</h4>{propHits.map(p=><button className="searchItem" key={p.id} onClick={()=>{setPage("properties"); onOpenProperty(p); setSearch("");}}><b>{p.title || "Без названия"}</b><span>{p.district} · {p.type} · {money(p.price)}</span></button>)}</section>}
    {!!postHits.length && <section><h4>Лента</h4>{postHits.map(p=><button className="searchItem" key={p.id} onClick={()=>{setPage("feed"); setSearch("");}}><b>{p.text || p.kind}</b><span>{p.date}</span></button>)}</section>}
    {!!eventHits.length && <section><h4>Календарь</h4>{eventHits.map(e=><button className="searchItem" key={e.id} onClick={()=>{setPage("calendar"); setSearch("");}}><b>{e.title}</b><span>{e.date} · {e.time} · {e.client}</span></button>)}</section>}
  </div>;
}

function CRMRuntimeStyles(){
  return <style>{`
    .layout{background:var(--soft,#f5f2ea)!important;color:var(--text,#111827)!important;}
    .mainWrap,.screen{background:transparent!important;}
    .card,.sheet,.lead,.post{background:var(--card,#ffffff)!important;color:var(--text,#111827)!important;}
    .sidebar,.top,.card.dark{background:var(--dark,#070707)!important;}
    .card.dark,.sidebar,.top{color:#fff!important;}
    .btn.primary,.bottom .active,.sideBtn.active,.chip.active,.badge.gold{background:var(--accent,#d6a500)!important;color:#0b0b0b!important;}
    .agencyTop{background:var(--accent,#d6a500)!important;color:#0b0b0b!important;}
    .btn.soft,.chip{background:color-mix(in srgb, var(--accent,#d6a500) 12%, #fff)!important;}
    .input, input, textarea, select{border-color:color-mix(in srgb, var(--accent,#d6a500) 22%, #e5e7eb)!important;}
    .searchPanel{position:sticky;top:10px;z-index:20;margin:12px 0;border:2px solid var(--accent,#d6a500)}
    .searchItem{width:100%;text-align:left;border:0;background:#f7f7f7;border-radius:16px;padding:12px 14px;margin:6px 0;display:flex;flex-direction:column;gap:4px;cursor:pointer}
    .searchItem span{color:#6b7280;font-size:13px}
    .feedFilters{display:flex;flex-wrap:wrap;gap:10px;align-items:center}
    .feedFilters .input{max-width:180px}
    @media (max-width: 760px){
      .topCompact{padding:14px 16px!important;border-radius:0 0 24px 24px!important;}
      .topCompact .search{display:none!important;}
      .calendarMobile .card.dark{display:none!important;}
      .calendarMobile .calendarToolbar{position:sticky;top:0;z-index:12;border-radius:0 0 22px 22px!important;padding:12px!important;}
      .calendarMobile .calendarToolbar .row{display:block!important;}
      .calendarMobile .calendarToolbar .grid2,.calendarMobile .calendarToolbar .grid4{display:grid!important;grid-template-columns:1fr 1fr!important;max-width:100%!important;gap:8px!important;}
      .calendarMobile .calgrid{display:grid!important;grid-template-columns:repeat(7, minmax(36px,1fr))!important;gap:4px!important;}
      .calendarMobile .week{grid-template-columns:repeat(7, 1fr)!important;font-size:11px!important;}
      .calendarMobile .day{min-height:48px!important;border-radius:12px!important;padding:5px!important;font-size:12px!important;}
      .calendarMobile .day .mini{display:block;max-width:7px;height:7px;padding:0;border-radius:999px;overflow:hidden;color:transparent;background:var(--accent,#d6a500);margin-top:3px;}
      .calendarMobile .grid2{grid-template-columns:1fr!important;}
      .calendarMobile .event{padding:14px!important;border-radius:18px!important;}
      .calendarMobile .monthBox,.calendarMobile .weekRow{min-height:auto!important;padding:12px!important;}
      .calendarMobile .btn{min-height:44px!important;font-size:14px!important;}
      .bottom button{font-size:12px!important;min-width:auto!important;}
    }
  `}</style>;
}
...
function Feed({posts,setPosts,role}) {
  ...
  const filteredPosts = useMemo(() => {
    ...
    let from = dateFrom;
    let to = dateTo;
    if (datePreset === "day") { from = todayIso; to = todayIso; }
    if (datePreset === "week") { const d = new Date(now); d.setDate(now.getDate()-6); from = isoDate(d); to = todayIso; }
    if (datePreset === "month") { from = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,"0")}-01`; to = todayIso; }
    if (datePreset === "all") { from = ""; to = ""; }
    return (posts || []).filter(p => inRange(postToDate(p), from, to));
  }, [posts, datePreset, dateFrom, dateTo, todayIso]);
  ...
  async function insertComment(postId, text) {
    const variants = [
      {news_id: Number(postId), content: text, author_name: role || "CRM"},
      {news_id: Number(postId), text, author: role || "CRM"},
      {post_id: Number(postId), text, author: role || "CRM"},
      {entity_type: "news", entity_id: String(postId), content: text, author_name: role || "CRM"},
      {entity_type: "news", entity_id: String(postId), text, author: role || "CRM"}
    ];
  ...
  return <main className="screen"><div className="feedList">
    ...
    <div className="card feedFilters">
      <b>Фильтр по датам:</b>
      <Button variant={datePreset==="all"?"primary":"soft"} onClick={()=>setDatePreset("all")}>Все</Button>
      <Button variant={datePreset==="day"?"primary":"soft"} onClick={()=>setDatePreset("day")}>День</Button>
      <Button variant={datePreset==="week"?"primary":"soft"} onClick={()=>setDatePreset("week")}>Неделя</Button>
      <Button variant={datePreset==="month"?"primary":"soft"} onClick={()=>setDatePreset("month")}>Месяц</Button>
      <Button variant={datePreset==="range"?"primary":"soft"} onClick={()=>setDatePreset("range")}>Период</Button>
      {datePreset==="range" && <>
        <input className="input" type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} />
        <input className="input" type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)} />
      </>}
      <span className="muted">Показано: {filteredPosts.length}</span>
    </div>
    ...
    {filteredPosts.map(post=><article className="card post" key={post.id}>
      ...
      <div className="comments">
        {(post.comments || []).map((c,i)=><p key={i}><b>Комментарий:</b> {typeof c === "string" ? c : (c.text || c.content || "")}</p>)}
        <div className="row">
...
function LeadModal({lead,setLeads,setEvents,setHelp,onClose,role,users,currentProfile}) {
  ...
  const lang = currentProfile?.language || "ru";
  ...
  const addEvent = () => {
    setEvents(prev=>[{id:"EV-"+Date.now(),leadId:local.id,client:local.name,type:event.type,title:event.title || `${event.type} — ${local.name}`,date:event.date,time:event.time,reminder:Number(event.reminder)||60,sound:event.sound||"classic",notified:false},...prev]);
  ...
  return <Modal onClose={onClose} wide>
    ...
    <div className="card inner" style={{border:"1px solid rgba(17,24,39,.08)", boxShadow:"0 16px 36px rgba(17,24,39,.07)", background:"linear-gradient(180deg,#ffffff,#fafafa)", overflow:"hidden"}}>
      <div className="row" style={{alignItems:"center", marginBottom:12}}>
        <section>
          <h3 style={{marginBottom:4}}>{trText("Календарь клиента", lang)}</h3>
          <p className="muted" style={{margin:0}}>{trText("Выбери тип события, дату, время и напоминание", lang)}</p>
        </section>
      </div>
      ...
      <div style={{marginTop:16, padding:16, borderRadius:26, background:"rgba(17,24,39,.035)", border:"1px solid rgba(17,24,39,.07)"}}>
        <div className="grid3" style={{alignItems:"end", gap:12}}>
          <input className="input" type="date" value={event.date} onChange={e=>setEvent({...event,date:e.target.value})} style={{borderRadius:18, fontWeight:800}}/>
          <input className="input" type="time" value={event.time} onChange={e=>setEvent({...event,time:e.target.value})} style={{borderRadius:18, fontWeight:800}}/>
          <input className="input" placeholder={trText("Название", lang)} value={event.title} onChange={e=>setEvent({...event,title:e.target.value})} style={{borderRadius:18, fontWeight:800}}/>
        </div>
      </div>
      <div style={{marginTop:14, padding:16, borderRadius:26, background:"rgba(214,165,0,.08)", border:"1px solid rgba(214,165,0,.18)"}}>
        <div className="grid2" style={{alignItems:"end", gap:12}}>
          <Field label={trText("Напомнить за", lang)}><select className="input" value={event.reminder} onChange={e=>setEvent({...event,reminder:e.target.value})} style={{borderRadius:18, fontWeight:800}}><option value="5">5 минут</option><option value="15">15 минут</option><option value="30">30 минут</option><option value="60">1 час</option><option value="120">2 часа</option><option value="1440">1 день</option></select></Field>
          <Field label={trText("Звук", lang)}><SoundOptions value={event.sound} onChange={sound=>setEvent({...event,sound})} lang={lang}/></Field>
        </div>
      </div>
      <Button className="full" onClick={addEvent} style={{marginTop:14}}>{trText("Сохранить событие", lang)}</Button>
    </div>
...
function Calendar({events,setEvents,leads,setLeads,onOpenLead}) {
  ...
  const create = (date, time="12:00") => setOpen({leadId:leads[0]?.id || "",type:"Звонок",title:"",date,time,reminder:60,sound:"classic",notified:false});
...
Дата следующего касания: ${ev.date} ${ev.time}`);
  ...
  return <main className="screen calendarMobile" style={{maxWidth:"100%"}}>
...
function PropertyModal({property,setProperties,onClose,users,currentProfile,role}) {
  ...
  async function generateClientPresentation(){
    ...
    alert("PDF-презентация создана. Ссылка появилась под кнопкой.");
  }
  ...
  return <Modal onClose={onClose} wide>
    ...
    <div className="card amber">
      ...
      {presentationUrl && <div className="grid2" style={{marginTop:12}}>
        <a className="btn green full" href={presentationUrl} download={`${(local.title || "object").replace(/[^a-zA-Zа-яА-Я0-9_-]+/g,"_")}_presentation.pdf`}>Скачать PDF-презентацию</a>
...
function More({role,setRole,setPage,theme,setTheme,lang,setLang,leads,setLeads,users,currentProfile,onLogout}) {
  ...
  return <main className="screen">
    <div className="card">
      <h3>{trText("Язык интерфейса", lang)}</h3>
      <div className="grid3"><Button variant={lang==="ru"?"primary":"soft"} onClick={()=>setLang("ru")}>{trText("Русский", lang)}</Button><Button variant={lang==="uk"?"primary":"soft"} onClick={()=>setLang("uk")}>{trText("Українська", lang)}</Button><Button variant={lang==="en"?"primary":"soft"} onClick={()=>setLang("en")}>{trText("English", lang)}</Button></div><p className="muted">Язык сохраняется индивидуально для пользователя и сразу меняет основные разделы CRM: меню, шапку, поиск, календарь и новые элементы интерфейса.</p>
    </div>

    <div className="card">
      <h3>{trText("Цветовая гамма CRM", lang)}</h3>
    ...
    <div className="card">
      <h3>{trText("Звук напоминания", lang)}</h3>
      <p className="muted">Выбери звук напоминаний и проверь его сразу. Сам звук сохраняется в каждом событии календаря.</p>
      <div className="grid3">
        {reminderSoundOptions.map(snd => <Button key={snd.id} variant="soft" onClick={()=>playReminderSound(snd.id)}>{snd[lang] || snd.ru}</Button>)}
      </div>
    </div>
...
export default function App(){
  ...
  function postFromDb(row, mediaRows = [], commentRows = []){
    ...
    const comments = (commentRows || [])
      ...
      .map(c => c.content || c.text || c.comment || "");
  ...
  useEffect(() => {
    const id = setTimeout(() => translateStaticDom(lang), 80);
    return () => clearTimeout(id);
  }, [lang, page, leads.length, posts.length, properties.length, events.length, theme]);

  useEffect(() => {
    if (!currentProfile?.id) return;
    supabase.from("users").update({theme, language: lang}).eq("id", currentProfile.id).then(()=>{});
  }, [theme, lang, currentProfile?.id]);
  ...
  useEffect(() => {
    ...
    const checkReminders = () => {
      ...
      const updated = events.map(ev => {
        if (!ev.date || !ev.time || ev.notified) return ev;
        const start = new Date(`${ev.date}T${ev.time}:00`).getTime();
        if (!Number.isFinite(start)) return ev;
        const remindMs = (Number(ev.reminder) || 60) * 60 * 1000;
        const due = nowMs >= start - remindMs && nowMs <= start + 2 * 60 * 1000;
        const allowed = isTech || visibleLeads.some(l => String(l.id) === String(ev.leadId));
        if (due && allowed) {
          playReminderSound(ev.sound || "classic");
          setTimeout(() => alert(`${trText("Напоминание", lang) || "Напоминание"}: ${ev.title || ev.type}\n${trText("Дата следующего касания", lang)}: ${ev.date} ${ev.time}`), 150);
          changed = true;
    ...
    return () => clearInterval(timer);
  }, [events, session, currentProfile?.id, lang, visibleLeads.length, isTech]);
  ...
  return <div className="layout" style={themeStyle(theme)}>
    ...
    <div className="mainWrap">
      <main className="main">
        <Top page={page} setPage={setPage} agencyName={agencyName} search={search} setSearch={setSearch} hideSearch={page==="calendar"} lang={lang}/>
        <GlobalSearchResults search={search} setSearch={setSearch} leads={visibleLeads} properties={properties} posts={posts} events={visibleEvents} setPage={setPage} onOpenLead={setLead} onOpenProperty={setProperty}/>
        {page==="feed"&&<Feed posts={posts} setPosts={setPosts} role={role}/>} 
        ...
        {page==="calendar"&&<Calendar events={visibleEvents} setEvents={setEvents} leads={visibleLeads} setLeads={setLeads} onOpenLead={setLead}/>} 
