...
const analyticsSourceCatalog = [
  {country:"Украина",name:"DIM.RIA",aliases:["DOM.RIA","DIM.RIA","dom.ria.com"]},
  {country:"Украина",name:"OLX",aliases:["OLX","OLX.ua"]},
  {country:"Украина",name:"RIELTOR.UA",aliases:["RIELTOR.UA","Rieltor.ua","RIELTOR"]},
  {country:"Украина",name:"ЛУН",aliases:["ЛУН","LUN","lun.ua"]},
  {country:"Украина",name:"REM.ua",aliases:["REM","REM.ua","rem.ua"]},
  {country:"Украина",name:"Telegram",aliases:["Telegram","Телеграм","Телеграмм"]},
  {country:"Украина",name:"Facebook Marketplace",aliases:["Facebook","Facebook Marketplace"]},
  {country:"ОАЭ",name:"Property Finder",aliases:["Property Finder","propertyfinder.ae"]},
  {country:"ОАЭ",name:"Bayut",aliases:["Bayut","bayut.com"]},
  {country:"ОАЭ",name:"Dubizzle",aliases:["Dubizzle","dubizzle"]},
  {country:"Испания",name:"idealista",aliases:["idealista"]},{country:"Португалия",name:"idealista",aliases:["idealista"]},{country:"Италия",name:"idealista",aliases:["idealista"]},
  {country:"Великобритания",name:"Rightmove",aliases:["Rightmove"]},{country:"Великобритания",name:"Zoopla",aliases:["Zoopla"]},
  {country:"Германия",name:"ImmoScout24",aliases:["ImmoScout24","ImmobilienScout24"]},{country:"Франция",name:"SeLoger",aliases:["SeLoger"]},
  {country:"Италия",name:"Immobiliare.it",aliases:["Immobiliare.it","Immobiliare"]},{country:"Нидерланды",name:"Funda",aliases:["Funda"]},
  {country:"Таиланд",name:"DDproperty",aliases:["DDproperty"]},{country:"Таиланд",name:"FazWaz",aliases:["FazWaz"]},
  {country:"Индонезия / Бали",name:"Rumah123",aliases:["Rumah123"]},{country:"Индонезия / Бали",name:"OLX Indonesia",aliases:["OLX Indonesia","OLX.co.id"]},
  {country:"Сингапур",name:"PropertyGuru",aliases:["PropertyGuru"]},{country:"Малайзия",name:"PropertyGuru",aliases:["PropertyGuru"]},
  {country:"США",name:"Zillow",aliases:["Zillow"]},{country:"США",name:"Realtor.com",aliases:["Realtor.com"]},
  {country:"Австралия",name:"realestate.com.au",aliases:["realestate.com.au","REA"]}
];
const officialCityDistricts = {
  "Украина|Одесса":["Киевский","Пересыпский","Приморский","Хаджибейский"],
  "Україна|Одеса":["Київський","Пересипський","Приморський","Хаджибейський"],
...
function Analytics({leads,properties,events,role,users,activities,currentProfile,onOpenProperty,agencyName,theme}) {
  ...
  const countries=["Все",...values(combinedListings,"country")];
  const geoCountry=country==="Все"?combinedListings:combinedListings.filter(x=>String(x.country||"")===country);
  const regions=["Все",...values(geoCountry,"region")];
  const geoRegion=region==="Все"?geoCountry:geoCountry.filter(x=>String(x.region||"")===region);
  const cities=["Все",...values(geoRegion,"city")];
  const geoCity=city==="Все"?geoRegion:geoRegion.filter(x=>String(x.city||"")===city);
  const officialKey=`${country}|${city}`;
  const officialDistricts=officialCityDistricts[officialKey]||[];
  const districtsMarket=["Все",...Array.from(new Set([...officialDistricts,...values(geoCity,"district")])).sort((a,b)=>a.localeCompare(b,"ru"))];
  const complexes=["Все",...values(geoCity,"complex_name")];
  const sourceCountryOptions=["Все",...Array.from(new Set(analyticsSourceCatalog.map(x=>x.country)))];
  const catalogForCountry=analyticsSourceCatalog.filter(x=>sourceCountry==="Все"||x.country===sourceCountry);
  const liveSourceNames=values(combinedListings,"source_name");
  const sourceOptions=Array.from(new Set([...catalogForCountry.map(x=>x.name),...liveSourceNames])).sort((a,b)=>a.localeCompare(b,"ru"));
  const sourceMatches=(x,name)=>{const raw=String(x.source_name||"").toLowerCase();const entry=analyticsSourceCatalog.find(s=>s.name===name);return raw===name.toLowerCase()||(entry?.aliases||[]).some(a=>raw.includes(String(a).toLowerCase()));};
  const filtered=combinedListings.filter(x=>{
    const num=v=>Number(v||0);
    return inMarketPeriod(x)
      &&(country==="Все"||String(x.country||"")===country)
      &&(region==="Все"||String(x.region||"")===region)
      &&(city==="Все"||String(x.city||"")===city)
      &&(districtFilter==="Все"||String(x.district||"")===districtFilter)
  ...
  return <main className="screen">
    <div className="card dark" style={{padding:26,borderRadius:24}}>
      <div className="row" style={{alignItems:"center",gap:18}}>
        <section><div style={{fontSize:13,fontWeight:900,letterSpacing:1.8,opacity:.7}}>MARKET INTELLIGENCE</div><h2 style={{fontSize:32,margin:"7px 0"}}>📊 Аналитика рынка</h2><p style={{margin:0}}>DIM.RIA • OLX • RIELTOR.UA • ЛУН • REM • Telegram • международные порталы • CRM</p></section>
    ...
    <div className="card" style={{padding:22,borderRadius:24}}>
      ...
      <div className="grid4">
        <div style={filterBox}><Field label="Страна"><select className="input" value={country} onChange={e=>{setCountry(e.target.value);setRegion("Все");setCity("Все");setDistrictFilter("Все");setComplexFilter("Все");}}>{countries.map(x=><option key={x}>{x}</option>)}</select></Field></div>
        ...
        <div style={filterBox}><Field label="Район"><select className="input" value={districtFilter} onChange={e=>setDistrictFilter(e.target.value)}>{districtsMarket.map(x=><option key={x}>{x}</option>)}</select></Field></div>
        ...
        <div style={{...filterBox,gridColumn:"span 2"}}><Field label="Источники · можно выбрать несколько"><div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}><select className="input" style={{maxWidth:260}} value={sourceCountry} onChange={e=>{setSourceCountry(e.target.value);setSelectedSources([]);}}>{sourceCountryOptions.map(x=><option key={x}>{x}</option>)}</select>{selectedSources.length>0&&<button className="chip" onClick={()=>setSelectedSources([])}>Очистить ({selectedSources.length})</button>}</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(155px,1fr))",gap:8,maxHeight:190,overflow:"auto",padding:8,border:"1px solid rgba(148,163,184,.2)",borderRadius:14}}>{sourceOptions.map(name=><label key={name} className={cn("chip",selectedSources.includes(name)&&"active")} style={{cursor:"pointer",justifyContent:"flex-start"}}><input type="checkbox" checked={selectedSources.includes(name)} onChange={()=>setSelectedSources(v=>v.includes(name)?v.filter(x=>x!==name):[...v,name])}/> {name}</label>)}</div></Field></div>
