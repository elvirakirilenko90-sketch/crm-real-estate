...
async function addLuxuryGalleryPage(pages,{photos,title,agencyName,themeId,visualId,lang,drawImage=drawRemoteImage,compact=false}){
  ...
  for(let start=0; start<photos.length; start+=4){
    ...
    ctx.fillStyle=pal.accent;ctx.fillRect(80,135,1080,3);
    if(!compact){ctx.fillStyle=pal.muted;ctx.font='21px Arial';ctx.fillText(`${title||""} · ${start+1}-${Math.min(start+4,photos.length)} / ${photos.length}`,80,178);}
    const standardBoxes=[[80,240,700,650],[805,240,355,310],[805,580,355,310],[80,920,1080,650]];
    const compactBoxes=chunk.length===1
      ?[[80,180,1080,1390]]
      :chunk.length===2
        ?[[80,180,1080,675],[80,885,1080,685]]
        :chunk.length===3
          ?[[80,180,700,1390],[805,180,355,675],[805,885,355,685]]
          :[[80,180,700,675],[805,180,355,325],[805,535,355,320],[80,885,1080,685]];
...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  const cleanPresentationTitle=value=>cleanPresentationDescription(value)
    .replace(/[‼❗🔥⭐🌟💥✅🔑🏠🏡]+/gu," ").replace(/!{2,}/g,"!").replace(/\s+/g," ").trim();
  ...
  const enrichAnalyticsPresentationItem=async item=>{
    ...
    try{
      ...
      const detailDescription=cleanPresentationDescription(data.description||"");
      const currentDescription=cleanPresentationDescription(item.description_uk||item.description||"");
      ...
      const fullTitle=remoteTitle||currentTitle;
      ...
      const complexFromText=(fullTitle+" "+fullDescription).match(/(?:^|\s)ЖК\s*[«\"']?([A-Za-zА-Яа-яЁёІіЇїЄє0-9'’ -]{2,70}?)(?=\s+(?:вид|район|в\s|на\s|Одесс|Одес|цена|ціна|площад|площа)|[»,.;|]|$)/iu)?.[1]?.trim()||"";
      return {...item,title:fullTitle||"Объект недвижимости",description:fullDescription,description_uk:fullDescription,media,
        ...
        complex_name:d.complex_name||item.complex_name||complexFromText||"",type:d.property_type||item.type||"",condition:d.condition||item.condition||"",layout:d.layout||"",kitchen_area:d.kitchen_area||"",wall_type:d.wall_type||"",deal_type:d.deal_type||""};
    }catch(e){console.warn("Analytics presentation detail fallback:",e);return {...item,title:cleanPresentationTitle(item.title),description:cleanPresentationDescription(item.description)};}
  };
  const drawAnalyticsPresentationImage=async(ctx,url,x,y,w,h,fit="cover")=>{
    if(!url)return false;
    if(/^data:image\//i.test(url)||/^blob:/i.test(url))return await drawRemoteImage(ctx,url,x,y,w,h,fit);
    let objectUrl="";
    try{
  ...
  const analyticsBlobToDataUrl=blob=>new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result||""));reader.onerror=()=>reject(reader.error||new Error("image read failed"));reader.readAsDataURL(blob);});
  const filterRenderableAnalyticsPhotos=async photos=>{
    ...
    for(let start=0;start<list.length;start+=4){
      const batch=await Promise.all(list.slice(start,start+4).map(async photo=>{
        try{
          const {data,error}=await supabase.functions.invoke("analytics-live-search",{body:{action:"presentation_image",url:photo.url}});
          if(error)throw error;
          const blob=data instanceof Blob?data:data instanceof ArrayBuffer?new Blob([data]):null;
          if(!blob)throw new Error("image proxy returned no image");
          const dataUrl=await analyticsBlobToDataUrl(blob),probe=document.createElement("canvas");probe.width=8;probe.height=8;
          return (await drawRemoteImage(probe.getContext("2d"),dataUrl,0,0,8,8,"cover"))?{...photo,original_url:photo.url,url:dataUrl}:null;
        }catch(e){console.warn("Analytics presentation skipped unavailable photo:",e);return null;}
      }));
  ...
  const createSelection=async()=>{
    ...
    try{
      ...
      for(const item of selectionItems){
        ...
        items.push({...enriched,media:validPhotos});
      }
      ...
      for(let index=0;index<items.length;index++){
        ...
        ctx.fillStyle=pal.ink;let titleFont=36;ctx.font=`bold ${titleFont}px Georgia, "Times New Roman", serif`;
        while(canvasTextLines(ctx,cleanTitle,900).length>4&&titleFont>24){titleFont-=2;ctx.font=`bold ${titleFont}px Georgia, "Times New Roman", serif`;}
        const titleBottom=pdfDrawWrapped(ctx,cleanTitle,220,65,900,titleFont+5);
        const heroY=Math.max(225,titleBottom+60),heroH=Math.max(180,915-heroY);
        ...
        ctx.fillStyle=pal.muted;ctx.font='21px Georgia, "Times New Roman", serif';const allDescriptionLines=canvasTextLines(ctx,preview,1080);
        if(allDescriptionLines.length>8){
          ctx.fillStyle=pal.accent;ctx.font='bold 20px Arial';ctx.fillText(selectionLang==="uk"?"ПОВНИЙ ОПИС — НА НАСТУПНІЙ СТОРІНЦІ":selectionLang==="en"?"FULL DESCRIPTION — ON THE NEXT PAGE":"ПОЛНОЕ ОПИСАНИЕ — НА СЛЕДУЮЩЕЙ СТРАНИЦЕ",80,1300);
        }else{
          ctx.fillStyle=pal.ink;ctx.font='bold 31px Georgia, "Times New Roman", serif';ctx.fillText(txt.about,80,1235);
          ctx.fillStyle=pal.muted;ctx.font='21px Georgia, "Times New Roman", serif';let ty=1290;allDescriptionLines.slice(0,8).forEach(line=>{ctx.fillText(line,80,ty);ty+=33;});
        }
        pages.push({bytes:pdfDataUrlToBytes(canvas.toDataURL("image/jpeg",.97)),width:1240,height:1754});
