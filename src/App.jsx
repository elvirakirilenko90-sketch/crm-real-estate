...
function Clients({leads,setLeads,onOpen,currentProfile,role,users}) {
  ...
  const create = () => {
    ...
    const lead = {
      ...
      history:[`Клиент создан вручную. Ответственный: ${managerName}`]
    };
      ...
      try {
        ...
        const rows = data
          ...
          .map(row => ({
            ...
            phone:String(row[1] || "+380").trim(),
            source:String(row[2] || sources[0]).trim(),
            notes:String(row.slice(3).join(" ") || "").trim()
          }))
          .filter(x=>x.name || x.phone);
