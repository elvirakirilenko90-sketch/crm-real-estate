...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  // Аналитика НЕ тянет всю market_listings одним гигантским JSON.
  // Supabase сначала применяет выбранные фильтры, а затем мы забираем ВСЕ совпадения страницами.
  // Общего лимита количества совпадений нет.
  ...
  },[agencyId,activeSourceTab,country,region,city,districtFilter,dealFilter,roomsFilter,priceFrom,priceTo,areaFrom,areaTo,period,dateFrom,dateTo]);
  ...
  const deduplicatedBase=noDuplicates?filteredBase.filter((x,index,arr)=>{
    const key=String(x.external_id||x.source_url||[x.title,x.city,x.price,x.area].join("|")).trim().toLowerCase();
    return arr.findIndex(item=>String(item.external_id||item.source_url||[item.title,item.city,item.price,item.area].join("|")).trim().toLowerCase()===key)===index;
  }):filteredBase;
  ...
  return <main className="screen">
    ...
    <div className="card" style={{padding:22,borderRadius:24}}>
      ...
      <div className="chips" style={{marginTop:14}}>
        ...
        <label className={cn("chip",noDuplicates&&"active")}><input type="checkbox" checked={noDuplicates} onChange={e=>setNoDuplicates(e.target.checked)}/> Поиск без дублей</label>
      </div>
    </div>
    ...
    <div className="stats" style={{marginBottom:16}}>
      ...
      {Object.entries(sourceCounts).sort((a,b)=>b[1]-a[1]).map(([name,count])=><div className="stat" key={name} style={{minHeight:110}}><span>{name}</span><b style={{fontSize:34}}>{count}</b></div>)}
    </div>
