...
function PropertyModal({property,setProperties,onClose,users,currentProfile,role,lang,agencyName,theme,leads,recordActivity}) {
  ...
  const isPropertyCreator = Boolean(currentManagerEmail) &&
    String(creatorEmail).trim().toLowerCase() === String(currentManagerEmail).trim().toLowerCase();
  ...
  async function addMediaBatch() {
    ...
    for (const f of files) {
      ...
      uploadedItems.push({id:data.id, kind:data.media_type || detectedKind, url:data.media_url || publicUrl, name:data.file_name || originalName, storage_path:path});
    }
  ...
  async function deletePropertyMedia(media, index) {
    ...
    try {
      ...
      const updated = {...local, media: nextMedia};
      setLocal(updated);
      setProperties(prev => prev.map(p =>
        String(p.id) === String(local.id) ? {...p, media: nextMedia} : p
      ));
      setViewer(null);
  ...
  return <Modal onClose={onClose} wide>
    ...
    <div className="grid2"><Button className="full" onClick={save} disabled={!canEditProperty || savingProperty}>{savingProperty ? "Сохраняем..." : "Сохранить объект"}</Button><Button className="full" variant="soft" disabled={!canEditProperty} onClick={()=>{setLocal({...local,olx_status:"pending"}); alert("Заявка на публикацию в OLX отправлена тех отделу");}}>Опубликовать в OLX</Button></div>{local.olx_status === "pending" && isTech && <Button className="full" onClick={()=>{setLocal({...local,olx_status:"approved"}); alert("Тех отдел подтвердил публикацию. Для реальной публикации нужен OLX API токен.");}}>Подтвердить публикацию OLX</Button>}
    ...
    <div className="card">
      ...
      <div className="grid2">
        {mediaList.map((m,i)=>
          <div key={m.id || m.url || i} style={{position:"relative"}}>
            <div onClick={()=>setViewer(i)} style={{cursor:"pointer"}}>
              <Media kind={m.kind} name={m.url} small />
            </div>
            {canEditProperty && <button
              type="button"
              className="icon"
              title="Удалить фото/видео"
              onClick={(e)=>{e.stopPropagation();deletePropertyMedia(m,i);}}
              style={{position:"absolute",top:8,right:8,zIndex:2,background:"#fff",color:"#b91c1c"}}
            >×</button>}
          </div>
        )}
      </div>
    ...
    {currentMedia && <Modal onClose={()=>setViewer(null)} wide>
      <div className="row">
        ...
        {canEditProperty && <Button variant="danger" onClick={()=>deletePropertyMedia(currentMedia,viewer)}>Удалить</Button>}
  ...
  function propertyFromDb(row, mediaRows = []){
    return {
      ...
      media: mediaRows
        ...
        .map(m => ({id:m.id, kind:m.media_type, url:m.media_url, name:m.file_name, storage_path:m.storage_path || ""})),
      hot: false,
      history: ["Загружено из Supabase"]
    };
  }
