  ...
  async function createSelection(){
    ...
    try {
      ...
      for (let index = 0; index < items.length; index++){
        ...
        pdfDrawWrapped(ctx, p.title || `Объект ${index+1}`, 90, 140, 1060, 54);

        let y = 270;
        ctx.fillStyle = "#111827";
        ctx.font = "bold 32px Arial";
        ctx.fillText(`Объект ${index+1} из ${items.length}`, 90, y);
        y += 58;
        ...
        ctx.font = "25px Arial";
        ctx.fillStyle = "#374151";
        y = pdfDrawWrapped(ctx, p.description || "Описание объекта не добавлено.", 90, y, 1060, 36);

        y += 28;
        ctx.fillStyle = "#b45309";
        ctx.font = "bold 22px Arial";
        pdfDrawWrapped(ctx, "Документ сформирован без персональных данных: собственник, телефон собственника и ответственный менеджер не указаны.", 90, y, 1060, 32);

        pages.push({bytes:pdfDataUrlToBytes(canvas.toDataURL("image/jpeg",0.92)), width:canvas.width, height:canvas.height});

        const photos = (p.media || []).filter(m=>String(m.kind || "").includes("Фото") && m.url).slice(0,3);
        for (const photo of photos){
          pages.push(await imageToPdfPage(photo.url, p.title || `Объект ${index+1}`));
        }
      }

      const blob = buildPdfFromCanvasPages(pages);
      const localUrl = URL.createObjectURL(blob);
      setSelectionDownloadUrl(localUrl);

      const path = `selections/${Date.now()}-crm-selection.pdf`;
      ...
      if (uploaded?.publicUrl){
        ...
        alert("PDF-подборка создана. Её можно открыть, скачать или отправить клиенту по ссылке.");
      } else {
        setSelectionUrl(localUrl);
        alert("PDF-подборка создана локально. Для постоянной ссылки клиенту создай public bucket presentations в Supabase Storage.");
      }
    } catch (e) {
      console.error(e);
      alert("Не удалось создать PDF-подборку: " + (e?.message || e));
    }
  }
