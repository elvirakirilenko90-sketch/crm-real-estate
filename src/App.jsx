...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  },[agencyId,activeSourceTab,country,region,city,districtFilter,dealFilter,roomsFilter,priceFrom,priceTo,areaFrom,areaTo,period,dateFrom,dateTo,complexFilter,propertyKindFilter,constructionYearFrom,constructionYearTo,governmentPrograms.join("|")]);
  ...
  // Список ЖК строится из всех реально загруженных объектов выбранного города,
  // включая названия, извлечённые из заголовка/описания объявлений DIM.RIA.
  ...
  const complexes=["Все",...Array.from(new Set([...directoryComplexNames,...values(geoCity,"complex_name")])).sort((a,b)=>a.localeCompare(b,"uk"))];
  ...
  const resetFilters=()=>{setActiveSourceTab("ALL");setPeriod("30");setDateFrom("");setDateTo("");setCountry("Все");setRegion("Все");setCity("Все");setDistrictFilter("Все");setComplexFilter("Все");setPropertyKindFilter("Все");setSellerFilter("Все");setConditionFilter("Все");setFloorFrom("");setFloorTo("");setLandAreaFrom("");setLandAreaTo("");setVerifiedOnly(false);setNoDuplicates(false);setGroupDuplicates(true);setConstructionYearFrom("");setConstructionYearTo("");setGovernmentPrograms([]);setDealFilter("Все");setSelectedSources([]);setSourceCountry("Все");setSearchQuery("");setRoomsFilter("Все");setPriceFrom("");setPriceTo("");setAreaFrom("");setAreaTo("");setPriceSort("none");setSelected([]);clearAnalyticsMarketCache();};
