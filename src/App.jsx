...
const analyticsSourceCatalog = [
  {country:"Украина",name:"DIM.RIA",aliases:["DOM.RIA","DIM.RIA","dom.ria.com"],method:"WEB",url:"https://dom.ria.com/prodazha-kvartir/"},
  ...
  {country:"ОАЭ",name:"Dubizzle",aliases:["Dubizzle","dubizzle"],method:"WEB",url:"https://dubai.dubizzle.com/property-for-sale/"},

  {country:"Испания",name:"idealista",aliases:["idealista"],method:"WEB",url:"https://www.idealista.com/venta-viviendas/"},
  ...
  {country:"Португалия",name:"idealista",aliases:["idealista"],method:"WEB",url:"https://www.idealista.pt/comprar-casas/"},
  ...
  {country:"Италия",name:"idealista",aliases:["idealista"],method:"WEB",url:"https://www.idealista.it/vendita-case/"},
  {country:"Германия",name:"ImmoScout24",aliases:["ImmoScout24","ImmobilienScout24"],method:"WEB",url:"https://www.immobilienscout24.de/Suche/de/wohnung-kaufen"},
...
const analyticsGeoCatalog = {
  "Украина":{
    regions:{
      "Винницкая область":["Винница"],"Волынская область":["Луцк"],"Днепропетровская область":["Днепр","Кривой Рог"],"Донецкая область":["Краматорск","Славянск"],
      "Житомирская область":["Житомир"],"Закарпатская область":["Ужгород","Мукачево"],"Запорожская область":["Запорожье"],"Ивано-Франковская область":["Ивано-Франковск","Яремче"],
      "Киевская область":["Киев","Бровары","Буча","Ирпень","Белая Церковь","Борисполь","Вышгород"],"Кировоградская область":["Кропивницкий"],"Львовская область":["Львов","Дрогобыч","Трускавец"],
      "Николаевская область":["Николаев"],"Одесская область":["Одесса","Черноморск","Южное","Измаил","Белгород-Днестровский"],"Полтавская область":["Полтава","Кременчуг"],
      "Ровненская область":["Ровно"],"Сумская область":["Сумы"],"Тернопольская область":["Тернополь"],"Харьковская область":["Харьков"],"Херсонская область":["Херсон"],
      "Хмельницкая область":["Хмельницкий","Каменец-Подольский"],"Черкасская область":["Черкассы","Умань"],"Черновицкая область":["Черновцы"],"Черниговская область":["Чернигов"]
    }
  },
...
const officialCityDistricts = {
  "Украина|Одесса":["Приморский","Киевский","Хаджибейский","Пересыпский","Центр","Средний Фонтан","Молдаванка","Аркадия","Таирова","Черемушки","Котовского пос.","Малый Фонтан","Судостроитель пос."],
  "Україна|Одеса":["Приморський","Київський","Хаджибейський","Пересипський","Центр","Середній Фонтан","Молдаванка","Аркадія","Таїрова","Черемушки","Котовського с-ще","Малий Фонтан"],
...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  const catalogRegions=country!=="Все"?Object.keys(analyticsGeoCatalog[country]?.regions||{}):[];
  ...
  const catalogCities=country!=="Все"
    ? (region!=="Все" ? (analyticsGeoCatalog[country]?.regions?.[region]||[]) : Object.values(analyticsGeoCatalog[country]?.regions||{}).flat())
    : [];
  ...
  const liveSourceNames=values(sourceCountry==="Все"?combinedListings:combinedListings.filter(x=>String(x.country||"")===sourceCountry),"source_name");
  ...
  const geoKey=v=>String(v||"").trim().toLowerCase()
    ...
    .replace(/ы/g,"и").replace(/ъ|ь/g,"")
    .replace(/ий|ый|ій/g,"и")
    .replace(/сс/g,"с")
    .replace(/\s+/g," ").trim();
  const geoEq=(a,b)=>{
    const aa=geoKey(a),bb=geoKey(b);
    ...
    const known={
      ...
      "харьков":"kharkiv","харків":"kharkiv","kharkiv":"kharkiv","kharkov":"kharkiv",
      "днепр":"dnipro","дніпро":"dnipro","dnipro":"dnipro","dnepr":"dnipro",
      "львов":"lviv","львів":"lviv","lviv":"lviv","lvov":"lviv",
      "запорожье":"zaporizhzhia","запоріжжя":"zaporizhzhia","zaporizhzhia":"zaporizhzhia","zaporozhye":"zaporizhzhia",
      "николаев":"mykolaiv","миколаїв":"mykolaiv","mykolaiv":"mykolaiv","nikolaev":"mykolaiv"
    };
    return (known[aa]||aa)===(known[bb]||bb);
  };
  ...
  const filtered=combinedListings.filter(x=>{
    ...
    return inMarketPeriod(x)
      &&(country==="Все"||geoEq(x.country,country))
      &&(region==="Все"||geoEq(x.region,region))
      &&(city==="Все"||geoEq(x.city,city))
  ...
  return <main className="screen">
    ...
    <div className="card" style={{padding:22,borderRadius:24}}>
      ...
      <div className="grid4">
        ...
        <div style={filterBox}><Field label="Район / микрорайон"><select className="input" value={districtFilter} onChange={e=>setDistrictFilter(e.target.value)}>{districtsMarket.map(x=><option key={x}>{x}</option>)}</select></Field></div>
