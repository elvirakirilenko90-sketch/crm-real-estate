...
function openWhatsAppPresentation({phone,url,lang,agencyName,clientName,title,isSelection=false}) {
  if (!url || !/^https?:\/\//i.test(url)) { alert("Для отправки через WhatsApp нужна публичная ссылка на PDF. Проверь public bucket presentations в Supabase Storage."); return; }
  const digits = messengerPhoneDigits(phone);
  if (!digits) { alert("У клиента не указан номер телефона."); return; }
...
function openViberPresentation({phone,url,lang,agencyName,clientName,title,isSelection=false}) {
  if (!url || !/^https?:\/\//i.test(url)) { alert("Для отправки через Viber нужна публичная ссылка на PDF."); return; }
  const digits = messengerPhoneDigits(phone);
  if (!digits) { alert("У клиента не указан номер телефона."); return; }
  const plusPhone = "+" + digits;
  const message = presentationSendText(lang,{agencyName,clientName,title,isSelection,url});
  try { navigator.clipboard?.writeText(message); } catch {}
  // Используем ту же рабочую схему Viber, что и в карточке клиента: открываем чат конкретного номера.
  // Текст одновременно копируется в буфер, чтобы менеджеру оставалось только вставить/отправить его в Viber.
  window.location.href = `viber://chat?number=${encodeURIComponent(plusPhone)}`;
}
...
function Properties({properties,setProperties,onOpen,users,currentProfile,lang,agencyName,theme,leads,recordActivity}) {
  ...
  return <main className="screen">
    ...
    {selectionUrl && <div className="card selectionBar">
      ...
      <div className="grid4">
        <Button className="full" onClick={()=>{openWhatsAppPresentation({phone:selectionClientPhone,url:selectionUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true});logPresentationSend("whatsapp");}}>WhatsApp</Button>
        <Button className="full" variant="purple" onClick={()=>{openViberPresentation({phone:selectionClientPhone,url:selectionUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true});logPresentationSend("viber");}}>Viber</Button>
    ...
    <div className="card amber">
      ...
      {presentationUrl && <div style={{marginTop:12}}>
        <div className="card" style={{padding:12,marginBottom:10}}>
          ...
          <p className="muted" style={{margin:"6px 0 10px"}}>CRM подставит имя клиента, название агентства и готовую ссылку на VIP-презентацию.</p>
          <div className="grid4">
            <Button className="full" onClick={()=>{openWhatsAppPresentation({phone:presentationClientPhone,url:presentationUrl,lang:presentationLang,agencyName,clientName:presentationClient,title:local.title,isSelection:false});logPropertyPresentationSend("whatsapp");}}>WhatsApp</Button>
            <Button className="full" variant="purple" onClick={()=>{openViberPresentation({phone:presentationClientPhone,url:presentationUrl,lang:presentationLang,agencyName,clientName:presentationClient,title:local.title,isSelection:false});logPropertyPresentationSend("viber");}}>Viber</Button>
