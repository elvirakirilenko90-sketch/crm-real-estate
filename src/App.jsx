  ...
  return <main className="screen">
    ...
    {viewMode === "list" && <div className="card">
      ...
      <div className="clientList">
        {filteredLeads.map(l => <div className="lead listLead" key={l.id}>
          <div className="leadTop">
            <section><b>{l.name || tr(lang,"Без имени")}</b><span>{l.id} · {l.source || ""}</span></section>
            {badge(l.status || "", "dark")}
          </div>
          <div className="phone">
            <b>☎ {l.phone || ""}</b>
            <p>{l.notes || tr(lang,"Без заметки")}</p>
            <p className="muted">{tr(lang,"Ответственный")}: {l.manager || tr(lang,"не назначен")}</p>
            {l.nextContact && <p className="muted">{tr(lang,"Следующий контакт")}: {l.nextContact}</p>}
          </div>
          <div className="grid4">
            ...
            <Button onClick={()=>onOpen(l)}>{tr(lang,"Открыть карточку")}</Button>
          </div>
        </div>)}
      </div>
    </div>}
