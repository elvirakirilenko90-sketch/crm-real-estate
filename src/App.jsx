...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  const cleanPresentationDescription=value=>String(value||"")
    .replace(/data-[a-z-]+=(?:"[^"]*"|'[^']*')/gi," ")
    .replace(/class=(?:"[^"]*"|'[^']*')/gi," ")
  ...
  const createSelection=async()=>{
    ...
    try{
      ...
      c.fillStyle=pal.bg;c.fillRect(0,0,1240,1754);
      const firstHero=extractPropertyPhotos(items[0])[0];
      if(firstHero){c.globalAlpha=selectionVisual==="luxuryDark"?.30:.16;await drawAnalyticsPresentationImage(c,firstHero.url,520,0,720,1754,"cover");c.globalAlpha=1;}
      c.fillStyle=pal.accent;c.fillRect(72,150,5,1260);
      if(brand.logo_url)await drawRemoteImage(c,brand.logo_url,95,75,180,100,"contain");
      c.fillStyle=selectionVisual==="luxuryDark"?"#fff":pal.ink;c.font='600 30px Arial';c.fillText(agencyName||"CRM Real Estate",95,245);
      c.font='bold 58px Georgia, "Times New Roman", serif';pdfDrawWrapped(c,txt.selection,95,405,720,68);
      c.fillStyle=pal.accent;c.font='bold 23px Arial';c.fillText(`${txt.date}: ${formatPresentationDate(selectionLang)}`,95,665);
      if(selectionClient.trim()){c.fillStyle=selectionVisual==="luxuryDark"?"#eee":pal.muted;c.font='22px Arial';c.fillText(txt.prepared,95,775);c.fillStyle=selectionVisual==="luxuryDark"?"#fff":pal.ink;c.font='bold 38px Georgia, "Times New Roman", serif';pdfDrawWrapped(c,selectionClient.trim(),95,835,700,48);}
      c.fillStyle=selectionVisual==="luxuryDark"?"#ddd":pal.muted;c.font='22px Arial';c.fillText(`${items.length} ${selectionLang==="en"?"properties":selectionLang==="uk"?"об'єктів":"объектов"}`,95,1055);
      pages.push({bytes:pdfDataUrlToBytes(cover.toDataURL("image/jpeg",.96)),width:1240,height:1754});
      ...
      ec.fillStyle=selectionVisual==="luxuryDark"?"#fff":pal.ink;ec.font='bold 48px Georgia, "Times New Roman", serif';pdfDrawWrapped(ec,txt.contact,92,445,860,58);
      ...
      ec.fillStyle=selectionVisual==="luxuryDark"?"#fff":pal.ink;ec.font='24px Arial';pdfDrawWrapped(ec,currentProfile?.phone||currentProfile?.email||"",92,775,850,32);ec.font='28px Georgia, "Times New Roman", serif';pdfDrawWrapped(ec,txt.thanks,92,1040,850,38);
