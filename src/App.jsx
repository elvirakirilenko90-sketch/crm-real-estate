...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  const analyticsSourceTabs=[
    ...
    {id:"OLX",label:"OLX",db:["OLX"]},
    {id:"DIMRIA",label:"DIM.RIA",db:["DIM.RIA Украина","DIM.RIA"]},
    {id:"RIELTOR",label:"RIELTOR.UA",db:["RIELTOR.UA"]},
    {id:"LUN",label:"ЛУН",db:["ЛУН","LUN"]},
    {id:"REM",label:"REM",db:["REM","REM.UA"]},
    {id:"TELEGRAM",label:"Telegram",db:["ПБН","TELEGRAM","Telegram"]},
    {id:"CRM",label:"CRM",db:[]}
  ];
  ...
  return <main className="screen">
    ...
    <div className="card" style={{padding:22,borderRadius:24}}>
      ...
      <div className="grid4">
        <div style={filterBox}><Field label="Страна"><select className="input" value={country} onChange={e=>{const next=e.target.value;setCountry(next);setRegion("Все");setCity("Все");setDistrictFilter("Все");setComplexFilter("Все");setSelectedSources([]);setSourceCountry(next==="Все"?"Все":next);}}>{countries.map(x=><option key={x} value={x}>{geoDisplay(x)}</option>)}</select></Field></div>
    ...
    {marketView==="list"&&filtered.length>0&&<div className="card" style={{padding:0,overflowX:"auto",borderRadius:20}}><table className="clientTable analyticsMobileTable" style={{width:"100%",borderCollapse:"collapse",minWidth:1120}}><thead><tr><th>Фото</th><th>Объект</th><th>Источник</th><th>Продавец</th><th>Телефон</th><th>Локация</th><th>Комнаты</th><th>Площадь</th><th>Цена</th><th></th></tr></thead><tbody>{renderedFiltered.map(x=>{const crm=crmMatch(x),photo=x.main_photo_url||(Array.isArray(x.photo_urls)?x.photo_urls[0]:"");return <tr key={`list-${x._key}`}><td style={{width:90}}>{photo?<img src={photo} alt="" loading="lazy" decoding="async" style={{width:72,height:54,objectFit:"cover",borderRadius:10}}/>:<div style={{width:72,height:54,display:"grid",placeItems:"center",background:"var(--soft)",borderRadius:10}}>🏠</div>}</td><td><b>{x.title||x.complex_name||"Объект недвижимости"}</b><div className="muted" style={{fontSize:12}}>{x.complex_name||""}</div></td><td>{x.source_name||"—"}</td><td><span className="badge">{sellerLabel(x)}</span></td><td>{sellerKind(x)==="realtor"?(contactPhone(x)||"—"):"—"}</td><td>{[geoDisplay(x.city),geoDisplay(x.district)].filter(Boolean).join(" · ")||"—"}</td><td>{x.rooms||"—"}</td><td>{Number(x.area||0)>0?`${x.area} м²`:"—"}</td><td><b style={{whiteSpace:"nowrap"}}>{marketMoney(x.price,marketCurrency(x))}</b></td><td><Button style={{minHeight:38}} onClick={()=>openListing(x)}>{crm?"Открыть CRM":"Открыть"}</Button></td></tr>})}</tbody></table></div>}
