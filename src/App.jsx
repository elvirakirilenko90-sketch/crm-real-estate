...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  const filterRenderableAnalyticsPhotos=async photos=>{
    ...
    for(let start=0;start<list.length;start+=8){
      const batch=await Promise.all(list.slice(start,start+8).map(async photo=>{
        try{
          const {data,error}=await supabase.functions.invoke("analytics-live-search",{body:{action:"presentation_image",url:photo.url}});
          if(error)throw error;
          const blob=data instanceof Blob?data:data instanceof ArrayBuffer?new Blob([data]):null;
          if(!blob)throw new Error("image proxy returned no image");
          const objectUrl=URL.createObjectURL(blob),probe=document.createElement("canvas");probe.width=8;probe.height=8;
          const ok=await drawRemoteImage(probe.getContext("2d"),objectUrl,0,0,8,8,"cover");
          if(!ok){URL.revokeObjectURL(objectUrl);return null;}
          return {...photo,original_url:photo.url,url:objectUrl};
  ...
  const createSelection=async()=>{
    ...
    try{
      ...
      const blob=buildPdfFromCanvasPages(pages),localUrl=URL.createObjectURL(blob);setSelectionDownloadUrl(localUrl);
      items.forEach(item=>extractPropertyPhotos(item).forEach(photo=>{if(/^blob:/i.test(photo.url))URL.revokeObjectURL(photo.url);}));
