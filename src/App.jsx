...
function Feed({posts,setPosts,role}) {
  ...
  async function uploadToNewsMedia(newsId) {
    ...
    const upload = await supabase.storage.from("news-media").upload(path, fileObj, {
      upsert: true,
...
function Calendar({events,setEvents,leads,setLeads,onOpenLead}) {
  ...
  const create = (date, time="12:00") => setOpen({leadId:leads[0]?.id || "",type:"Звонок",title:"",date,time,reminder:60});
  ...
  return <main className="screen" style={{maxWidth:"100%"}}>
    <div className="card dark">
      <div className="row">
        <section>
          <h2>Календарь</h2>
          <p>Встречи, звонки, показы и задачи по клиентам. Визуально сделан в стиле Google Calendar.</p>
        </section>
        <a className="btn soft" href="https://calendar.google.com/" target="_blank" rel="noopener noreferrer">Открыть Google Calendar</a>
      </div>
    </div>
    ...
    <div className="grid2">
      {events.filter(e=>!monthKey || String(e.date || "").startsWith(monthKey)).map(e=><div className="card event" key={e.id} onClick={()=>setOpen(e)}>
        <div className="row"><section><b>{e.title}</b><p className="muted">{e.type} · {e.date} · {e.time}</p></section>{badge(e.reminder+" мин","gold")}</div>
    ...
    {open && <Modal onClose={()=>setOpen(null)} wide>
      ...
      {selectedEventLead && <div className="card">
        ...
        <Button variant="soft" onClick={()=>onOpenLead && onOpenLead(selectedEventLead)}>Открыть полную карточку клиента</Button>
        {(selectedEventLead.history || []).slice(-5).map((h,i)=><p className="muted" key={i}>• {h}</p>)}
      </div>}
      <div className="grid2"><input className="input" type="date" value={open.date} onChange={e=>setOpen({...open,date:e.target.value})}/><input className="input" type="time" value={open.time} onChange={e=>setOpen({...open,time:e.target.value})}/></div>
...
function More({role,setRole,setPage,theme,setTheme,leads,setLeads,users}) {
  const isTech = role === "Администратор тех отдел";
  const [manualLead,setManualLead] = useState({name:"",phone:"+380",source:sources[0],status:stages[0],manager:"Елена",notes:""});
  const [importText,setImportText] = useState("");
  const managers = (users || []).filter(u => (u.role || "").includes("Менеджер")).map(u => u.full_name || u.name).filter(Boolean);
  const managerOptions = managers.length ? managers : ["Елена","Андрей"];
  ...
  const createManualLead = () => {
    ...
    const record = {
      ...
      notes:`Ответственный менеджер: ${manualLead.manager}\n${manualLead.notes || ""}`.trim(),
      nextContact:"",
      manager:manualLead.manager,
      history:[`Клиент создан тех отделом и назначен: ${manualLead.manager}`]
    };
    setLeads(prev=>[record,...prev]);
    setManualLead({name:"",phone:"+380",source:sources[0],status:stages[0],manager:managerOptions[0] || "Елена",notes:""});
  };
  ...
  const importClients = () => {
    const manager = manualLead.manager || managerOptions[0] || "Елена";
    ...
    const created = rows.map((r,i)=>({
      ...
      status:stages[0],
      notes:`Ответственный менеджер: ${manager}\n${r.notes || ""}`.trim(),
      nextContact:"",
      manager,
  ...
  return <main className="screen">
    <div className="card">
      ...
      <div className="grid3"><Button>Русский</Button><Button variant="soft">Українська</Button><Button variant="soft">English</Button></div>
    </div>
    ...
    {isTech && <div className="card">
      ...
      <div className="grid2">
        ...
        <select className="input" value={manualLead.manager} onChange={e=>setManualLead({...manualLead,manager:e.target.value})}>{managerOptions.map(m=><option key={m}>{m}</option>)}</select>
      </div>
    ...
    {isTech && <div className="card">
      ...
      <p className="muted">Поддерживаются CSV/TXT/TSV: имя;телефон;источник;заметка. Для Excel сначала сохрани файл как CSV.</p>
      <input className="input" type="file" accept=".csv,.txt,.tsv" onChange={e=>readFile(e.target.files?.[0])}/>
      <select className="input" value={manualLead.manager} onChange={e=>setManualLead({...manualLead,manager:e.target.value})}>{managerOptions.map(m=><option key={m}>{m}</option>)}</select>
      <textarea className="input" style={{minHeight:120}} placeholder="Или вставь список клиентов сюда: Имя;Телефон;Источник;Заметка" value={importText} onChange={e=>setImportText(e.target.value)}/>
...
export default function App(){
  ...
  function leadFromDb(row){
    return {
      ...
      manager: row.manager || extractManager(row.notes),
  ...
  async function syncLeadToSupabase(item){
    const manager = item.manager || extractManager(item.notes);
  ...
  return <div className="layout" style={themeStyle(theme)}>
    ...
    <div className="mainWrap">
      <main className="main">
        <Top page={page} setPage={setPage}/>
        {page==="feed"&&<Feed posts={posts} setPosts={setPosts} role={role}/>}
        {page==="clients"&&<Clients leads={leads} setLeads={setLeads} onOpen={setLead}/>}
        {page==="calendar"&&<Calendar events={events} setEvents={setEvents} leads={leads} setLeads={setLeads} onOpenLead={setLead}/>}
        {page==="properties"&&<Properties properties={properties} setProperties={setProperties} onOpen={setProperty}/>}
        {page==="analytics"&&<Analytics leads={leads} properties={properties} events={events} role={role}/>}
        {page==="help"&&<Help help={help} setHelp={setHelp} setLeads={setLeads} leads={leads} onOpen={setLead}/>}
        {page==="access"&&role==="Администратор тех отдел"&&<Access agencies={agencies} setAgencies={setAgencies} users={users} setUsers={setUsers}/>}
        {page==="more"&&<More role={role} setRole={setRole} setPage={setPage} theme={theme} setTheme={setTheme} leads={leads} setLeads={setLeads} users={users}/>}
        <Bottom page={page} setPage={setPage}/>
        {lead&&<LeadModal lead={lead} setLeads={setLeads} setEvents={setEvents} setHelp={setHelp} onClose={()=>setLead(null)}/>}
        {property&&<PropertyModal property={property} setProperties={setProperties} onClose={()=>setProperty(null)}/>}
      </main>
    </div>
  </div>
}
