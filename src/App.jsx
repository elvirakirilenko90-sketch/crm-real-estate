...
async function ensurePublicPresentationUrl(url, prefix="presentation"){
  ...
  const path = `messenger/${Date.now()}-${Math.random().toString(36).slice(2,8)}-${safePrefix}.pdf`;
...
async function openViberPresentation({phone,url,lang,agencyName,clientName,title,isSelection=false}) {
  ...
  try {
    ...
    // Viber: копируем готовый текст со ссылкой и сразу открываем чат нужного клиента.
    try {
...
function Properties({properties,setProperties,onOpen,users,currentProfile,lang,agencyName,theme,leads,recordActivity}) {
  ...
  return <main className="screen">
    ...
    {selectionUrl && <div className="card selectionBar">
      ...
      <div className="grid4">
        <Button className="full" onClick={async()=>{if(await openWhatsAppPresentation({phone:selectionClientPhone,url:selectionUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true})) logPresentationSend("whatsapp");}}>Отправить в WhatsApp</Button>
        <Button className="full" variant="purple" onClick={async()=>{if(await openViberPresentation({phone:selectionClientPhone,url:selectionUrl,lang:selectionLang,agencyName,clientName:selectionClient,isSelection:true})) logPresentationSend("viber");}}>Отправить в Viber</Button>
    ...
    <div className="card amber">
      ...
      {presentationUrl && <div style={{marginTop:12}}>
        <div className="card" style={{padding:12,marginBottom:10}}>
          ...
          <div className="grid4">
            <Button className="full" onClick={async()=>{if(await openWhatsAppPresentation({phone:presentationClientPhone,url:presentationUrl,lang:presentationLang,agencyName,clientName:presentationClient,title:local.title,isSelection:false})) logPropertyPresentationSend("whatsapp");}}>Отправить в WhatsApp</Button>
            <Button className="full" variant="purple" onClick={async()=>{if(await openViberPresentation({phone:presentationClientPhone,url:presentationUrl,lang:presentationLang,agencyName,clientName:presentationClient,title:local.title,isSelection:false})) logPropertyPresentationSend("viber");}}>Отправить в Viber</Button>
