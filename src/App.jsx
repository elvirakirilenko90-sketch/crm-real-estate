...
function MarketTools({lang,currentProfile,properties=[]}){
  ...
  useEffect(()=>{ loadMarketData(); },[agencyId]);
  ...
  return <div>
    ...
    {tab==="sources"&&<div className="card">
      ...
      {sourcesX.map(s=><div className="dealRow" key={s.id}>
        <b>{s.name}</b><span>{s.country}</span><span className="badge">{s.source_type||s.type}</span>
        <span className="badge gold">{s.enabled?"Активен":"Отключен"}</span>
    ...
    {tab==="telegram"&&<div className="card">
      ...
      {channels.map(c=><div className="dealRow" key={c.id}>
        <b>{c.name}</b><span>{c.city||"—"} · {c.country}</span><span className="badge">Telegram</span>
        <span className="badge gold">{c.enabled?"Активен":"Отключен"}</span>
