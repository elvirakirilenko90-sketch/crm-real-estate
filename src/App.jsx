...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  // ВАЖНО: Supabase/PostgREST обычно возвращает максимум 1000 строк за запрос.
  // Поэтому Аналитика загружает рынок страницами, иначе свежий массовый DIM.RIA вытесняет OLX/Telegram/CRM из первых 1000 строк.
  const fetchAllMarketListings=async()=>{
    // ВАЖНО: используем cursor pagination по id, а не range/offset.
    // Так Supabase гарантированно отдаёт OLX + DIM.RIA + Telegram, даже когда один источник
    // массово обновился и занял первые 1000 строк по last_seen_at.
    const pageSize=1000;
    const maxRows=20000;
    let rows=[];
    let lastId=null;
    while(rows.length<maxRows){
      ...
      if(batch.length<pageSize||lastId===null||lastId===undefined)break;
    }
  ...
  const filtered=[...filteredBase].sort((a,b)=>{
    ...
    return String(getListingDate(b)||"").localeCompare(String(getListingDate(a)||""));
  });
  ...
  return <main className="screen">
    ...
    <div className="row" style={{margin:"20px 0 10px",gap:12,flexWrap:"wrap"}}><section><h3 style={{margin:0}}>Объявления рынка</h3><p className="muted">{loadingMarket?"Загрузка…":`Показано ${filtered.length} объектов с указанной ценой`}</p></section><div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}><select className="input" style={{minWidth:220}} value={priceSort} onChange={e=>setPriceSort(e.target.value)}><option value="none">Сортировка: новые сначала</option><option value="asc">Цена: по возрастанию</option><option value="desc">Цена: по убыванию</option></select><Button variant={marketView==="cards"?"primary":"soft"} onClick={()=>setMarketView("cards")}>▦ Карточки</Button><Button variant={marketView==="list"?"primary":"soft"} onClick={()=>setMarketView("list")}>☷ Список</Button></div></div>
    ...
    {marketView==="list"&&filtered.length>0&&<div className="card" style={{padding:0,overflowX:"auto",borderRadius:20}}><table className="clientTable" style={{width:"100%",borderCollapse:"collapse",minWidth:1050}}><thead><tr><th>Фото</th><th>Объект</th><th>Источник</th><th>Локация</th><th>Комнаты</th><th>Площадь</th><th>Цена</th><th></th></tr></thead><tbody>{filtered.map(x=>{const crm=crmMatch(x),photo=x.main_photo_url||(Array.isArray(x.photo_urls)?x.photo_urls[0]:"");return <tr key={`list-${x._key}`}><td style={{width:90}}>{photo?<img src={photo} alt="" style={{width:72,height:54,objectFit:"cover",borderRadius:10}}/>:<div style={{width:72,height:54,display:"grid",placeItems:"center",background:"var(--soft)",borderRadius:10}}>🏠</div>}</td><td><b>{x.title||x.complex_name||"Объект недвижимости"}</b><div className="muted" style={{fontSize:12}}>{x.complex_name||""}</div></td><td>{x.source_name||"—"}</td><td>{[x.city,x.district].filter(Boolean).join(" · ")||"—"}</td><td>{x.rooms||"—"}</td><td>{Number(x.area||0)>0?`${x.area} м²`:"—"}</td><td><b style={{whiteSpace:"nowrap"}}>{marketMoney(x.price,marketCurrency(x))}</b></td><td><Button style={{minHeight:38}} onClick={()=>openListing(x)}>{crm?"Открыть CRM":"Открыть"}</Button></td></tr>})}</tbody></table></div>}
