...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  const fetchAllMarketListings=async()=>{
    ...
    if(!wanted.length){setLiveSourceMeta([]);return [];}
    const {data,error}=await supabase.functions.invoke("analytics-live-search",{body:{
    ...
    setLiveSourceMeta(results.map(x=>({source:x.source,total:x.total,exact:x.count_is_exact!==false,ok:x.ok!==false,error:x.error||"",url:x.url||""})));
    return results.flatMap(x=>(x.items||[]).map((row,index)=>({...row,
      id:`live-${x.source}-${row.external_id||index}`,origin:"live",live_total:x.total,
  ...
  const combinedListings=[...normalizedMarketListings,...(livePage===1?internalMarketListings:[])];
  ...
  return <main className="screen">
    ...
    <div className="stats" style={{marginBottom:16}}>
      ...
      {(liveTotals.length?liveTotals.map(x=>[x.source,Number(x.total),x.exact]):Object.entries(sourceCounts).map(([name,count])=>[name,count,true])).sort((a,b)=>b[1]-a[1]).map(([name,count,exact])=><div className="stat" key={name} style={{minHeight:110}}><span>{name}</span><b style={{fontSize:34}}>{exact?count:`≈ ${count}`}</b></div>)}
    </div>
    ...
    <div className="row" style={{margin:"20px 0 10px",gap:12,flexWrap:"wrap"}}><section><h3 style={{margin:0}}>Объявления рынка</h3><p className="muted">{loadingMarket?"Загрузка…":`${filtered.length} карточек на странице сайтов ${livePage} · на экране ${pageFrom}–${pageTo}`}</p></section><div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}><Button variant="soft" disabled={loadingMarket||livePage<=1} onClick={()=>{setResultPage(1);setLivePage(p=>Math.max(1,p-1));}}>‹ Предыдущая страница сайтов</Button><span className="chip" style={{minHeight:42,display:"inline-flex",alignItems:"center",fontWeight:900}}>Страница сайтов {livePage}</span><Button variant="soft" disabled={loadingMarket||(!filtered.length&&livePage>1)} onClick={()=>{setResultPage(1);setLivePage(p=>p+1);}}>Следующая страница сайтов ›</Button><select className="input" style={{minWidth:250}} value={priceSort} onChange={e=>setPriceSort(e.target.value)}><option value="none">От самых новых</option><option value="asc">От самого дешевого</option><option value="desc">От самого дорогого</option><option value="areaAsc">От наименьшей площади</option><option value="areaDesc">От наибольшей площади</option></select><Button variant={marketView==="cards"?"primary":"soft"} onClick={()=>setMarketView("cards")}>▦ Карточки</Button><Button variant={marketView==="list"?"primary":"soft"} onClick={()=>setMarketView("list")}>☷ Список</Button></div></div>
