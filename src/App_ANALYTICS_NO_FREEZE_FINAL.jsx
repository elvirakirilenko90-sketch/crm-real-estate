...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  const marketServerParams=()=>{
    ...
    return {
      ...
      // aliases источников (DIM.RIA / DIM.RIA Украина и т.п.) оставляем клиентскому sourceMatches,
      // чтобы сервер не отбрасывал реальные карточки из-за названия источника.
      p_sources:null,
  ...
  },[agencyId,country,region,city,districtFilter,dealFilter,roomsFilter,priceFrom,priceTo,areaFrom,areaTo,period,dateFrom,dateTo]);
  ...
  const resetFilters=()=>{setPeriod("30");setDateFrom("");setDateTo("");setCountry("Все");setRegion("Все");setCity("Все");setDistrictFilter("Все");setComplexFilter("Все");setDealFilter("Все");setSelectedSources([]);setSourceCountry("Все");setSearchQuery("");setRoomsFilter("Все");setPriceFrom("");setPriceTo("");setAreaFrom("");setAreaTo("");setPriceSort("none");setSelected([]);clearAnalyticsMarketCache();};
  ...
  return <main className="screen">
    ...
    <div className="card" style={{padding:"14px 18px",borderRadius:18,display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
      ...
      {!searchQuery&&country==="Все"&&region==="Все"&&city==="Все"&&districtFilter==="Все"&&sourceCountry==="Все"&&!selectedSources.length&&<span className="muted">без географических ограничений</span>}
    </div>
