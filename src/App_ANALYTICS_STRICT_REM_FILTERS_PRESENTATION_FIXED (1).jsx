  ...
  const enrichAnalyticsPresentationItem=async item=>{
    ...
    try{
      ...
      return {...item,title:cleanPresentationTitle(item.title||data.title)||"Объект недвижимости",description:detailDescription||cleanPresentationDescription(item.description),description_uk:detailDescription||cleanPresentationDescription(item.description_uk||item.description),media,
        floor:d.floor||item.floor||"",total_floors:d.total_floors||item.total_floors||"",area:d.area||item.area||0,rooms:d.rooms||item.rooms||"",
  ...
  const createSelection=async()=>{
    ...
    try{
      ...
      for(let index=0;index<items.length;index++){
        ...
        ctx.fillStyle=pal.muted;ctx.font='20px Arial';ctx.fillText(`${formatPresentationDate(selectionLang)} · ${index+1}/${items.length}`,220,165);
        if(photo)await drawAnalyticsPresentationImage(ctx,photo.url,80,225,1080,690,"cover");
        else{ctx.fillStyle="#eee8dc";ctx.fillRect(80,225,1080,690);ctx.fillStyle=pal.muted;ctx.font='bold 28px Arial';ctx.textAlign="center";ctx.fillText("Фото не передано рекламной площадкой",620,530);ctx.font='22px Arial';ctx.fillText(p.source_name||"Источник",620,575);ctx.textAlign="left";}
        ...
        const specs=[[txt.price,money(p.price)],[txt.area,p.area?`${p.area} м²`:"—"],[roomLabel,p.rooms||"—"],[txt.floor,p.floor||"—"],[floorsLabel,p.total_floors||"—"],[txt.district,p.district||"—"],[txt.type,p.type||"—"],[complexLabel,p.complex_name||"—"],[conditionLabel,p.condition||"—"],[sourceLabel,p.source_name||"CRM"]];
        ...
        ctx.fillStyle=pal.muted;ctx.font='21px Georgia, "Times New Roman", serif';const teaserLines=canvasTextLines(ctx,preview,1080).slice(0,7);let ty=1290;teaserLines.forEach(line=>{ctx.fillText(line,80,ty);ty+=33;});
        pages.push({bytes:pdfDataUrlToBytes(canvas.toDataURL("image/jpeg",.97)),width:1240,height:1754});
