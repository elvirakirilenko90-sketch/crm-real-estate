...
async function addLuxuryGalleryPage(pages,{photos,title,agencyName,themeId,visualId,lang,drawImage=drawRemoteImage}){
...
function MarketAnalyticsV2({leads,properties,currentProfile,onOpenProperty,agencyName,theme,lang="ru"}) {
  ...
  const selectionItems=combinedListings.filter(x=>selected.includes(String(x._key))).map(x=>{
    ...
    return {
      ...
      source_url:x.source_url||"",source_name:x.source_name||"",rooms:x.rooms||"",complex_name:x.complex_name||"",raw_data:x.raw_data||{}
    };
  });
  ...
  const cleanPresentationTitle=value=>cleanPresentationDescription(value)
    .replace(/[‼❗🔥⭐🌟💥✅🔑🏠🏡]+/gu," ").replace(/!{2,}/g,"!").replace(/\s+/g," ").trim().slice(0,120);
  const enrichAnalyticsPresentationItem=async item=>{
    ...
    try{
      ...
      const remotePhotos=Array.isArray(data.images)?data.images.filter(Boolean):[];
      const all=[...currentPhotos.map(x=>x.url),...remotePhotos].filter(Boolean);
      const media=[...new Set(all)].map(url=>({kind:"Фото",url}));
      const detailDescription=cleanPresentationDescription(data.description||"");
      return {...item,title:cleanPresentationTitle(item.title||data.title)||"Объект недвижимости",description:detailDescription||cleanPresentationDescription(item.description),description_uk:detailDescription||cleanPresentationDescription(item.description_uk||item.description),media};
  ...
  const createSelection=async()=>{
    ...
    try{
      ...
      const pal=presentationPalette(selectionVisual,theme);
      const items=[];
      for(const item of selectionItems)items.push(await enrichAnalyticsPresentationItem(item));

      // Обложка — тот же стиль, что в разделе «Вторичка».
      const cover=document.createElement("canvas");cover.width=1240;cover.height=1754;let c=cover.getContext("2d");
      c.fillStyle=pal.bg;c.fillRect(0,0,1240,1754);
      const firstHero=extractPropertyPhotos(items[0])[0];
      if(firstHero){c.globalAlpha=.34;await drawAnalyticsPresentationImage(c,firstHero.url,500,0,740,1754,"cover");c.globalAlpha=1;}
      if(selectionVisual==="luxuryDark"){const shade=c.createLinearGradient(0,0,920,0);shade.addColorStop(0,"rgba(7,15,32,.98)");shade.addColorStop(.72,"rgba(7,15,32,.90)");shade.addColorStop(1,"rgba(7,15,32,.18)");c.fillStyle=shade;c.fillRect(0,0,980,1754);}
      c.fillStyle=pal.accent;c.fillRect(72,150,5,1260);
      if(brand.logo_url)await drawRemoteImage(c,brand.logo_url,95,75,180,100,"contain");
      const coverText=selectionVisual==="luxuryDark"?"#ffffff":pal.ink;
      c.fillStyle=coverText;c.font='600 30px Arial';c.fillText(agencyName||"CRM Real Estate",95,245);
      c.fillStyle=coverText;c.font='bold 58px Georgia, "Times New Roman", serif';pdfDrawWrapped(c,txt.selection,95,405,720,68);
      c.fillStyle=pal.accent;c.font='bold 23px Arial';c.fillText(`${txt.date}: ${formatPresentationDate(selectionLang)}`,95,665);
      if(selectionClient.trim()){c.fillStyle=selectionVisual==="luxuryDark"?"#f1f5f9":pal.muted;c.font='22px Arial';c.fillText(txt.prepared,95,775);c.fillStyle=coverText;c.font='bold 38px Georgia, "Times New Roman", serif';pdfDrawWrapped(c,selectionClient.trim(),95,835,700,48);}
      c.fillStyle=selectionVisual==="luxuryDark"?"#e2e8f0":pal.muted;c.font='22px Arial';c.fillText(`${items.length} ${selectionLang==="en"?"properties":selectionLang==="uk"?"об'єктів":"объектов"}`,95,1055);
      pages.push({bytes:pdfDataUrlToBytes(cover.toDataURL("image/jpeg",.96)),width:1240,height:1754});
      ...
      for(let index=0;index<items.length;index++){
        ...
        ctx.fillStyle=pal.ink;ctx.font='bold 40px Georgia, "Times New Roman", serif';pdfDrawWrapped(ctx,cleanTitle,220,72,900,45);
        ctx.fillStyle=pal.muted;ctx.font='20px Arial';ctx.fillText(`${formatPresentationDate(selectionLang)} · ${index+1}/${items.length}`,220,165);
        if(photo)await drawAnalyticsPresentationImage(ctx,photo.url,80,225,1080,620,"cover");
        else{ctx.fillStyle="#eee8dc";ctx.fillRect(80,225,1080,620);ctx.fillStyle=pal.muted;ctx.font='bold 28px Arial';ctx.textAlign="center";ctx.fillText("Фото не передано рекламной площадкой",620,510);ctx.font='22px Arial';ctx.fillText(p.source_name||"Источник",620,555);ctx.textAlign="left";}
        ...
        const sourceLabel=selectionLang==="uk"?"Джерело":selectionLang==="en"?"Source":"Источник";
        const specs=[[txt.price,money(p.price)],[txt.area,p.area?`${p.area} м²`:"—"],[roomLabel,p.rooms||"—"],[txt.floor,p.floor||"—"],[txt.district,p.district||"—"],[txt.type,p.type||"—"],[complexLabel,p.complex_name||"—"],[sourceLabel,p.source_name||"CRM"]];
        specs.forEach(([a,b],i)=>{const col=i%4,row=Math.floor(i/4),x=80+col*270,y=890+row*118;ctx.strokeStyle=pal.line;ctx.lineWidth=1;ctx.strokeRect(x,y,250,100);ctx.fillStyle=pal.muted;ctx.font='15px Arial';ctx.fillText(a,x+14,y+29);ctx.fillStyle=pal.ink;ctx.font='bold 20px Arial';pdfDrawWrapped(ctx,String(b),x+14,y+60,220,24);});
        ctx.fillStyle=pal.ink;ctx.font='bold 31px Georgia, "Times New Roman", serif';ctx.fillText(txt.about,80,1175);
        ...
        ctx.fillStyle=pal.muted;ctx.font='21px Georgia, "Times New Roman", serif';const teaserLines=canvasTextLines(ctx,preview,1080).slice(0,8);let ty=1230;teaserLines.forEach(line=>{ctx.fillText(line,80,ty);ty+=33;});
        pages.push({bytes:pdfDataUrlToBytes(canvas.toDataURL("image/jpeg",.97)),width:1240,height:1754});
        const previewLineCount=canvasTextLines(ctx,preview,1080).length;
        ...
        await addLuxuryGalleryPage(pages,{photos:extractPropertyPhotos(p),title:p.title,agencyName,themeId:theme,visualId:selectionVisual,lang:selectionLang,drawImage:drawAnalyticsPresentationImage});
      }
      ...
      const endText=selectionVisual==="luxuryDark"?"#ffffff":pal.ink;
      ec.fillStyle=endText;ec.font='bold 48px Georgia, "Times New Roman", serif';pdfDrawWrapped(ec,txt.contact,92,445,860,58);
      ec.fillStyle=pal.accent;ec.font='bold 32px Georgia, "Times New Roman", serif';pdfDrawWrapped(ec,managerDisplayName(currentProfile),92,675,850,40);
      ec.fillStyle=endText;ec.font='24px Arial';pdfDrawWrapped(ec,currentProfile?.phone||currentProfile?.email||"",92,775,850,32);ec.fillStyle=endText;ec.font='28px Georgia, "Times New Roman", serif';pdfDrawWrapped(ec,txt.thanks,92,1040,850,38);
