...
function PropertyModal({property,setProperties,onClose,users,currentProfile,role,lang}) {
  ...
  const changePropertyManager = (email) => {
    ...
    setLocal(prev => ({
      ...prev,
      property_manager_name: selected ? managerDisplayName(selected) : "",
  ...
  async function saveObject(closeAfter = false) {
    if (!canEditProperty) {
      alert("Этот объект можно только просматривать. Редактировать может ответственный менеджер или тех отдел.");
      return null;
    }
  ...
  async function addMediaBatch() {
    if (!canEditProperty) {
      alert("Этот объект можно только просматривать. Фото/видео добавляет только ответственный менеджер или тех отдел.");
      return;
    }
    ...
    // ВАЖНО: сначала сохраняем/создаём объект один раз, получаем один propertyId,
    // и уже в него загружаем все выбранные фото/видео. Так не создаются отдельные объекты под каждое фото.
    const propertyId = await saveObject(false);
    if (!propertyId) return;
    ...
    const updated = {...local, id:String(propertyId), media:[...(local.media || []), ...uploadedItems]};
    setLocal(updated);
    setProperties(prev => {
      const exists = prev.some(p => String(p.id) === String(propertyId));
      if (!exists) return [updated, ...prev];
    ...
    setMediaFile(null);
    setMediaFiles([]);
    alert("Фото/видео сохранены в один объект");
  }
  ...
  return <Modal onClose={onClose} wide>
    ...
    {!canEditProperty && <div className="card amber"><b>Режим просмотра.</b><p className="muted">Редактировать этот объект может только ответственный менеджер или тех отдел. Ты можешь смотреть объект, связаться с ответственным менеджером и отправить презентацию клиенту.</p></div>}

    <div className="grid2">
      <Field label="Название объекта"><input className="input" value={local.title || ""} disabled={!canEditProperty} onChange={e=>setLocal({...local,title:e.target.value})}/></Field>
      <Field label="Тип"><select className="input" value={local.type || types[0]} disabled={!canEditProperty} onChange={e=>setLocal({...local,type:e.target.value})}>{types.map(t=><option key={t}>{t}</option>)}</select></Field>
      <Field label="Район"><select className="input" value={local.district || districts[0]} disabled={!canEditProperty} onChange={e=>setLocal({...local,district:e.target.value})}>{districts.map(d=><option key={d}>{d}</option>)}</select></Field>
      <Field label="Цена"><input className="input" value={local.price || ""} disabled={!canEditProperty} onChange={e=>setLocal({...local,price:e.target.value})}/></Field>
      <Field label="Площадь"><input className="input" value={local.area || ""} disabled={!canEditProperty} onChange={e=>setLocal({...local,area:e.target.value})}/></Field>
      <Field label="Этаж"><input className="input" value={local.floor || ""} disabled={!canEditProperty} onChange={e=>setLocal({...local,floor:e.target.value})}/></Field>
      <Field label="Собственник"><input className="input" value={local.owner || ""} disabled={!canEditProperty} onChange={e=>setLocal({...local,owner:e.target.value})}/></Field>
      <Field label="Телефон собственника"><input className="input" value={local.ownerPhone || ""} disabled={!canEditProperty} onChange={e=>setLocal({...local,ownerPhone:e.target.value})}/></Field>
      <Field label="Ответственный менеджер по объекту">
        {isTech ? <select className="input" value={local.property_manager_email || ""} onChange={e=>changePropertyManager(e.target.value)}>
          <option value="">Выбери ответственного менеджера</option>
    ...
    <Field label="Описание"><textarea className="input" value={local.description || ""} disabled={!canEditProperty} onChange={e=>setLocal({...local,description:e.target.value})}/></Field>
    ...
    <div className="card">
      ...
      <input className="input" type="file" multiple accept="image/*,video/*" disabled={!canEditProperty} onChange={e=>{const files=Array.from(e.target.files || []); setMediaFiles(files); setMediaFile(files[0] || null);}}/>
